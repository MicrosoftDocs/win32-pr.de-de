...
Dialogfelder im  **Explorer-Stil** Öffnen und Speichern unter bieten Benutzeroberflächenfeatures, die den Folgenden Windows-Explorer. Das System unterstützt jedoch weiterhin  dialogfelder  im alten Stil öffnen und speichern unter für Anwendungen, die mit der alten Benutzeroberfläche konsistent sein müssen.

Zusätzlich zum Unterschied in der Darstellung unterscheiden sich die Dialogfelder im Explorer- und alten Stil in der Verwendung benutzerdefinierter Vorlagen und Hookverfahren zum Anpassen der Dialogfelder. Die Dialogfelder im Explorer-Stil und im alten Stil haben jedoch das gleiche Verhalten für die meisten grundlegenden Vorgänge, z. B. das Angeben eines Dateinamenfilters, das Überprüfen der Eingabe des Benutzers und das Abrufen des vom Benutzer angegebenen Dateinamens. Weitere Informationen zu den Dialogfeldern im Explorer- und alten Stil finden Sie unter [Öffnen und Speichern unter Dialogfeldanpassung.](#open-and-save-as-dialog-box-customization)
...
-   [Öffnen und Speichern unter (Dialogfeldanpassung)](#open-and-save-as-dialog-box-customization)
-   [Hookprozessen im Explorer-Stil](#explorer-style-hook-procedures)
...
Das Dialogfeld verwendet das aktuelle Verzeichnis für den aufrufenden Prozess als erstes Verzeichnis, aus dem Dateien und Verzeichnisse angezeigt werden. Verwenden Sie [**die Funktionen GetCurrentDirectory**](/windows/desktop/api/winbase/nf-winbase-getcurrentdirectory) und [**SetCurrentDirectory,**](/windows/desktop/api/winbase/nf-winbase-setcurrentdirectory) um das aktuelle Verzeichnis eines Prozesses zu erhalten und zu ändern. Um ein anderes Anfangsverzeichnis anzugeben, ohne das aktuelle Verzeichnis zu ändern, verwenden Sie das **lpstrInitialDir-Mitglied,** um den Namen eines Verzeichnisses anzugeben. Das Dialogfeld ändert automatisch Ihr aktuelles Verzeichnis, wenn der Benutzer ein anderes Laufwerk oder Verzeichnis auswählt. Um zu verhindern, dass das Dialogfeld Ihr aktuelles Verzeichnis ändert, legen Sie das **OFN \_ NOCHANGEDIR-Flag** fest. Dieses Flag verhindert nicht, dass der Benutzer die Verzeichnisse ändert, um nach einer Datei zu suchen.

Verwenden Sie den Member **lpstrDefExt,** um eine Standarddateinamenerweiterung anzugeben. Wenn der Benutzer einen Dateinamen ohne Erweiterung angibt, fügt das Dialogfeld die Standarderweiterung hinzu. Wenn Sie eine Standarderweiterung angeben und der Benutzer einen Dateinamen mit einer anderen Erweiterung angibt, legt das Dialogfeld das **FLAG OFN \_ EXTENSIONDIFFERENT** fest.

Damit der Benutzer mehrere Dateien aus einem Verzeichnis auswählen kann, legen Sie das **FLAG OFN \_ ALLOWMULTISELECT** fest. Für die Kompatibilität mit älteren Anwendungen wird im Standarddialogfeld für die Mehrfachauswahl die Benutzeroberfläche im alten Stil verwendet. Um ein Dialogfeld mit mehrfacher Auswahl im Explorer-Stil anzuzeigen, müssen Sie auch das **\_ OFN-EXPLORER-Flag** festlegen.

Wenn der Benutzer mehrere Dateien auswählt, gibt der Puffer, auf den der **lpstrFile-Member** zeigt, den Pfad zum aktuellen Verzeichnis gefolgt von den Dateinamen der ausgewählten Dateien zurück. Der **nFileOffset-Member** ist der Offset zum ersten Dateinamen, und der **nFileExtension-Member** wird nicht verwendet. In der folgenden Tabelle wird der Unterschied zwischen Dialogfeldern im Explorer-Stil und im alten Stil bei der Rückgabe mehrerer Dateinamen beschrieben.



| Stil des Dialogfelds            | Beschreibung                                                                                                                                                                                                               |
|-----------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Dialogfelder im Explorer-Stil | Die Verzeichnis- und  Dateinamenszeichenfolgen sind NULL-getrennt, mit einem zusätzlichen **NULL-Zeichen** nach dem letzten Dateinamen. Dieses Format ermöglicht es den Dialogfeldern im Explorer-Stil, lange Dateinamen zurückzugeben, die Leerzeichen enthalten. |
...
Das **nFilterIndex-Element** der [**OPENFILENAME-Struktur**](/windows/win32/api/commdlg/ns-commdlg-openfilenamea) gibt einen Index an, der angibt, welcher Filter das Dialogfeld anfänglich verwendet. Der erste Filter im Puffer verfügt über Index 1, den zweiten 2 und so weiter. Wenn der Benutzer den Filter ändert, während er das Dialogfeld verwendet, wird das **nFilterIndex-Element** bei der Rückgabe auf den Index des ausgewählten Filters festgelegt.
...
Bei Dialogfeldern im Explorer-Stil kann sich die Standarderweiterung ändern, wenn der Benutzer einen anderen Filter auswählt. Wenn der Benutzer einen Filter auswählt, dessen erstes Muster das Format auf \* hat.*xxx* (d.h. die Erweiterung enthält kein Platzhalterzeichen), das Dialogfeld verwendet *xxx* als Standarderweiterung. Dies tritt nur auf, wenn Sie eine Standarderweiterung im **lpstrDefExt-Member** der [**OPENFILENAME-Struktur**](/windows/win32/api/commdlg/ns-commdlg-openfilenamea) angegeben haben. Wenn der Benutzer z. B. die Option "Quelle \\ 0 " auswählt. \* C; \* . CXX \\ 0"-Filter, ändert sich die Standarderweiterung in "C". Wenn Sie den Filter jedoch als "Quelle \\ 0" definiert \* haben. C \* \\ 0", würde sich die Standarderweiterung nicht ändern, da die Erweiterung einen Platzhalter enthält.

Die [**CDN \_ INCLUDEITEM-Benachrichtigungsmeldung**](cdn-includeitem.md) bietet eine weitere Möglichkeit, die Namen zu filtern, die im Dialogfeld angezeigt werden. Um diese Meldung zu verwenden, stellen Sie eine [**OFNHookProc-Hookprozedur**](/windows/win32/api/commdlg/nc-commdlg-lpofnhookproc) bereit, und geben Sie beim Erstellen des Dialogfelds das **FLAG OFN \_ ENABLEINCLUDENOTIFY** in der [**OPENFILENAME-Struktur**](/windows/win32/api/commdlg/ns-commdlg-openfilenamea) an. Jedes Mal, wenn der Benutzer einen Ordner öffnet, sendet das Dialogfeld eine **CDN \_ INCLUDEITEM-Benachrichtigung** an Die Hookprozedur für jedes Element im neu geöffneten Ordner. Der Rückgabewert der Hookprozedur gibt an, ob das Element im Dialogfeld in der Elementliste des Ordners angezeigt werden soll.
...
Im Dialogfeld werden vom Benutzer typifizierte Dateinamen automatisch überprüft, um sicherzustellen, dass die Namen nur gültige Zeichen enthalten. Um die Überprüfung des Dateinamenzeichens zu überschreiben, legen Sie das **OFN \_ NOVALIDATE-Flag** fest.

Um zu erzwingen, dass das Dialogfeld überprüft, ob der Benutzer den Namen einer vorhandenen Datei angegeben hat, legen Sie das **OFN \_ FILEMUSTEXIST-Flag** fest. Um die Überprüfung zu erzwingen, dass der angegebene Pfad vorhanden ist, legen Sie das **OFN \_ PATHMUSTEXIST-Flag** fest. Wenn Sie das **OFN \_ CREATEPROMPT-Flag** festlegen, fordert das Dialogfeld den Benutzer zur Berechtigung auf, eine nicht vorhandene Datei zu erstellen. Wenn dieses Flag festgelegt ist und der Benutzer eine neue Datei erstellt, wird das Dialogfeld geschlossen, und die Funktion gibt den angegebenen Namen zurück. Andernfalls bleibt das Dialogfeld geöffnet.

Wenn Sie das **Dialogfeld** Speichern unter verwenden, können Sie das Dialogfeld dazu auffordern, den Benutzer auf die Berechtigung zum Überschreiben einer vorhandenen Datei zu bitten, indem Sie das **OFN \_ OVERWRITEPROMPT-Flag** festlegen.

Standardmäßig erstellt das Dialogfeld eine Testdatei der Länge 0 (null), um zu bestimmen, ob eine neue Datei im ausgewählten Verzeichnis erstellt werden kann. Um die Erstellung dieser Testdatei zu verhindern, legen Sie das **OFN \_ NOTESTFILECREATE-Flag** fest.

Wenn Sie eine Hookprozedur aktivieren, benachrichtigt das Dialogfeld Ihre Hookprozedur, wenn ein Verstoß gegen die Netzwerkfreigabe für den vom Benutzer angegebenen Dateinamen auftritt. Wenn Sie das **\_ OFN-EXPLORER-Flag** festlegen, sendet das Dialogfeld die [**CDN \_ SHAREVIOLATION-Nachricht**](cdn-shareviolation.md) an die Hookprozedur. Wenn Sie **OFN \_ EXPLORER** nicht festlegen, sendet das Dialogfeld die registrierte [**SHAREVISTRING-Nachricht**](sharevistring.md) an die Hookprozedur. Um zu verhindern, dass das Dialogfeld Benachrichtigungen zu Freigabeverstößen sendet, legen Sie **das OFN \_ SHAREAWARE-Flag** fest.

Wenn der Benutzer das Kontrollkästchen schreibgeschützt aktiviert, legt das Dialogfeld das **OFN \_ READONLY-Flag** bei der Rückgabe fest. Um das **Kontrollkästchen Als schreibgeschützt öffnen auszublenden,** legen Sie das **OFN \_ HIDEREADONLY-Flag** fest. Um zu verhindern, dass das Dialogfeld Namen vorhandener Dateien mit dem schreibgeschützten Attribut zurückgibt, legen Sie das **FLAG OFN \_ NOREADONLYRETURN** fest.
...
Derzeit können unter diesem Schlüssel nur fünf Einträge enthalten sein, und der Wert/Name-Index ist null-basiert. Die Namen für die Einträge sollten Place0, Place1, Place2, Place3 und Place4 sein. Die Werte der Einträge können **REG \_ DWORD-,** **REG \_ SZ-** oder **REG EXPAND \_ \_ SZ-Werte** sein, die Orte identifizieren, die in die Stellenleiste ein- und ausdehnen sollen.
