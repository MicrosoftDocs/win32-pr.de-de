---
description: Stellt den Status des Schlüssel-Wert-Paaraustauschdiensts dar, der einen Mechanismus zum Austauschen von Daten zwischen dem virtuellen Computer und dem Betriebssystem bietet, das auf dem Verwaltungsbetriebssystem ausgeführt wird.
...
ms.locfileid: "117811717"
---
# <a name="msvm_kvpexchangecomponent-class"></a>Msvm \_ KvpExchangeComponent-Klasse

Stellt den Status des Schlüssel-Wert-Paaraustauschdiensts dar, der einen Mechanismus zum Austauschen von Daten zwischen dem virtuellen Computer und dem Betriebssystem bietet, das auf dem Verwaltungsbetriebssystem ausgeführt wird.
...
Die **Msvm \_ KvpExchangeComponent-Klasse** verfügt über die folgenden Membertypen:
...
| [**RequestStateChange**](msvm-kvpexchangecomponent-requeststatechange.md) | Fordert eine Zustandsänderung an.<br/>      |
...
Der aktivierte Zustand des Elements. Diese Eigenschaft wird von [**CIM \_ EnabledLogicalElement geerbt.**](/previous-versions//cc136818(v=vs.85))
...
Ein Array eingebetteter [**Msvm \_ KvpExchangeDataItem-Instanzen,**](msvm-kvpexchangedataitem.md) die den Satz von Schlüssel-Wert-Paaren enthalten, die dienste, die im Gastbetriebssystem ausgeführt werden, per Push nach oben geordnet wurden, um für den Zugriff durch externe Clients verfügbar zu sein. Dieses Array enthält keine systeminternen Elemente, die direkt vom Integrationsdienst per Push übergeben werden.
...
Ein Array eingebetteter [**Msvm \_ KvpExchangeDataItem-Instanzen,**](msvm-kvpexchangedataitem.md) die den Satz von Schlüssel-Wert-Paaren enthalten, die das Gastbetriebssystem per Push nach oben übermittelt hat, um für den Zugriff durch externe Clients verfügbar zu sein. Dieses Array enthält keine Datenelemente, die von anderen Diensten, die im Gastbetriebssystem ausgeführt werden, per Push übermittelt werden.
...
Das Datum oder die Uhrzeit der letzten Änderung des aktivierten Zustands des Elements. Diese Eigenschaft wird von [**CIM \_ EnabledLogicalElement geerbt,**](/previous-versions//cc136818(v=vs.85))aber nicht verwendet.
...
Der Zugriff auf die **Msvm \_ KvpExchangeComponent-Klasse** kann durch UAC-Filterung eingeschränkt werden. Weitere Informationen finden Sie unter [Benutzerkontensteuerung und WMI.](/windows/desktop/WmiSdk/user-account-control-and-wmi)
