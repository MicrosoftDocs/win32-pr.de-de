...
---
# <a name="msvm_kvpexchangecomponent-class"></a>MSVM- \_ kvpexchangecomponent-Klasse

Stellt den Status des Schlüssel-Wert-Paar-Exchange-Dienstanbieter dar, der einen Mechanismus zum Austauschen von Daten zwischen dem virtuellen Computer und dem Betriebssystem bereitstellt, das unter dem Verwaltungs Betriebssystem ausgeführt wird.
...
Die **MSVM- \_ kvpexchangecomponent** -Klasse verfügt über diese Typen von Membern:
...
Die **MSVM- \_ kvpexchangecomponent** -Klasse verfügt über diese Methoden.
...
| [**RequestStateChange**](msvm-kvpexchangecomponent-requeststatechange.md) | Fordert eine Statusänderung an.<br/>      |
| [**Zurücksetzen**](msvm-kvpexchangecomponent-reset.md)                           | Setzt die Komponente zurück.<br/>         |
...
Die **MSVM- \_ kvpexchangecomponent** -Klasse verfügt über diese Eigenschaften.
...
Der aktivierte Status des Elements. Diese Eigenschaft wird von [**CIM \_ enabledlogicalelement**](/previous-versions//cc136818(v=vs.85))geerbt.
...
Ein Array von eingebetteten [**MSVM- \_ kvpexchangedataitem**](msvm-kvpexchangedataitem.md) -Instanzen, die den Satz von Schlüssel-Wert-Paaren enthalten, die von Diensten, die im Gast Betriebssystem ausgeführt werden, für den Zugriff durch externe Clients bereitgestellt wurden. Dieses Array enthält keine systeminternen Elemente, die durch den Integrations Dienst direkt per Pushvorgang übermittelt werden.
...
Ein Array von eingebetteten [**MSVM- \_ kvpexchangedataitem**](msvm-kvpexchangedataitem.md) -Instanzen, die den Satz von Schlüssel-Wert-Paaren enthalten, die das Gast Betriebssystem für den Zugriff durch externe Clients bereitgestellt hat. Dieses Array enthält keine Datenelemente, die von anderen Diensten übermittelt werden, die innerhalb des Gast Betriebssystems ausgeführt werden.
...
Das Datum oder die Uhrzeit, zu dem der aktivierte Status des Elements zuletzt geändert wurde. Diese Eigenschaft wird von [**CIM \_ enabledlogicalelement**](/previous-versions//cc136818(v=vs.85))geerbt, aber nicht verwendet.
...
Der Zugriff auf die **MSVM \_ kvpexchangecomponent** -Klasse kann durch die UAC-Filterung eingeschränkt werden. Weitere Informationen finden Sie unter [Benutzerkontensteuerung und WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
