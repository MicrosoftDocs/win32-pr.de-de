...
---
# <a name="msvm_processor-class"></a>MSVM- \_ Prozessor Klasse

Stellt den virtuellen Prozessor in einem virtuellen Computer dar.
...
Die **MSVM- \_ Prozessor** Klasse verfügt über diese Typen von Membern:
...
Die **MSVM- \_ Prozessor** Klasse verfügt über diese Methoden.
...
| [**RequestStateChange**](msvm-processor-requeststatechange.md) | Fordert eine Statusänderung an.<br/>      |
| [**Zurücksetzen**](msvm-processor-reset.md)                           | Setzt das virtuelle Gerät zurück.<br/>    |
...
Die **MSVM- \_ Prozessor** Klasse verfügt über diese Eigenschaften.
...
Alle zusätzlichen Verfügbarkeit und den Status des Geräts, außer den in der **Availability** -Eigenschaft angegebenen. Die **Availability** -Eigenschaft gibt den primären Status und die Verfügbarkeit des Geräts an. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt.
...
Die Prozessor Adress Breite in Bits. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt, und der Wert ist entweder 32 oder 64, abhängig von den Merkmalen der virtuellen Maschine.
...
Der aktuelle Status des Prozessors. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.
...
Der Name der Klasse oder Unterklasse, die bei der Erstellung einer-Instanz verwendet wird. Wenn diese Eigenschaft mit anderen Schlüsseleigenschaften der-Klasse verwendet wird, können alle Instanzen der-Klasse und deren Unterklassen eindeutig identifiziert werden. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt.
...
Die maximale Geschwindigkeit des physischen Prozessors unter Berücksichtigung der Kapazität für den virtuellen Prozessor. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.
...
Die Prozessor Daten Breite in Bits. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt, und der Wert ist entweder 32 oder 64, abhängig von den Merkmalen der virtuellen Maschine.
...
Eine Adresse oder andere identifizierende Informationen, um das logische Gerät eindeutig zu benennen. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt und ist immer auf "Microsoft:*GUID* \\ *gerätespezifische Daten*" festgelegt.
...
Die Geschwindigkeit der externen busuhr des zugrunde liegenden physischen Prozessors. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.
...
Die Prozessorfamilie des zugrunde liegenden physischen Prozessors. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.
...
Ein Array von Freiform-Zeichen folgen, die Erklärungen und Details hinter den Einträgen im " [**OtherIdentifyingInfo**](msvm-keyboard.md) "-Array bereitstellen. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt und ist immer auf **null** festgelegt.
...
Der aktuelle Prozentsatz der gesamten Verarbeitungsleistung, die vom virtuellen Computer beansprucht wird. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.

</dd> <dt>

**Loadprozentuagehistory**
</dt> <dd> <dl> <dt>
...
Der aufgezeichnete Verlauf des Prozentsatzes der gesamten Verarbeitungsleistung, die vom virtuellen Computer beansprucht wird. Dabei handelt es sich um ein Array mit Beispielen.
...
Die maximale Taktfrequenz des virtuellen Prozessors in Megahertz. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.
...
Eine Beschreibung des Typs der Prozessorfamilie. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt und ist immer auf **null** festgelegt.
...
Alle zusätzlichen Daten über Geräte-ID-Informationen, die zur Identifizierung eines logischen Geräts verwendet werden könnten. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt.
...
Gibt an, ob die Energie Verwaltung auf dem Gerät unterstützt wird. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt, aber nicht verwendet.
...
Der zuletzt angeforderte oder gewünschte Zustand für das Element. Diese Eigenschaft wird von [**CIM \_ enabledlogicalelement**](/previous-versions//cc136818(v=vs.85))geerbt.
...
Eine Zeichenfolge, die die Rolle des Prozessors beschreibt. Beispielsweise "Central Processor" oder "math Processor". Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.
...
Die Revisions Ebene des Prozessors in der Prozessorfamilie des zugrunde liegenden physischen Prozessors. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.
...
Der Name des Bereichs Systems. Dieser Wert entspricht dem Wert der [**Name**](msvm-computersystem.md) -Eigenschaft der **MSVM \_ Computersystem** -Klasse für die Bereichs bezogene virtuelle Maschine. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt.
...
Eine Globally Unique Identifier für den Prozessor. Dieser Bezeichner kann nur innerhalb einer Prozessorfamilie eindeutig sein. Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt und ist immer auf **null** festgelegt.
...
Die CPU-Socketinformationen, die Daten zum Upgrade des Prozessors enthalten (wenn Upgrades unterstützt werden). Diese Eigenschaft wird vom [**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)geerbt.

</dd> </dl>

## <a name="remarks"></a>Bemerkungen

Der Zugriff auf die **MSVM- \_ Prozessor** Klasse kann durch die UAC-Filterung eingeschränkt werden. Weitere Informationen finden Sie unter [Benutzerkontensteuerung und WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
...
[**CIM- \_ Prozessor**](/windows/desktop/CIMWin32Prov/cim-processor)
</dt> <dt>
