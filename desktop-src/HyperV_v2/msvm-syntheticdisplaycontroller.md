---
description: Stellt den Zustand des synthetischen Anzeigecontrollers dar, der in jeder Konfiguration des virtuellen Computers vorhanden ist.
...
ms.locfileid: "118949889"
---
# <a name="msvm_syntheticdisplaycontroller-class"></a>Msvm \_ SyntheticDisplayController-Klasse

Stellt den Zustand des synthetischen Anzeigecontrollers dar, der in jeder Konfiguration des virtuellen Computers vorhanden ist. Auf einem virtuellen Computer kann jederzeit nur ein Anzeigecontroller aktiv sein, und der synthetische Controller kann nur aktiviert werden, wenn das Gastbetriebssystem die erforderlichen Videobeschleunigungsdienste geladen hat.
...
Die **Msvm \_ SyntheticDisplayController-Klasse** verfügt über folgende Typen von Membern:
...
Die **Msvm \_ SyntheticDisplayController-Klasse** verfügt über diese Methoden.
...
| [**RequestStateChange**](msvm-syntheticdisplaycontroller-requeststatechange.md) | Fordert eine Zustandsänderung an.<br/>      |
| [**Zurücksetzen**](msvm-syntheticdisplaycontroller-reset.md)                           | Setzt das virtuelle Gerät zurück.<br/>    |
...
Die **Msvm \_ SyntheticDisplayController-Klasse** verfügt über diese Eigenschaften.
...
Die Grafiken und 3D-Funktionen des Anzeigecontrollers. Diese Eigenschaft wird von [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))geerbt und immer auf 2 (Grafikbeschleunigung) festgelegt.
...
Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt und immer auf 6 (Nicht zutreffend) festgelegt.
...
Ein Array von Freiformzeichenfolgen, die ausführlichere Erklärungen für alle Videobeschleunigungsfunktionen bereitstellen, die im **AcceleratorCapabilities-Eigenschaftenarray** angegeben sind. Jeder Eintrag dieses Arrays bezieht sich auf den Eintrag im **AcceleratorCapabilities-Eigenschaftenarray,** das sich am gleichen Index befindet. Diese Eigenschaft wird von [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))geerbt und immer auf "Graphics Accelerator" festgelegt.
...
Eine kurze Beschreibung des -Objekts. Diese Eigenschaft wird von [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)geerbt und immer auf "Display Controller" festgelegt.
...
Der Name der Klasse oder Unterklasse, die bei der Erstellung einer Instanz verwendet wird. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt und immer auf "Msvm \_ SyntheticDisplayController" festgelegt.
...
Eine Beschreibung des -Objekts. Diese Eigenschaft wird von [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)geerbt und immer auf "Microsoft Synthetic Display Controller" festgelegt.
...
Ein Anzeigename für das Objekt. Diese Eigenschaft wird von [**CIM \_ ManagedElement**](/previous-versions/windows/desktop/iscsitarg/cim-managedelement)geerbt und ist standardmäßig immer auf "Display Controller" festgelegt.
...
Der aktivierte und deaktivierte Status eines Elements. Sie kann auch die Übergänge zwischen diesen angeforderten Zuständen angeben. Diese Eigenschaft wird von [**CIM \_ EnabledLogicalElement**](/previous-versions//cc136818(v=vs.85))geerbt und immer auf 2 (Aktiviert) oder 3 (Deaktiviert) festgelegt.
...
Die aktuelle Integrität des Elements. Dieses Attribut drückt die Integrität dieses Elements aus, jedoch nicht notwendigerweise die Integrität seiner Unterelemente. Die möglichen Werte sind 0 bis 30, wobei 5 bedeutet, dass das Element vollständig fehlerfrei ist und 30 bedeutet, dass das Element vollständig nicht funktionslos ist. Diese Eigenschaft wird von [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement)geerbt und immer auf 5 (OK) festgelegt.
...
Die maximal unterstützte Arbeitsspeichermenge in Bytes. Diese Eigenschaft wird von [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))geerbt und immer auf 4.194.304 (0x400000) festgelegt.
...
Die Anzahl der unterstützten Videoseiten bei den aktuellen Auflösungen und dem verfügbaren Arbeitsspeicher. Diese Eigenschaft wird von [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))geerbt und immer auf 1024 festgelegt.
...
Der Videospeichertyp, wenn die **VideoMemoryType-Eigenschaft** der Instanz 1 (Sonstige) ist. Diese Eigenschaft wird von [**CIM \_ DisplayController**](/previous-versions//cc136810(v=vs.85))geerbt und immer auf **NULL** festgelegt.
...
Eine Zeichenfolge, die weitere Informationen im Zusammenhang mit dem vom Controller unterstützten Protokoll bereitstellt. Diese Eigenschaft wird vom [**\_ CIM-Controller**](/windows/desktop/CIMWin32Prov/cim-controller)geerbt und immer auf "Video" festgelegt.
...
Das Protokoll, das vom Controller für den Zugriff auf kontrollierte Geräte verwendet wird. Diese Eigenschaft wird vom [**\_ CIM-Controller**](/windows/desktop/CIMWin32Prov/cim-controller)geerbt und immer auf 1 (Sonstige) festgelegt.
...
Das letzte Einschalten des virtuellen Computers. Diese Eigenschaft wird vom [**\_ CIM-Controller geerbt.**](/windows/desktop/CIMWin32Prov/cim-controller)
...
Gibt die Videoarchitektur des Anzeigecontrollers an, die zum Generieren des Videosignals verwendet wird. In der Regel generiert ein dedizierter Videoprozessor das Videosignal in Übereinstimmung mit der angegebenen Architektur. Es ist ein Indikator für die maximale Auflösungsfunktion des Anzeigecontrollers. Diese Eigenschaft wird von [**CIM \_ DisplayController geerbt.**](/previous-versions//cc136810(v=vs.85))
...
Der Typ des Videospeichers. Diese Eigenschaft wird von [**CIM \_ DisplayController geerbt**](/previous-versions//cc136810(v=vs.85))und immer auf 2 (VRAM) festgelegt.
...
Eine Zeichenfolge, die den Videoprozessor/-controller beschreibt. Diese Eigenschaft wird von [**CIM \_ DisplayController geerbt**](/previous-versions//cc136810(v=vs.85))und immer auf "Synthetic Video Processor" festgelegt.

</dd> </dl>

## <a name="remarks"></a>Hinweise

Der Zugriff auf die **Msvm \_ SyntheticDisplayController-Klasse** kann durch UAC-Filterung eingeschränkt werden. Weitere Informationen finden Sie unter [Benutzerkontensteuerung und WMI.](/windows/desktop/WmiSdk/user-account-control-and-wmi)
