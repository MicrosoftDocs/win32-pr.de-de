...
Beschreibt einen virtuellen Ressourcentyp, der für die Verwendung auf virtuellen Computern verfügbar ist. Der Ressourcenpool aggregiert physische Ressourcen und wird zum Zuordnen von Ressourcen zu virtuellen Computern verwendet. In Hyper-V sind alle Ressourcenpools ursprünglicher, und es gibt genau einen Pool für jeden bestimmten Ressourcentyp, der einem virtuellen Computer zugewiesen werden kann.
...
Die **MSVM \_ resourcepool** -Klasse verfügt über diese Typen von Membern:
...
Die Zuordnungs Einheiten, die vom Ressourcenpool verwendet werden. Diese Eigenschaft wird von [**CIM \_ resourcepool**](/previous-versions//cc136903(v=vs.85))geerbt und auf "Megabyte" festgelegt.
...
Gibt die Menge an Ressourcen an, die der Ressourcenpool aktuell für Consumer darstellt. Diese Eigenschaft unterscheidet sich von der **reservierten** Eigenschaft darin, dass Sie die Consumer-Ansicht der Ressource beschreibt, während die **reservierte** Eigenschaft die Producer-Ansicht der Ressource beschreibt.
...
Wenn keine QoS-bezogenen Bedingungen erkannt wurden, wird der primäre Status (OperationalStatus \[ 0 \] ) auf OK (2) festgelegt. Andernfalls wird der primäre Status auf "heruntergestuft" (3) festgelegt, und mindestens ein sekundärer Statuswert wird in das Array eingefügt, beginnend bei Index 1, der nach dieser Tabelle spezifischere Bedingungen meldet.



| Wert                                                                                                                                                                                                    | BESCHREIBUNG                                                                                                                      |
|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| <span id="_Insufficient_Throughput___32788_"></span><span id="_insufficient_throughput___32788_"></span><span id="_INSUFFICIENT_THROUGHPUT___32788_"></span> Unzureichender Durchsatz (32788)<br/> | Mindestens eine der vom Pool zugewiesenen virtuellen Datenträger meldet derzeit einen unzureichenden Durchsatz Status.<br/> |



 

Der Hyper-V-WMI-Anbieter löst jedes Mal ein [**MSVM \_ storagealert**](msvm-storagealert.md) -Ereignis aus, wenn sich der OperationalStatus der **MSVM \_ resourcepool** -Klasse ändert.
...
Der Ressourcentyp, der von diesem Ressourcenpool möglicherweise zugeteilt wird. Diese Eigenschaft wird von [**CIM \_ resourcepool**](/previous-versions//cc136903(v=vs.85))geerbt und ist auf 4 ("Memory") festgelegt.
