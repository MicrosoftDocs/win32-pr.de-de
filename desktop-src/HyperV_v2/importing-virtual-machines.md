---
description: Im folgenden c#-Beispiel wird veranschaulicht, wie Sie die MigrateVirtualSystemToHost-Methode verwenden, um eine einfache Migration einer virtuellen Maschine auszuführen.
ms.assetid: A3E4AAA2-3AE6-4D7D-8D7B-1ED367D398C0
title: Migrieren virtueller Maschinen
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103754241"
---
# <a name="migrating-virtual-machines"></a><span data-ttu-id="575da-103">Migrieren virtueller Maschinen</span><span class="sxs-lookup"><span data-stu-id="575da-103">Migrating virtual machines</span></span>

<span data-ttu-id="575da-104">Im folgenden c#-Beispiel wird veranschaulicht, wie Sie die [**MigrateVirtualSystemToHost**](migratevirtualsystemtohost-msvm-virtualsystemmigrationservice.md) -Methode verwenden, um eine einfache Migration einer virtuellen Maschine auszuführen.</span><span class="sxs-lookup"><span data-stu-id="575da-104">The following C# sample demonstrates how to use the [**MigrateVirtualSystemToHost**](migratevirtualsystemtohost-msvm-virtualsystemmigrationservice.md) method to perform a simple migration of a virtual machine.</span></span> <span data-ttu-id="575da-105">In diesem Beispiel werden Ressourcenpools zum Abrufen der richtigen VHD-Pfade verwendet.</span><span class="sxs-lookup"><span data-stu-id="575da-105">This example uses resource pools to obtain the correct VHD paths.</span></span> <span data-ttu-id="575da-106">Dieser Code stammt aus dem Beispiel zum Migrieren [virtueller Hyper-V-Computer](https://github.com/microsoftarchive/msdn-code-gallery-microsoft/tree/master/Official%20Windows%20Platform%20Sample/Hyper-V%20virtual%20machine%20migration%20sample).</span><span class="sxs-lookup"><span data-stu-id="575da-106">This code is taken from the [Hyper-V virtual machine migration sample](https://github.com/microsoftarchive/msdn-code-gallery-microsoft/tree/master/Official%20Windows%20Platform%20Sample/Hyper-V%20virtual%20machine%20migration%20sample).</span></span>

<span data-ttu-id="575da-107">Die Befehlszeilen Syntax zum Ausführen dieses Beispiels lautet wie folgt:</span><span class="sxs-lookup"><span data-stu-id="575da-107">The command-line syntax to run this sample is:</span></span>

<span data-ttu-id="575da-108">**MigrationSamples.exe VM-und-Storage-Simple** *sourcehost* *destinationhost* *VMName*</span><span class="sxs-lookup"><span data-stu-id="575da-108">**MigrationSamples.exe vm-and-storage-simple** *SourceHost* *DestinationHost* *VmName*</span></span>

<span data-ttu-id="575da-109">die Parameter lauten wie folgt:</span><span class="sxs-lookup"><span data-stu-id="575da-109">where the parameters are as follows:</span></span>

-   <span data-ttu-id="575da-110">*Sourcehost* ist der Name des aktuellen Hosts der virtuellen Maschine.</span><span class="sxs-lookup"><span data-stu-id="575da-110">*SourceHost* is the name of the current host of the virtual machine.</span></span>
-   <span data-ttu-id="575da-111">*Destinationhost* ist der Name des Zielhosts.</span><span class="sxs-lookup"><span data-stu-id="575da-111">*DestinationHost* is the name of the destination host.</span></span>
-   <span data-ttu-id="575da-112">*VMName* ist der Name des virtuellen Computers.</span><span class="sxs-lookup"><span data-stu-id="575da-112">*VmName* is the name of the virtual machine.</span></span>
...
<span data-ttu-id="575da-113">Der folgende c#-Code enthält die Implementierung der meisten Dienstprogramm Methoden, die im obigen Beispiel aufgerufen wurden.</span><span class="sxs-lookup"><span data-stu-id="575da-113">The following C# code contains the implementation of most of the utility methods called from the example above.</span></span>
