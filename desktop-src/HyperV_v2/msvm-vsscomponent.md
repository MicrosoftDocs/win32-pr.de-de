...
---
# <a name="msvm_vsscomponent-class"></a>MSVM- \_ vsscomponent-Klasse

Stellt den Status des Volumeschattenkopie-Dienst-Diensts (VSS) dar, der die VSS-Anforderer im Gast Betriebssystem implementiert.
...
Die **MSVM- \_ vsscomponent** -Klasse verfügt über diese Typen von Membern:
...
Die **MSVM- \_ vsscomponent** -Klasse verfügt über diese Methoden.
...
| [**RequestStateChange**](msvm-vsscomponent-requeststatechange.md) | Fordert eine Statusänderung an.<br/>      |
| [**Zurücksetzen**](msvm-vsscomponent-reset.md)                           | Setzt das virtuelle Gerät zurück.<br/>    |
...
Die **MSVM- \_ vsscomponent** -Klasse verfügt über diese Eigenschaften.
...
Eine Zeichenfolge, die weitere Informationen über den Fehler enthält, der in **LastErrorCode** aufgezeichnet wurde, sowie Informationen zu den Maßnahmen, die ausgeführt werden können. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice) geerbt, aber nicht verwendet.
...
Ein Array von Freiform-Zeichen folgen, die Erklärungen und Details hinter den Einträgen im " **OtherIdentifyingInfo** "-Array bereitstellen. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt, aber nicht verwendet.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <dl> <dt>2,2</dt> </dl>                                                                                                                                                                                                    |                                                                                                                                                                                                                                          |
| <span id="OK"></span><span id="ok"></span><dl> <dt>**OK**</dt> <dt>2</dt> </dl>                                                                                                  | Der Dienst wird normal ausgeführt. Die Eigenschaftswerte **OperationalStatus** \[ 1 \] und **Statusbeschreibungen** \[ 1 \] enthalten möglicherweise weitere Informationen.<br/>                                                                               |
| <span id="Degraded"></span><span id="degraded"></span><span id="DEGRADED"></span><dl> <dt></dt> Heruntergestuft <dt>3</dt> </dl>                                                     | Der Dienst wird normal ausgeführt, aber der Gast Dienst hat eine kompatible Kommunikationsprotokoll Version ausgehandelt. Die Eigenschaftswerte **OperationalStatus** \[ 1 \] und **Statusbeschreibungen** \[ 1 \] enthalten möglicherweise weitere Informationen.<br/> |
| <span id="Non-Recoverable_Error"></span><span id="non-recoverable_error"></span><span id="NON-RECOVERABLE_ERROR"></span><dl> <dt>**Nicht BEHEB barer Fehler**</dt> <dt>7</dt> </dl> | Der Gast unterstützt keine kompatible Protokollversion. Die Eigenschaftswerte **OperationalStatus** \[ 1 \] und **Statusbeschreibungen** \[ 1 \] enthalten möglicherweise weitere Informationen.<br/>                                                        |
| <span id="No_Contact"></span><span id="no_contact"></span><span id="NO_CONTACT"></span><dl> <dt>**Kein Kontakt**</dt> <dt>12</dt> </dl>                                            | Der Gast Dienst ist nicht installiert, oder er wurde noch nicht kontaktiert.<br/>                                                                                                                                                             |
| <span id="Lost_Communication"></span><span id="lost_communication"></span><span id="LOST_COMMUNICATION"></span><dl> <dt>**Verlorene Kommunikation**</dt> <dt>13</dt> </dl>            | Der Gast Dienst antwortet nicht mehr normal.<br/>                                                                                                                                                                           |
...
Zusätzliche Daten über Geräte-ID-Informationen, die zur Identifizierung eines logischen Geräts verwendet werden könnten. Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](/windows/desktop/CIMWin32Prov/cim-logicaldevice)geerbt und ist immer auf **null** festgelegt.
...
Eine Zeichenfolge, die den aktuellen Status des-Objekts angibt. Diese Eigenschaft wird von [**CIM \_ ManagedSystemElement**](/windows/desktop/CIMWin32Prov/cim-managedsystemelement) geerbt, aber nicht verwendet.
...
Das Datum oder die Uhrzeit der letzten Änderung des **enabledstate** des Elements. Diese Eigenschaft wird von [**CIM \_ enabledlogicalelement**](/previous-versions//cc136818(v=vs.85))geerbt und ist immer auf **null** festgelegt.
...
Der Zugriff auf die **MSVM \_ vsscomponent** -Klasse kann durch die UAC-Filterung eingeschränkt werden. Weitere Informationen finden Sie unter [Benutzerkontensteuerung und WMI](/windows/desktop/WmiSdk/user-account-control-and-wmi).
