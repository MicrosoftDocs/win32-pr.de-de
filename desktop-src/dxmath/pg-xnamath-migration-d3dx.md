...
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: 0f16f0c8a78a120686243d464eafa63061abd13b877ffcae340eab224243fca3
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 08/11/2021
ms.locfileid: "119118130"
---
# <a name="working-with-d3dxmath"></a>Verwenden von D3DXMath

D3DXMath ist eine mathematische Hilfsbibliothek für Direct3D-Anwendungen. D3DXMath ist seit langem vorhanden, ist in D3DX 9 und D3DX 10 enthalten und stammt ebenfalls aus älteren Versionen von DirectX.

> [!Note]  
> Die D3DX-Hilfsprogrammbibliothek (D3DX 9, D3DX 10 und D3DX 11) ist für Windows 8 veraltet. Daher wird dringend empfohlen, zu DirectXMath zu migrieren, anstatt D3DXMath zu verwenden.

 

DirectXMath nutzt einen Großteil derselben Funktionalität in D3DXMath, und intern enthält D3DXMath eine Reihe von prozessorspezifischen Optimierungen. Der Hauptunterschied besteht darin, dass D3DXMath im D3DX9 gehostet \* wird. DLLs und D3DX10 \* . DLLs und nur wenige der Funktionen sind inline. Die Aufrufkonvention der DirectXMath-Bibliothek ist explizit SIMD-freundlicher, während D3DXMath Lade- und Speicherkonvertierungen ausführen muss, um die SIMD-Optimierung zu implementieren.

## <a name="mixing-directxmath-and-d3dxmath"></a>Kombinieren von DirectXMath und D3DXMath

D3DX11 enthält nicht D3DXMath, und im Allgemeinen wird die Verwendung von DirectXMath empfohlen. Sie können jedoch weiterhin eine Verknüpfung mit D3DX9 und/oder D3DX10 in Ihrer Anwendung erstellen. Daher können Sie weiterhin D3DXMath oder D3DXMath und DirectXMath gleichzeitig in Ihrer Anwendung verwenden.

Es ist im Allgemeinen sicher, einen XMVECTOR in eine Funktion zu \* casten, die D3DXVECTOR4 annimmt, \* oder eine XMMATRIX in eine Funktion zu \* casten, die D3DXMATRIX \* annimmt. Die Umkehrung ist jedoch im Allgemeinen nicht sicher, da XMVECTOR und XMMATRIX 16-Byte-ausgerichtet sein müssen, während D3DXVECTOR4 und D3DXMATRIX keine solche Anforderung haben. Wenn diese Anforderung nicht erfüllt wird, kann dies zur Laufzeit zu Ungültigen Ausrichtungsausnahmen führen.

Es ist sicher, einen XMVECTOR in eine Funktion zu \* umwandeln, die D3DXVECTOR2 \* oder D3DXVECTOR3 \* annimmt, aber nicht umgekehrt. Sowohl die Ausrichtung als auch die Tatsache, dass D3DXVECTOR2 und D3DXVECTOR3 kleinere Strukturen sind, machen dies zu einem unsicheren Vorgang.

> [!Note]  
> D3DX (und somit D3DXMath) gilt als veraltet und ist nicht für Windows Store Apps verfügbar, die auf Windows 8 ausgeführt werden und nicht im Windows 8 SDK für Desktop-Apps enthalten sind.

 

## <a name="using-directxmath-with-direct3d"></a>Verwenden von DirectXMath mit Direct3D

DirectXMath und D3DXMath sind bei der Arbeit mit Direct3D optional. Direct3D 9 definierte D3DMATRIX und D3DCOLOR als Teil der Direct3D-API zur Unterstützung der (jetzt veralteten) Pipeline mit festen Funktionen. D3DXMath in D3DX9 erweitert diese Direct3D 9-Typen um allgemeine mathematische Grafikoperationen. Für Direct3D 10.x und Direct3D 11 verwendet die API nur die programmierbare Pipeline, sodass es keine API-spezifische Struktur für Matrizen oder Farbwerte gibt. Wenn die neueren APIs einen Farbwert erfordern, verwenden sie ein explizites Array von float-Werten oder einen generischen Puffer konstanter Daten, die vom HLSL-Shader interpretiert werden. HLSL selbst kann entweder Zeilen- oder Spaltenhauptmatrixformate unterstützen, sodass das Layout vollständig Ihren Vorstellungen entspricht (weitere Informationen finden Sie unter HLSL, [Matrixreihenfolge.](../direct3dhlsl/dx-graphics-hlsl-per-component-math.md)Wenn Sie Spaltenhauptmatrixformate in Ihren Shadern verwenden, müssen Sie die DirectXMath-Matrixdaten beim Platzieren in Ihre konstanten Pufferstrukturen transponieren). Die Bibliotheken DirectXMath und D3DXMath sind zwar optional, bieten aber beide allgemeine Grafikfunktionen und sind daher äußerst praktisch, wenn Sie Direct3D-Programmierung durchführen.

Es ist sicher, einen XMVECTOR \* in einen D3DVECTOR oder XMMATRIX in D3DMATRIX zu casten, \* da \* \* Direct3D 9 keine Ausrichtungsannahmen über die eingehende Datenstruktur trifft. Es ist auch sicher, XMCOLOR in D3DCOLOR zu casten. Sie können eine 4-Float-Darstellung der Farbe über XMStoreColor() in XMCOLOR konvertieren, um das 32-Bit-DWORD von 8:8:8:8 abzurufen, das D3DCOLOR entspricht.

Bei der Arbeit mit Direct3D 10.x oder Direct3D 11 verwenden Sie in der Regel DirectXMath-Typen, um eine Struktur für jeden Ihrer konstanten Puffer zu erstellen. In diesen Fällen hängt dies größtenteils von Ihrer Fähigkeit ab, die Ausrichtung zu steuern, um diese effizient zu gestalten, oder \* XMStore-Vorgänge (), um XMVECTOR- und XMMATRIX-Daten in die richtigen Datentypen zu konvertieren. Beim Aufrufen von Direct3D 10.x- oder Direct3D 11-APIs, die ein Float \[ \] 4-Array von Farbwerten erfordern, können Sie einen XMVECTOR- \* oder XMFLOAT4-Wert \* mit den Farbdaten umwandlungen.
...
<tr class="odd">
<td>D3DXFLOAT16</td>
<td><a href="half-data-type.md"><strong>Hälfte</strong></a></td>
</tr>
<tr class="even">
<td>D3DXMATRIX</td>
...
<td><a href="xmvector-data-type.md"><strong>XMVECTOR</strong></a> wird anstelle eindeutiger Typen verwendet, sodass Sie wahrscheinlich <a href="/windows/desktop/api/directxmath/ns-directxmath-xmfloat4"> <strong>XMFLOAT4</strong> verwenden müssen.</a>
<blockquote>
[!Note]<br />
[<strong>D3DXQUATERNION::operator *</strong>] (.. /direct3d9/d3dxquaternion-extensions.md) ruft die <a href="/windows/desktop/direct3d9/d3dxquaternionmultiply"><strong>D3DXQuaternionMultiply-Funktion</strong></a> auf, die zwei Quaternionen multipliziert. Wenn Sie jedoch nicht explizit die <a href="/windows/desktop/api/directxmath/nf-directxmath-xmquaternionmultiply"><strong>XMQuaternionMultiply-Funktion</strong></a> verwenden, erhalten Sie eine falsche Antwort, wenn Sie <a href="xmvector-operator-mul.md"><strong>XMVECTOR::operator *</strong></a> für eine Quaternion verwenden.
</blockquote>
<br/></td>
</tr>
<tr class="odd">
...
<tr class="even">
<td>D3DXVECTOR4</td>
<td><a href="/windows/desktop/api/directxmath/ns-directxmath-xmfloat4"><strong>XMFLOAT4</strong></a>(oder , wenn Sie garantieren können, dass die Daten auf 16 Byte ausgerichtet sind, <a href="xmvector-data-type.md"><strong>XMVECTOR</strong></a> oder <a href="/previous-versions/windows/desktop/legacy/ee419609(v=vs.85)"><strong>XMFLOAT4A</strong></a> )<br/></td>
</tr>
<tr class="odd">
...
| D3DXVec2Lerp                       | [**XMVectorLerp**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerp) oder [ **XMVectorLerpV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerpv)                                                                                                                                                                   |
...
| D3DXVec2Hermite                    | [**XMVectorHermite**](/windows/win32/api/directxmath/nf-directxmath-xmvectorhermite) oder [ **XMVectorHermiteV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorhermitev)                                                                                                                                                       |
| D3DXVec2CatmullRom                 | [**XMVectorCatmullRom**](/windows/win32/api/directxmath/nf-directxmath-xmvectorcatmullrom) oder [ **XMVectorCatmullRomV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorcatmullromv)                                                                                                                                           |
| D3DXVec2BaryCentric                | [**XMVectorBaryCentric**](/windows/win32/api/directxmath/nf-directxmath-xmvectorbarycentric) oder [ **XMVectorBaryCentricV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorbarycentricv)                                                                                                                                       |
...
| D3DXVec3Lerp                       | [**XMVectorLerp**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerp) oder [ **XMVectorLerpV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerpv)                                                                                                                                                                   |
...
| D3DXVec3Hermite                    | [**XMVectorHermite**](/windows/win32/api/directxmath/nf-directxmath-xmvectorhermite) oder [ **XMVectorHermiteV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorhermitev)                                                                                                                                                       |
| D3DXVec3CatmullRom                 | [**XMVectorCatmullRom**](/windows/win32/api/directxmath/nf-directxmath-xmvectorcatmullrom) oder [ **XMVectorCatmullRomV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorcatmullromv)                                                                                                                                           |
| D3DXVec3BaryCentric                | [**XMVectorBaryCentric**](/windows/win32/api/directxmath/nf-directxmath-xmvectorbarycentric) oder [ **XMVectorBaryCentricV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorbarycentricv)                                                                                                                                       |
...
| D3DXVec4Lerp                       | [**XMVectorLerp**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerp) oder [ **XMVectorLerpV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerpv)                                                                                                                                                                   |
...
| D3DXVec4Hermite                    | [**XMVectorHermite**](/windows/win32/api/directxmath/nf-directxmath-xmvectorhermite) oder [ **XMVectorHermiteV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorhermitev)                                                                                                                                                       |
| D3DXVec4CatmullRom                 | [**XMVectorCatmullRom**](/windows/win32/api/directxmath/nf-directxmath-xmvectorcatmullrom) oder [ **XMVectorCatmullRomV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorcatmullromv)                                                                                                                                           |
| D3DXVec4BaryCentric                | [**XMVectorBaryCentric**](/windows/win32/api/directxmath/nf-directxmath-xmvectorbarycentric) oder [ **XMVectorBaryCentricV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorbarycentricv)                                                                                                                                       |
...
| D3DXMatrixRotationYawPitchRoll     | [**XMMatrixRotationRollPitchYaw**](/windows/win32/api/directxmath/nf-directxmath-xmmatrixrotationrollpitchyaw) (Beachten Sie, dass die Reihenfolge der Parameter unterschiedlich ist: D3DXMatrixRotationYawPitchRoll übernimmt yaw, pitch, roll, **XMMatrixRotationRollPitchYaw** übernimmt Pitch, yaw, roll)                 |
...
| D3DXQuaternionRotationYawPitchRoll | [**XMQuaternionRotationRollPitchYaw**](/windows/win32/api/directxmath/nf-directxmath-xmquaternionrotationrollpitchyaw) (Beachten Sie, dass die Reihenfolge der Parameter unterschiedlich ist: D3DXQuaternionRotationYawPitchRoll nimmt yaw, pitch, roll, **XMQuaternionRotationRollPitchYaw** übernimmt Pitch, yaw, roll) |
...
| D3DXQuaternionBaryCentric          | [**XMQuaternionBaryCentric**](/windows/win32/api/directxmath/nf-directxmath-xmquaternionbarycentric) oder [ **XMQuaternionBaryCentricV**](/windows/win32/api/directxmath/nf-directxmath-xmquaternionbarycentricv)                                                                                                                       |
...
| D3DXColorLerp                      | [**XMVectorLerp**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerp) oder [ **XMVectorLerpV**](/windows/win32/api/directxmath/nf-directxmath-xmvectorlerpv)                                                                                                                                                                   |
...
> [!Note]  
> [Sphärische Funktionen für](https://github.com/Microsoft/DirectXMath/tree/master/SHMath) DirectXMath sind separat verfügbar. Es gibt keine DirectXMath-Entsprechung zu **ID3DXMatrixStack.**
