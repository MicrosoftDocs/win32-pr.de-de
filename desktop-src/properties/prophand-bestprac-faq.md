...
---
# <a name="property-handler-best-practices-and-faq"></a>Bewährte Methoden und häufig gestellte Fragen zu Eigenschaftenhandlern
...
Die hier beschriebenen bewährten Methoden bieten nützliche Implementierungstipps.

### <a name="overriding-file-system-properties"></a>Überschreiben von Dateisystemeigenschaften
...
-   **Frage:** Gewusst wie wissen, welche Dateiformate offene Metadaten unterstützen?
...
-   **Frage:** Können VT \_ NULL-Werte mithilfe eines Eigenschaftenhandlers gespeichert werden?
...
-   **Frage:** Welche Datumszeichenfolgenformate werden von der [**PropVariantChangeType-Funktion**](/windows/win32/api/propvarutil/nf-propvarutil-propvariantchangetype) unterstützt?
...
-   **Frage:** Ist es möglich, einen schreibgeschützten Eigenschaftenhandler zu erstellen?
...
-   **Frage:** Kann ein Eigenschaftenhandler eine Eigenschaft als schreibgeschützt behandeln, auch wenn das Schema angibt, dass die Eigenschaft schreibbar ist?

    Ja. Im Schemasystem werden Eigenschaften als schreibgeschützt (einschließlich derjenigen mit `isInnate = "true"` ) oder Als Lese-/Schreibzugriff versehen. Eigenschaftenhandler, die das Schreiben einer bestimmten Eigenschaft, die das Schema besagt, nicht unterstützen, sollten [**IPropertyStoreCapabilities**](/windows/win32/api/propsys/nn-propsys-ipropertystorecapabilities) implementieren und S \_ FALSE bei Aufrufen von [**IPropertyStoreCapabilities::IsPropertyWritable**](/windows/win32/api/propsys/nf-propsys-ipropertystorecapabilities-ispropertywritable) für diese Eigenschaft zurückgeben. Dies gibt an, dass die -Eigenschaft im Kontext dieses Handlers und dieser Datei nicht schreibbar ist.

    > [!Note]  
    > Die umgekehrte Aktion ist nicht möglich. Sie können es einem Eigenschaftenhandler nicht ermöglichen, eine Eigenschaft zu schreiben, die im Schema als schreibgeschützt markiert ist.
