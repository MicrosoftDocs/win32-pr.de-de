...
Ein Beispiel mit mehreren Typen grafischer Elemente.

Grafische Elemente sind in der Regel nicht interaktiv. Trennzeichen sind jedoch interaktiv für inhalte, deren Größe geändert werden kann, und Handles sind Grafiken, die Interaktivität anzeigen.
...
Grafikelemente sind zwar ein sicheres visuelles Mittel zum Angeben von Beziehungen, aber die übermäßig hohe Nutzung sorgt für visuelle Überladenheit und reduziert den verfügbaren Platz auf einer Oberfläche. Sie sollten nur selten verwendet werden.
...
-   **Ist die Kommunikation ohne Text effektiv?** Verwenden Sie andernfalls ein [Gruppenfeld,](ctrl-group-boxes.md)ein bezeichnetes Trennzeichen oder eine andere [Bezeichnung.](text-ui.md)
...
-   **Grafiken sind am effektivsten, wenn sie eine einzelne einfache Idee verstärken.** Komplexe Grafiken, die interpretiert werden müssen, funktionieren nicht gut. Hieroglyphen sind am besten für Cave-Zeichnungen übrig.
    ...
    **Falsch:**
    ...
    In diesem Beispiel versucht eine komplexe Grafik aus Windows XP ineffektiv, eine komplexe Vertrauensentscheidung zu erklären.
...
-   **Vermeiden Sie in Ihren Entwürfen Reines Rot, Gelb und Grün.** Um Verwirrung zu vermeiden, reservieren Sie diese Farben, um den Status zu kommunizieren. Wenn Sie diese Farben für etwas anderes als status verwenden müssen, verwenden Sie stummgeschaltete Töne anstelle von reinen Farben.
...
-   **Um Den Inhalt hervorzuheben, verwenden Sie dunklen Text auf einem hellen Hintergrund.** Schwarzer Text auf einem hellgrauen oder gelben Hintergrund funktioniert gut.

    ![Screenshot des Blaulinktexts im gelben Hintergrund ](images/vis-graphic-image15.png)

    In diesem Beispiel erhält der Link die Aufmerksamkeit des Benutzers, da er sich auf einem gelben Hintergrund befindet.

-   **Um den Inhalt zu deaktivieren, verwenden Sie hellen Text auf einem dunklen Hintergrund.** Weißer Text auf einem dunkelgrauen oder blauen Hintergrund funktioniert gut.
...
-   **Wenn ein Farbverlauf verwendet wird, stellen Sie sicher, dass die Textfarbe einen guten Kontrast über den gesamten Farbverlauf aufweist.**
-   **Verwenden Sie immer ein 16 x 16 Pixel großes Symbol, um die Aufmerksamkeit auf das Banner zu lenken.** Banner sind andernfalls zu leicht zu übersehen. Weitere Richtlinien und Beispiele finden Sie unter [Standardsymbole.](vis-std-icons.md)
-   **Verwenden Sie Hintergründe und Banner mit Vorsicht.** Während die Absicht des Hintergrunds oder Banners darin bestehen kann, Inhalte hervorzuheben, sind die Ergebnisse häufig das Gegenteil eines Satzes, der als "Bannerblindheit" bezeichnet wird.
...
-   **Verwenden Sie vertikale und horizontale Linien für Trennzeichen.** Stellen Sie sicher, dass zwischen den Trennzeichen und dem zu trennenden Inhalt ausreichend Speicherplatz vorhanden ist.
-   Zeigen Sie für Trennzeichen zwischen anskalierbaren Inhalten (Teilern) den Größenänderungszeiger an, wenn Sie darauf zeigen.

![Screenshot: Splitter mit Größenänderungszeiger](images/vis-graphic-image17.png)

![Screenshot des Splitters mit Zeiger zum Ändern der Größe ](images/vis-graphic-image18.png)

In diesen Beispielen werden Zeiger zur Größenänderung angezeigt, wenn sie darauf zeigen.

### <a name="shadows"></a>Shadows

-   **Verwenden Sie nur , um die wichtigsten Inhalte oder Objekte Ihres Programms visuell vor dem Hintergrund hervorzuheben.** Schatten sollten unter anderen Umständen visuell unübersichtlich sein.

### <a name="high-dpi-support"></a>Unterstützung für hohe DPI-Daten

-   **Unterstützung von Videomodi mit 96 und 120 Punkten pro Zoll (dpi).** Erkennen Sie den DPI-Modus beim Start, und behandeln Sie DPI-Änderungsereignisse. Windows ist für 96 und 120 dpi optimiert und verwendet standardmäßig 96 dpi.
-   **Es empfiehlt sich, separate Bitmaps bereitzustellen, die speziell für 96- und 120 dpi-Grafiken gerendert werden, gegenüber skalierungsgrafiken.** Stellen Sie mindestens 96- und 120 dpi-Versionen für die wichtigsten, sichtbarsten Bitmaps bereit, und zentrieren oder skalieren Sie die anderen. Solche Anwendungen gelten als "hochdpi-fähige" Anwendungen und bieten eine bessere visuelle Gesamtdarstellung als Programme, die automatisch von Windows skaliert werden.
    -   Entwickler: Sie können ein Programm mit hoher DPI-Erkennung deklarieren (und die automatische Skalierung verhindern), indem Sie das dpi-fähige Flag im Programmmanifest festlegen oder während der Programminitialisierung die SetProcessDPIAware()-API aufrufen. Sie können Makros verwenden, um die Auswahl der richtigen Grafiken zu vereinfachen. Für Win32-Bitmaps können Sie SS \_ CENTERIMAGE zum Zentrieren oder SS \_ REALSIZECONTROL zum Skalieren verwenden.
-   Überprüfen Sie Ihr Programm sowohl in 96 als auch in 120 dpi auf:
    -   Grafiken, die zu klein oder zu groß sind.
    -   Abgeschnittene, überlappende oder anderweitig nicht passende Grafiken.
    -   Grafiken, die schlecht gestreckt sind ("pixilated").
    -   Text, der abgeschnitten ist oder in grafischen Hintergründen nicht passt.

## <a name="text"></a>Text

-   **Verwenden Sie für Barrierefreiheit und Lokalisierung keinen Text in Grafiken.** Machen Sie Ausnahmen nur, um [Branding](exper-branding.md) und Text als abstraktes Konzept darzustellen.
