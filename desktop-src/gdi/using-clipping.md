...
---
# <a name="using-clipping"></a><span data-ttu-id="132c7-104">Verwenden von Clipping</span><span class="sxs-lookup"><span data-stu-id="132c7-104">Using Clipping</span></span>

<span data-ttu-id="132c7-105">Dieser Abschnitt enthält Beispielcode, in dem gezeigt wird, wie ein Clip Pfad generiert wird, der aus einer Zeichenfolge besteht.</span><span class="sxs-lookup"><span data-stu-id="132c7-105">This section contains example code that shows how to generate a clip path consisting of a character string.</span></span> <span data-ttu-id="132c7-106">Im Beispiel wird eine logische Schriftart erstellt und verwendet, um eine Zeichenfolge innerhalb eines Clip Pfades zu zeichnen, und dann wird der Pfad durch das Zeichnen von horizontalen und vertikalen Linien aufgefüllt.</span><span class="sxs-lookup"><span data-stu-id="132c7-106">The example creates a logical font and uses it to draw a string within a clip path, then fills the path by drawing horizontal and vertical lines.</span></span>
...
<span data-ttu-id="132c7-107">Ein Beispiel, das veranschaulicht, wie eine Anwendung einen rechteckigen Clippingbereich erstellt, finden Sie unter [Regionen](regions.md).</span><span class="sxs-lookup"><span data-stu-id="132c7-107">For an example that demonstrates how an application creates a rectangular clipping region, see [Regions](regions.md).</span></span>
