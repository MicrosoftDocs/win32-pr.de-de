...
---
# <a name="updating-a-service-connection-point"></a>Aktualisieren eines Dienst Verbindungs Punkts

Im folgenden Codebeispiel wird gezeigt, wie ein Dienst Verbindungspunkt aktualisiert wird. Dieser Code wird in der Regel von einem Dienst ausgeführt, wenn er gestartet wird.

In diesem Beispiel wird die GUID-Bindungs Zeichenfolge des SCP abgerufen, die der Dienst Installer in der Registrierung zwischengespeichert hat. Diese Zeichenfolge wird verwendet, um eine Bindung an einen [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) -Zeiger auf das SCP-Objekt zu erhalten, und anschließend wird die [**IDirectoryObject:: getobjectattributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-getobjectattributes) -Methode aufgerufen, um die Attribute **servicednsname** und **serviceBindingInformation** für den SCP zu erhalten. Beachten Sie, dass Ihr Dienst möglicherweise erfordert, dass zusätzliche Attribute überprüft und aktualisiert werden.

Der Code vergleicht den **servicednsname** -Wert mit dem DNS-Namen, der von der [**GetComputerNameEx**](/windows/desktop/api/sysinfoapi/nf-sysinfoapi-getcomputernameexa) -Funktion zurückgegeben wird. Außerdem wird die aktuelle Portnummer des dienstanders mit der im **serviceBindingInformation** -Attribut gespeicherten Portnummer verglichen. Wenn sich einer dieser Werte geändert hat, ruft der Code die [**IDirectoryObject:: setobjectattributes**](/windows/desktop/api/iads/nf-iads-idirectoryobject-setobjectattributes) -Methode auf, um die SCP-Attribute zu aktualisieren.
