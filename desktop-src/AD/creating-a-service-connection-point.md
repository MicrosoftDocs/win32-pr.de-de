...
---
# <a name="creating-a-service-connection-point"></a>Erstellen eines Dienst Verbindungs Punkts

Im folgenden Codebeispiel wird gezeigt, wie ein Dienst Verbindungspunkt erstellt und initialisiert wird. Das Codebeispiel führt zusätzliche Schritte aus, die es dem Dienst ermöglichen, die SCP-Werte zur Laufzeit zu aktualisieren. In der Regel führt ein Dienst Installationsprogramm diese Schritte im Rahmen der Installation einer Dienst Instanz auf einem Host Computer durch.

In diesem Codebeispiel wird das SCP-Objekt als untergeordnetes Objekt für das-Objekt des lokalen Computers erstellt. Die Funktion " [**getcomputerobjectname**](/windows/desktop/api/secext/nf-secext-getcomputerobjectnamea) " wird verwendet, um den DN des lokalen Computer Objekts abzurufen. Anschließend wird der DN verwendet, um eine Bindung an einen [**IDirectoryObject**](/windows/desktop/api/iads/nn-iads-idirectoryobject) -Zeiger für das Computer Objekt herzustellen. Die [**IDirectoryObject:: deedsobject**](/windows/desktop/api/iads/nf-iads-idirectoryobject-createdsobject) -Methode erstellt den SCP und gibt die Anfangswerte für die wichtigen SCP-Attribute an.

" [**Kreatedsobject**](/windows/desktop/api/iads/nf-iads-idirectoryobject-createdsobject) " gibt einen Zeiger auf den neuen Dienst Verbindungspunkt zurück, der im Codebeispiel zum Abrufen eines [**IADs**](/windows/desktop/api/iads/nn-iads-iads) -Zeigers für den Dienst Verbindungspunkt verwendet wird. Im Codebeispiel werden die **IADs** -Methoden verwendet, um die Attribute " **objectGUID** " und " **erkennbar Name** " des SCP abzurufen. Im Codebeispiel wird die **objectGUID** verwendet, um eine Zeichenfolge zu erstellen, die für die Bindung an den Dienst Verbindungspunkt verwendet wird. Anschließend wird die GUID-Bindungs Zeichenfolge in der lokalen Registrierung zwischengespeichert, wo Sie zur Laufzeit vom Dienst abgerufen werden kann. Der " **Unterzeichner** " wird an den Funktions Aufrufer zurückgegeben, der zum Verfassen eines Dienst Prinzipal namens (SPN) für die Dienst Instanz verwendet wird.

Im folgenden Codebeispiel wird diese Funktion als Teil der grundlegenden Schritte zur Installation eines Verzeichnis fähigen Dienstanbieter aufgerufen. Weitere Informationen finden Sie unter [Installieren eines Diensts auf einem Host Computer](installing-a-service-on-a-host-computer.md).
