...
Summiert die Elemente eines Tensors entlang einer Achse und schreibt die laufende Tally der Summe in den Ausgabe-Tensor.
...
### <a name="example-1-cumulative-summation-across-horizontal-slivers"></a>Beispiel 1: Kumulierte Summierung über horizontale Slivers hinweg
...
Wenn *HasExclusiveSum* auf **TRUE** festgelegt wird, wird der Wert des aktuellen Elements beim Schreiben in den Ausgabe-Tensor von der ausgeführten Tally ausgeschlossen.
...
Wenn *AxisDirection* auf [DML_AXIS_DIRECTION_DECREASING](/windows/win32/api/directml/ne-directml-dml_axis_direction) festgelegt wird, wird die Durchlaufreihenfolge von Elementen umgekehrt, wenn die ausgeführte tally-Berechnung erfolgt.
...
In diesem Beispiel erfolgt die Summierung vertikal entlang der Höhenachse (zweite Dimension).
...
Dieser Operator unterstützt die direkt ausgeführte Ausführung, d. h., der *OutputTensor* darf während der Bindung einen Alias für *InputTensor* setzen.
