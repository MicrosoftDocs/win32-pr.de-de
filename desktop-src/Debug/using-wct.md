...
---
# <a name="using-wct"></a>Verwenden von WCT

Im folgenden Beispielcode wird die Verwendung der Wait Chain Traversale-API veranschaulicht. Sie listet alle Threads im System auf und druckt die Warte Kette für jeden Thread.

Um alle Threads im System aufzulisten, führen Sie das Beispiel ohne Parameter aus. Um nur die Threads eines angegebenen Prozesses aufzulisten, führen Sie das Beispiel aus, und übergeben Sie die Prozess-ID als Parameter. Das Beispiel führt die folgenden Schritte aus:

1. Ruft die [registerwaitchaincomcallback](/windows/desktop/api/Wct/nf-wct-registerwaitchaincomcallback) -Funktion auf, um die com-Rückruf Funktionen zu registrieren.
2. Ruft die [openthumwaitchainsession](/windows/desktop/api/Wct/nf-wct-openthreadwaitchainsession) -Funktion auf, um die Warte Ketten Sitzung zu erstellen.
3. Ruft die Funktion "- [tokenprivilegien](/windows/win32/api/securitybaseapi/nf-securitybaseapi-adjusttokenprivileges) " auf, um die Berechtigung "SE Debug Name" zu aktivieren \_ \_ .
4. Ruft die [EnumProcesses](/windows/win32/api/psapi/nf-psapi-enumprocesses) -und [CreateToolhelp32Snapshot](/windows/win32/api/tlhelp32/nf-tlhelp32-createtoolhelp32snapshot) -Funktionen auf, um die angegebenen Threads aufzuzählen.
5. Ruft die [GetThreadWaitChain](/windows/desktop/api/Wct/nf-wct-getthreadwaitchain) -Funktion auf, um ein Array von [waitchain- \_ Knoten \_ Info](/windows/desktop/api/Wct/ns-wct-waitchain_node_info) -Strukturen abzurufen, die die Knoten der Warte Kette enthalten.
6. Druckt Informationen aus der Warte Kette.
7. Ruft die [closethreadwaitchainsession](/windows/desktop/api/Wct/nf-wct-closethreadwaitchainsession) -Funktion auf, um die Warte Kette zu bereinigen.
...
[Wait Chain Traversal](wait-chain-traversal.md), [WCT Reference](wct-reference.md), [MSDN Magazine 2007 Juli-Bugslayer: Wait Chain Traversal](/archive/msdn-magazine/2007/july/bugslayer-wait-chain-traversal), [Microsoft-Support: Beheben von Problemen mit apps von Microsoft Store](https://support.microsoft.com/help/4027498/microsoft-store-fix-problems-with-apps)
