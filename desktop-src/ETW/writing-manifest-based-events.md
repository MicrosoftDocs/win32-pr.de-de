...
---
# <a name="writing-manifest-based-events"></a><span data-ttu-id="cb1b4-103">Schreiben von Manifest-basierten Ereignissen</span><span class="sxs-lookup"><span data-stu-id="cb1b4-103">Writing Manifest-based Events</span></span>

<span data-ttu-id="cb1b4-104">Bevor Sie Ereignisse in eine Ablauf Verfolgungs Sitzung schreiben können, müssen Sie den Anbieter registrieren.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-104">Before you can write events to a trace session, you must register your provider.</span></span> <span data-ttu-id="cb1b4-105">Wenn Sie einen Anbieter registrieren, wird etw mitgeteilt, dass der Anbieter zum Schreiben von Ereignissen in eine Ablauf Verfolgungs Sitzung bereit ist.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-105">Registering a provider tells ETW that your provider is ready to write events to a trace session.</span></span> <span data-ttu-id="cb1b4-106">Ein Prozess kann bis zu 1.024 Anbieter-GUIDs registrieren. Sie sollten jedoch die Anzahl der Anbieter, die Ihr Prozess registriert, auf einen oder zwei beschränken.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-106">A process can register up to 1,024 provider GUIDs; however, you should limit the number of providers that your process registers to one or two.</span></span>

<span data-ttu-id="cb1b4-107">**Vor Windows Vista:** Es gibt keine Beschränkung für die Anzahl der Anbieter, die ein Prozess registrieren kann.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-107">**Prior to Windows Vista:** There is no limit to the number of providers that a process can register.</span></span>

<span data-ttu-id="cb1b4-108">Um einen Manifest-basierten Anbieter zu registrieren, nennen Sie die [**EventRegister**](/windows/desktop/api/Evntprov/nf-evntprov-eventregister) -Funktion.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-108">To register a manifest-based provider, call the [**EventRegister**](/windows/desktop/api/Evntprov/nf-evntprov-eventregister) function.</span></span> <span data-ttu-id="cb1b4-109">Die Funktion registriert die GUID des Anbieters und identifiziert einen optionalen Rückruf, der von etw aufgerufen wird, wenn ein Controller den Anbieter aktiviert oder deaktiviert.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-109">The function registers the provider's GUID and identifies an optional callback that ETW calls when a controller enables or disables the provider.</span></span>

<span data-ttu-id="cb1b4-110">Bevor der Anbieter beendet wird, wenden Sie die [**EventUnregister**](/windows/desktop/api/Evntprov/nf-evntprov-eventunregister) -Funktion an, um die Registrierung des Anbieters von etw zu entfernen.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-110">Before the provider exits, call the [**EventUnregister**](/windows/desktop/api/Evntprov/nf-evntprov-eventunregister) function to remove the provider's registration from ETW.</span></span> <span data-ttu-id="cb1b4-111">Die [**EventRegister**](/windows/desktop/api/Evntprov/nf-evntprov-eventregister) -Funktion gibt das Registrierungs Handle zurück, das Sie an die **EventUnregister** -Funktion übergeben.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-111">The [**EventRegister**](/windows/desktop/api/Evntprov/nf-evntprov-eventregister) function returns the registration handle that you pass to the **EventUnregister** function.</span></span>

<span data-ttu-id="cb1b4-112">Manifestressourcen müssen keine [**enablecallback**](/windows/desktop/api/Evntprov/nc-evntprov-penablecallback) [-](about-event-tracing.md) Funktion implementieren, um Benachrichtigungen zu empfangen, wenn der Anbieter durch eine Sitzung aktiviert oder deaktiviert wird.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-112">[Manifest-based](about-event-tracing.md) providers do not have to implement an [**EnableCallback**](/windows/desktop/api/Evntprov/nc-evntprov-penablecallback) function to receive notifications when a session enables or disables the provider.</span></span> <span data-ttu-id="cb1b4-113">Der Rückruf ist optional und wird zu Informationszwecken verwendet. beim Registrieren des Anbieters muss der Rückruf nicht angegeben oder implementiert werden.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-113">The callback is optional and is used for informational purposes; you do not need to specify or implement the callback when registering the provider.</span></span> <span data-ttu-id="cb1b4-114">Ein Manifest-basierter Anbieter kann einfach Ereignisse schreiben, und etw entscheidet, ob das Ereignis in einer Ablauf Verfolgungs Sitzung protokolliert wird.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-114">A manifest-based provider can simply write events and ETW will decide if the event gets logged to a trace session.</span></span> <span data-ttu-id="cb1b4-115">Wenn ein Ereignis erfordert, dass Sie umfangreiche Arbeiten ausführen müssen, um die Daten des Ereignisses zu generieren, können Sie zuerst die [**eventaktivierte**](/windows/desktop/api/Evntprov/nf-evntprov-eventenabled) Funktion oder die [**eventprovideraktivierte**](/windows/desktop/api/Evntprov/nf-evntprov-eventproviderenabled) Funktion aufzurufen, um zu überprüfen, ob das Ereignis in eine Sitzung geschrieben wird, bevor Sie die Arbeit ausführen.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-115">If an event requires that you perform extensive work to generate the event's data, you may want to call the [**EventEnabled**](/windows/desktop/api/Evntprov/nf-evntprov-eventenabled) or [**EventProviderEnabled**](/windows/desktop/api/Evntprov/nf-evntprov-eventproviderenabled) function first to verify that the event will be written to a session before performing the work.</span></span>

<span data-ttu-id="cb1b4-116">In der Regel implementieren Sie den Rückruf, wenn der Anbieter erfordert, dass der Controller Anbieter definierte Filterdaten (siehe den *FilterData* -Parameter von [**enablecallback**](/windows/desktop/api/Evntprov/nc-evntprov-penablecallback)) an den Anbieter übergibt, oder der Anbieter verwendet die Kontextinformationen, die er bei der Registrierung angegeben hat (siehe *callbackcontext* -Parameter von [**EventRegister**](/windows/desktop/api/Evntprov/nf-evntprov-eventregister)).</span><span class="sxs-lookup"><span data-stu-id="cb1b4-116">Typically, you would implement the callback if your provider requires that the controller pass provider-defined filter data (see the *FilterData* parameter of [**EnableCallback**](/windows/desktop/api/Evntprov/nc-evntprov-penablecallback)) to the provider, or the provider uses the context information that it specified when it registered itself (see the *CallbackContext* parameter of [**EventRegister**](/windows/desktop/api/Evntprov/nf-evntprov-eventregister)).</span></span>

<span data-ttu-id="cb1b4-117">Manifestressourcenbasierte Anbieter nennen die [**EventWrite**](/windows/desktop/api/Evntprov/nf-evntprov-eventwrite) -oder [**EventWrite-String**](/windows/desktop/api/Evntprov/nf-evntprov-eventwritestring) [-](about-event-tracing.md) Funktion, um Ereignisse in eine Sitzung zu schreiben.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-117">[Manifest-based](about-event-tracing.md) providers call the [**EventWrite**](/windows/desktop/api/Evntprov/nf-evntprov-eventwrite) or [**EventWriteString**](/windows/desktop/api/Evntprov/nf-evntprov-eventwritestring) function to write events to a session.</span></span> <span data-ttu-id="cb1b4-118">Wenn es sich bei den Ereignisdaten um eine Zeichenfolge handelt, oder wenn Sie kein Manifest für Ihren Anbieter definieren und die Ereignisdaten eine einzelne Zeichenfolge sind, müssen Sie die [**EventWrite-String**](/windows/desktop/api/Evntprov/nf-evntprov-eventwritestring) -Funktion verwenden, um das Ereignis zu schreiben.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-118">If your event data is a string, or if you do not define a manifest for your provider and your event data is a single string, call the [**EventWriteString**](/windows/desktop/api/Evntprov/nf-evntprov-eventwritestring) function to write the event.</span></span> <span data-ttu-id="cb1b4-119">Bei Ereignisdaten, die numerische oder komplexe Datentypen enthalten, wird die [**EventWrite**](/windows/desktop/api/Evntprov/nf-evntprov-eventwrite) -Funktion aufgerufen, um das Ereignis zu protokollieren.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-119">For event data that contains numeric or complex data types, call the [**EventWrite**](/windows/desktop/api/Evntprov/nf-evntprov-eventwrite) function to log the event.</span></span>

<span data-ttu-id="cb1b4-120">Im folgenden Beispiel wird gezeigt, wie die Ereignisdaten vorbereitet werden, die mit der [**EventWrite**](/windows/desktop/api/Evntprov/nf-evntprov-eventwrite) -Funktion geschrieben werden sollen.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-120">The following example shows how to prepare the event data to be written using the [**EventWrite**](/windows/desktop/api/Evntprov/nf-evntprov-eventwrite) function.</span></span> <span data-ttu-id="cb1b4-121">Das Beispiel verweist auf die Ereignisse, die in [Veröffentlichen des Ereignis Schemas für einen Manifest-basierten Anbieter](publishing-your-event-schema-for-a-manifest-base-provider.md)definiert sind.</span><span class="sxs-lookup"><span data-stu-id="cb1b4-121">The example references the events defined in [Publishing Your Event Schema for a Manifest-based Provider](publishing-your-event-schema-for-a-manifest-base-provider.md).</span></span>
...
<span data-ttu-id="cb1b4-122">Wenn Sie das Manifest kompilieren (siehe [Kompilieren eines Instrumentierungs Manifests](../wes/compiling-an-instrumentation-manifest.md)), das im obigen Beispiel verwendet wird, wird die folgende Header Datei erstellt (im obigen Beispiel wird darauf verwiesen).</span><span class="sxs-lookup"><span data-stu-id="cb1b4-122">When you compile the manifest (see [Compiling an Instrumentation Manifest](../wes/compiling-an-instrumentation-manifest.md)) that the example above uses, it creates the following header file (referenced in the example above).</span></span>
