...
Der synchrone Reader bietet mehr Flexibilität beim Abrufen von Beispielen aus einer ASF-Datei als der asynchrone Reader. Der synchrone Reader kann Beispiele sowohl nach Streamnummer als auch nach Ausgabe übermitteln. Anhand der Streamnummer übermittelte Beispiele können komprimiert oder unkomprimiert werden. Der synchrone Reader kann auch während der Wiedergabe zwischen komprimierter und nicht komprimierter Übermittlung wechseln. Dieses Feature wird als "schnelle Bearbeitung" bezeichnet. Mit diesem Feature kann eine Bearbeitungsanwendung Windows medienbasierten Inhalt lesen und direkt an den Writer übergeben, bis ein gewünschter Frame erreicht ist. An diesem Punkt kann die Anwendung den Reader anzuweisen, unkomprimierten Inhalt zu übermitteln, den die Anwendung dann ändern und zur Erneutkomprimierung an den Writer übergeben kann. Wenn die Anwendung die angegebenen Frames geändert hat, kann sie den Reader an weisen, erneut komprimierte Frames zu übermitteln.

Die grundlegendsten Funktionen des synchronen Readerobjekts können in die folgenden Schritte unterteilt werden. In diesen Schritten bezieht sich "die Anwendung" auf das Programm, das Sie mit dem Windows Media Format SDK schreiben.

1.  Die Anwendung übergibt den Namen einer zu lesende Datei an den synchronen Reader. Wenn der synchrone Reader die Datei öffnet, weist er jedem Stream eine Ausgabenummer zu. Wenn die Datei gegenseitigen Ausschluss verwendet, weist der Reader eine einzelne Ausgabe für alle sich gegenseitig ausschließenden Streams zu.
2.  Die Anwendung ruft Informationen zur Konfiguration der verschiedenen Ausgaben des Readers ab. Die gesammelten Informationen ermöglichen der Anwendung das ordnungsgemäße Rendern von Medienbeispielen.
3.  Die Anwendung beginnt nacheinander damit, Stichproben vom synchronen Reader anzufordern. Der synchrone Reader übermittelt jedes Beispiel in einem Pufferobjekt, für das er die [**INSSBuffer-Schnittstelle**](/previous-versions/windows/desktop/api/wmsbuffer/nn-wmsbuffer-inssbuffer) übermittelt.
4.  Die Anwendung ist für das Rendern von Daten nach der Übermittlung durch den Reader verantwortlich. Das Windows Media Format SDK stellt keine Renderingroutinen bereit. In der Regel verwenden Anwendungen andere SDKs zum Rendern von Daten, z. B. das Microsoft Direct X SDK oder die Multimediafunktionen des Microsoft Windows Platform SDK.
...
-   Geben Sie einen Bereich von Stichproben an, die nach Zeit oder Framenummer abgerufen werden sollen.
-   Steuern sie die Datenstromauswahl für sich gegenseitig ausschließende Streams.
...
-   [So suchen Sie nach Zeit mit dem synchronen Reader](to-seek-by-time-using-the-synchronous-reader.md)
-   [So suchen Sie nach Framenummer mit dem synchronen Reader](to-seek-by-frame-number-using-the-synchronous-reader.md)
-   [So suchen Sie nach SMPTE-Zeitcode mit dem synchronen Reader](to-seek-by-smpte-time-code-using-the-synchronous-reader.md)
...
Der folgende Beispielcode zeigt, wie Sie Mithilfe des synchronen Readers Beispiele aus einer ASF-Datei lesen. Sie gibt anhand der Framenummer eine Reihe von Beispielen an, die übermittelt werden sollen.
