---
description: 'Beispiel für MUI: Application-Specific Einstellungen (Pre-Windows Vista)'
ms.assetid: 932aa981-ddd9-4a5b-9003-7dafd98e3ae4
title: 'Beispiel für MUI: Application-Specific Einstellungen (Pre-Windows Vista)'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "103753529"
---
# <a name="mui-application-specific-settings-sample-pre-windows-vista"></a><span data-ttu-id="d95f6-103">Beispiel für MUI: Application-Specific Einstellungen (Pre-Windows Vista)</span><span class="sxs-lookup"><span data-stu-id="d95f6-103">MUI: Application-Specific Settings Sample (Pre-Windows Vista)</span></span>

<span data-ttu-id="d95f6-104">Die in diesem Thema beschriebene Beispielanwendung ähnelt der in [MUI: Application-Specific Settings Sample (Windows Vista)](mui-application-specific-settings-sample-vista.md)gezeigten Beispielanwendung, mit der Ausnahme, dass diese Anwendung unter Windows 2000 und höher ausgeführt werden kann.</span><span class="sxs-lookup"><span data-stu-id="d95f6-104">The sample application described in this topic is similar to the one shown in [MUI: Application-Specific Settings Sample (Windows Vista)](mui-application-specific-settings-sample-vista.md), except that this application will run on Windows 2000 and later.</span></span> <span data-ttu-id="d95f6-105">Der Hauptunterschied zum anderen Beispiel besteht darin, dass der sprach Fall Back an mehrere Betriebssysteme angepasst werden muss.</span><span class="sxs-lookup"><span data-stu-id="d95f6-105">The main difference from the other sample is that language fallback has to be customized to run on multiple operating systems.</span></span>

<span data-ttu-id="d95f6-106">Diese Anwendung analysiert zunächst eine durch Trennzeichen getrennte Sprachliste in einer Textdatei und konvertiert sie in eine Sprachliste mit mehreren Zeichen folgen, um die anwendungsspezifischen Spracheinstellungen zu definieren.</span><span class="sxs-lookup"><span data-stu-id="d95f6-106">This application first parses a delimited language list in a text file and converts it to a multi-string language list to define the application-specific language preferences.</span></span> <span data-ttu-id="d95f6-107">Die im Beispiel unterstützten Trennzeichen sind ",", ";", ":" und "".</span><span class="sxs-lookup"><span data-stu-id="d95f6-107">The delimiters supported in the sample are ",", ";", ":", and " ".</span></span> <span data-ttu-id="d95f6-108">Nach dem Durchsuchen der Liste sucht der Code die Ressourcen in der identifizierten Sprache und lädt Sie.</span><span class="sxs-lookup"><span data-stu-id="d95f6-108">After parsing the list, the code finds and loads the resources in the identified language.</span></span> <span data-ttu-id="d95f6-109">Im Gegensatz zum anderen anwendungsspezifischen Einstellungs Beispiel lädt dieser Code Ressourcen Dateien mithilfe von Aufrufen der MUI-Funktionen " [**loadmuilibrary**](/windows/desktop/api/Muiload/nf-muiload-loadmuilibrarya) " und " [**fremuilibrary**](/windows/desktop/api/Muiload/nf-muiload-freemuilibrary)" und gibt diese frei.</span><span class="sxs-lookup"><span data-stu-id="d95f6-109">In contrast to the other application-specific settings sample, this code loads and releases resource files using calls to the MUI functions [**LoadMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-loadmuilibrarya) and [**FreeMUILibrary**](/windows/desktop/api/Muiload/nf-muiload-freemuilibrary).</span></span>
