...
---
# <a name="seeking-fast-forward-and-reverse-play"></a><span data-ttu-id="202c8-103">Suchen, schnelles vorwärts und umgekehrtes spielen</span><span class="sxs-lookup"><span data-stu-id="202c8-103">Seeking, Fast Forward, and Reverse Play</span></span>

<span data-ttu-id="202c8-104">Dieses Thema enthält Beispielcode zum Verwalten von Such-und Raten Änderungen, wenn die [Medien Sitzung](media-session.md) für die Wiedergabe verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="202c8-104">This topic shows example code to manage seeking and rate changes, when using the [Media Session](media-session.md) for playback.</span></span>

<span data-ttu-id="202c8-105">Wenn Sie die Medien Sitzung für die Wiedergabe verwenden, kann eine Anwendung die Wiedergabe Rate wie folgt suchen und ändern:</span><span class="sxs-lookup"><span data-stu-id="202c8-105">When using the Media Session for playback, an application can seek and change the playback rate as follows:</span></span>

-   <span data-ttu-id="202c8-106">Um zu suchen, nennen Sie [**imfmediasession:: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start) , und geben Sie die Suchposition an.</span><span class="sxs-lookup"><span data-stu-id="202c8-106">To seek, call [**IMFMediaSession::Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start) and specify the seek position.</span></span>
-   <span data-ttu-id="202c8-107">Um die Wiedergabe Rate zu ändern, verwenden Sie die [**imfratecontrol**](/windows/desktop/api/mfidl/nn-mfidl-imfratecontrol) -Schnittstelle, wie in [Raten Steuerung](rate-control.md)beschrieben.</span><span class="sxs-lookup"><span data-stu-id="202c8-107">To change the playback rate, use the [**IMFRateControl**](/windows/desktop/api/mfidl/nn-mfidl-imfratecontrol) interface as described in [Rate Control](rate-control.md).</span></span>
-   <span data-ttu-id="202c8-108">Wenn Sie beim Suchen aktualisierte Video Frames abrufen möchten, legen Sie die Wiedergabe Rate auf 0 (null) fest, bevor Sie [**imfmediasession:: Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start)aufrufen.</span><span class="sxs-lookup"><span data-stu-id="202c8-108">To get updated video frames while seeking, set the playback rate to zero before calling [**IMFMediaSession::Start**](/windows/desktop/api/mfidl/nf-mfidl-imfmediasession-start).</span></span> <span data-ttu-id="202c8-109">Dieser Vorgang wird als *Bereinigungs* bezeichnet.</span><span class="sxs-lookup"><span data-stu-id="202c8-109">This operation is called *scrubbing*.</span></span> <span data-ttu-id="202c8-110">(Siehe Vorgehens [Weise beim Ausführen von](how-to-perform-scrubbing.md)Bereinigungs arbeiten.)</span><span class="sxs-lookup"><span data-stu-id="202c8-110">(See [How to Perform Scrubbing](how-to-perform-scrubbing.md).)</span></span>

<span data-ttu-id="202c8-111">Um die beste Benutzerfunktion zu erstellen, müssen Sie jedoch die folgenden Verhaltensweisen berücksichtigen:</span><span class="sxs-lookup"><span data-stu-id="202c8-111">To create the best user experience, however, you must take the following behaviors into account:</span></span>

-   <span data-ttu-id="202c8-112">Die Suche erfolgt asynchron, und die Medien Sitzung fügt alle Suchanforderungen in einer FIFO-Warteschlange in die Warteschlange ein.</span><span class="sxs-lookup"><span data-stu-id="202c8-112">Seeking is asynchronous, and the Media Session queues all seek requests on a FIFO queue.</span></span> <span data-ttu-id="202c8-113">Wenn Sie mehrere Suchanforderungen übermitteln, wird die Benutzeroberfläche möglicherweise vor dem tatsächlichen Wiedergabe Status ausgeführt.</span><span class="sxs-lookup"><span data-stu-id="202c8-113">If you submit multiple seek requests, the UI might run ahead of the actual playback state.</span></span> <span data-ttu-id="202c8-114">Angenommen, Ihre Anwendung implementiert eine Suchleiste.</span><span class="sxs-lookup"><span data-stu-id="202c8-114">For example, suppose your application implements a seek bar.</span></span> <span data-ttu-id="202c8-115">Wenn der Benutzer die Suchleiste vorwärts und rückwärts zieht, kann es zu einer Verzögerung kommen, während die Forward-Suchvorgänge ausgeführt werden.</span><span class="sxs-lookup"><span data-stu-id="202c8-115">If the user drags the seek bar forward and backward, there might be a lag while the forward seeks are executed.</span></span> <span data-ttu-id="202c8-116">Während eine Suche ausgeführt wird, sollten Sie die Suchanforderungen des Benutzers Zwischenspeichern.</span><span class="sxs-lookup"><span data-stu-id="202c8-116">While a seek is in progress, you should cache the user's seek requests.</span></span> <span data-ttu-id="202c8-117">Wenn der aktuelle Suchvorgang abgeschlossen ist, übermitteln Sie die aktuellste Such Anforderung des Benutzers, und verwerfen Sie die anderen.</span><span class="sxs-lookup"><span data-stu-id="202c8-117">When the current seek operation completes, submit the user's most recent seek request and discard the others.</span></span>
-   <span data-ttu-id="202c8-118">Einige raten Übergänge sind in einigen Transport Zuständen nicht zulässig.</span><span class="sxs-lookup"><span data-stu-id="202c8-118">Some rate transitions are not allowed in some transport states.</span></span> <span data-ttu-id="202c8-119">Beispielsweise ist der Wechsel von der vorwärts Wiedergabe zur umgekehrten Wiedergabe bei der Wiedergabe nicht zulässig.</span><span class="sxs-lookup"><span data-stu-id="202c8-119">For example, switching from forward playback to reverse playback is not allowed while playing.</span></span> <span data-ttu-id="202c8-120">Die unterstützten Übergänge werden in [**imfratecontrol:: abtrate**](/windows/desktop/api/mfidl/nf-mfidl-imfratecontrol-setrate)beschrieben.</span><span class="sxs-lookup"><span data-stu-id="202c8-120">The supported transitions are described in [**IMFRateControl::SetRate**](/windows/desktop/api/mfidl/nf-mfidl-imfratecontrol-setrate).</span></span> <span data-ttu-id="202c8-121">Die Raten Änderungen sind ebenfalls asynchron.</span><span class="sxs-lookup"><span data-stu-id="202c8-121">Rate changes are also asynchronous.</span></span>

<span data-ttu-id="202c8-122">Die folgende Hilfsklasse kann zum Verwalten von Suchanforderungen und Raten Änderungen verwendet werden.</span><span class="sxs-lookup"><span data-stu-id="202c8-122">The following helper class can be used to manage seek requests and rate changes.</span></span> <span data-ttu-id="202c8-123">Während ein asynchroner Vorgang aussteht, speichert die Klasse alle Such-oder Raten Änderungsanforderungen zwischen.</span><span class="sxs-lookup"><span data-stu-id="202c8-123">While an asynchronous operation is pending, the class caches any seek or rate-change requests.</span></span> <span data-ttu-id="202c8-124">Wenn der aktuelle Vorgang abgeschlossen ist, wird die aktuelle Anforderung von der-Klasse übermittelt, sofern vorhanden.</span><span class="sxs-lookup"><span data-stu-id="202c8-124">When the current operation completes, the class submits the most recent request, if any.</span></span> <span data-ttu-id="202c8-125">Die-Klasse verwaltet auch den Transport Status, um ungültige Raten Änderungen zu vermeiden.</span><span class="sxs-lookup"><span data-stu-id="202c8-125">The class also manages the transport state to avoid invalid rate changes.</span></span>

<span data-ttu-id="202c8-126">Hier ist die Deklaration der playersuchende-Klasse.</span><span class="sxs-lookup"><span data-stu-id="202c8-126">Here is the declaration of the PlayerSeeking class.</span></span>
...
<span data-ttu-id="202c8-127">Hier ist die Implementierung der playersuchende-Klasse.</span><span class="sxs-lookup"><span data-stu-id="202c8-127">Here is the implementation of the PlayerSeeking class.</span></span>
...
## <a name="related-topics"></a><span data-ttu-id="202c8-128">Zugehörige Themen</span><span class="sxs-lookup"><span data-stu-id="202c8-128">Related topics</span></span>

<dl> <dt>

[<span data-ttu-id="202c8-129">Raten Steuerung</span><span class="sxs-lookup"><span data-stu-id="202c8-129">Rate Control</span></span>](rate-control.md)
</dt> <dt>

[<span data-ttu-id="202c8-130">Audio-/Videowiedergabe</span><span class="sxs-lookup"><span data-stu-id="202c8-130">Audio/Video Playback</span></span>](audio-video-playback.md)
</dt> </dl>
