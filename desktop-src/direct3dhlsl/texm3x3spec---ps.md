...
Führt eine 3X3-Matrix Multiplikation aus und verwendet das Ergebnis, um eine Textur Suche auszuführen. Dies kann für die Glanz Reflektion und die Umgebungs Zuordnung verwendet werden. texm3x3spec muss in Verbindung mit zwei [texm3x3pad-PS-](texm3x3pad---ps.md) Anweisungen verwendet werden.

## <a name="syntax"></a>Syntax



| texm3x3spec DST, src0, Quelle1 |
|-----------------------------|
...
-   src0 und Quelle1 sind Quell Register.
...
Diese Anweisung führt die letzte Zeile einer 3X3-Matrix Multiplikation aus, verwendet den resultierenden Vektor als normalen Vektor, um einen Augen Strahl Vektor widerzuspiegeln, und verwendet dann den reflektierten Vektor, um eine Textur Suche auszuführen. Der Shader liest den Augen Strahl Vektor aus einem konstanten Register. Die 3X3-Matrix Multiplikation ist in der Regel hilfreich, um einen normalen Vektor auf den richtigen Tangenten Raum für die zu rendernde Oberfläche zu ausrichten.

Die 3X3-Matrix besteht aus den Texturkoordinaten der dritten Textur Phase und den beiden vorangehenden Textur Stufen. Der resultierende postreflection Vector (u, v, w) wird verwendet, um die Textur auf der abschließenden Textur Phase zu testen. Jede Textur, die den vorherigen zwei Textur Stufen zugewiesen ist, wird ignoriert.

Diese Anweisung muss mit zwei texm3x3pad-Anweisungen verwendet werden. Textur Register müssen die folgende Sequenz verwenden.
...
Die texm3x3spec-Anweisung führt dann eine reflektionsanberechnung durch.
...
und der Eye-Ray-Vektor E wird durch das konstantenregister angegeben.

E = c \# (beliebiges konstantes Register--C0, C1, C2 usw.--kann verwendet werden)

Schließlich wird die texm3x3spec-Anweisung mit t (m + 2) with (u<sup>'</sup>, v<sup>'</sup>, w<sup>'</sup>) und das Ergebnis in t (m + 2) gespeichert.
...
-   Phase 0 wird eine Textur Zuordnung mit normalen Daten zugewiesen. Dies wird häufig als Bump Map bezeichnet. Bei den Daten handelt es sich um (XYZ) normale für jedes texaus. Texturkoordinaten in Phase n definieren, wo diese normale Karte Stichproben gibt.
...
-   Der Augen Strahl Vektor e wird durch ein konstantes Register e = c angegeben \# .
