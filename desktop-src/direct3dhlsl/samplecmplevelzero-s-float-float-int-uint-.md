...
description: Samples a Texture2D on mipmap level 0 only and compares the result to a comparison value, and then returns status about the operation. Für Texture2D.
...
Samples a [**Texture2D**](sm5-object-texture2d.md) on mipmap level 0 only and compares the result to a comparison value. Gibt den Status des Vorgangs zurück.
