---
description: Skalartransformation, die vom Zeichnen oder InkWord verwendet wird.
ms.assetid: 88fc3b90-9ec6-41c0-9267-ed5b585ea07b
title: ScalarTransform-Element
ms.topic: reference
ms.date: 05/31/2018
ms.openlocfilehash: a1f2f9e5acf05d46d7057fd41bce5e7a54dc8b09e40fc5f38f528abaf963f9d0
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 08/11/2021
ms.locfileid: "118449340"
---
# <a name="scalartransform-element"></a>ScalarTransform-Element

Skalartransformation, die von [**der Zeichnung oder**](drawing-element.md) von [**InkWord verwendet wird.**](inkword-element.md)

## <a name="definition"></a>Definition

``` syntax
<xs:element name="ScalarTransform" minOccurs="0" type="ScalarTransformType" />
```

## <a name="parent-elements"></a>Übergeordnete Elemente

[**Zeichnung**](drawing-element.md)

[**InkWord**](inkword-element.md)

## <a name="child-elements"></a>Untergeordnete Elemente

Keine

## <a name="attributes"></a>Attribute



| attribute | Typ           | Erforderlich | Beschreibung | Mögliche Werte     |
|-----------|----------------|----------|-------------|---------------------|
| **Mat1**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat2**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat3**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat4**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat5**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat6**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat7**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat8**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |
| **Mat9**  | **xs:decimal** | Erforderlich |             | Eine beliebige Dezimalzahl. |



 

## <a name="element-information"></a>Elementinformationen



| Element      | Wert                                                                       |
|--------------|-----------------------------------------------------------------------------|
| Elementtyp | [**complexType: ScalarTransformType**](scalartransformtype-complex-type.md) |
| Namespace    | urn:schemas-microsoft-com:tabletpc:richink                                  |
| Schemaname  | Journalreader                                                              |



 

 

 



