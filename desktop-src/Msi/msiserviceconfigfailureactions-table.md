---
description: In der Tabelle MsiServiceConfigFailureActions werden Vorgänge aufgeführt, die nach einem Dienstfailing ausgeführt werden sollen. Die in dieser Tabelle angegebenen Vorgänge werden beim nächsten Start des Systems ausgeführt.
...
In der Tabelle MsiServiceConfigFailureActions werden Vorgänge aufgeführt, die nach einem Dienstfailing ausgeführt werden sollen. Die in dieser Tabelle angegebenen Vorgänge werden beim nächsten Start des Systems ausgeführt.
...
Diese Spalte gibt an, wann die Konfiguration des Diensts geändert werden soll. Die folgenden Werte sind Bitfelder, die kombiniert werden können, um mehrere Vorgänge zu darstellen. Alle anderen Bitfeldwerte werden ignoriert.
...
Der Zurücksetzungszeitraum in Sekunden der Fehleranzahl des Diensts. Der [Dienststeuerungs-Manager](../services/service-control-manager.md) (Service Control Manager, SCM) zählt die Anzahl der Fehler bei jedem Dienst seit dem letzten Neustart des Systems. Die Anzahl wird auf 0 (null) zurückgesetzt, wenn der Dienst während des Zurücksetzungszeitraums nicht fehlschlägt. Wenn der Dienst zum N. Mal ausfällt, führt das System die Aktion aus, die im Element N-1 des im Feld Aktionen angegebenen \[ \] Arrays angegeben ist.
...
Die Befehlszeile, die von dem Prozess ausgeführt wird, der von der [**CreateProcess-Funktion**](/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessa) als Reaktion auf eine **SC ACTION RUN \_ \_ \_ COMMAND-Aktion** erstellt wurde, die in der Spalte Aktionen angegeben ist. Der neue Prozess wird nur dann unter demselben Konto wie der Dienst ausgeführt, wenn das Feld Aktion **SC ACTION RUN COMMAND \_ \_ \_ ist.** Sie können die leere Zeichenfolge "" verwenden, um die aktuelle Befehlszeile unverändert zu verwenden. Sie können die Syntax des Datentyps Formatiert verwenden, um die aktuelle Befehlszeile zu löschen und keinen Vorgang auszuführen, \[ ~ \] wenn der Dienst ausfällt. [](formatted.md)
...
Dieses Feld enthält ein Array von ganzzahligen Werten, die die Wartezeit in Millisekunden angeben, bevor die in der Spalte Aktion angegebene Aktion durchgeführt wird. Trennen Sie die Werte im Array durch \[ ~ \] . Die Anzahl der Elemente im DelayActions-Array muss der Anzahl der Elemente im Actions-Array entspricht. Das N-ten Element des DelayActions-Arrays gibt die Zeitverzögerung für das n-ten Element des Actions-Arrays an.
...
Externer Schlüssel zur Spalte einer der [Komponententabellen.](component-table.md)
