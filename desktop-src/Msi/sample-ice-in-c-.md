---
description: Dieser Beispielcode basiert auf einer benutzerdefinierten Ice-Aktion (ICE01). Er überprüft, ob der ICE-Mechanismus funktioniert, indem er die Uhrzeit anzeigt. Das Ice stellt eine Nachricht bereit, die den Zeitpunkt der Zeit des Installers für den ICE bereitstellt. Dieses Eis sollte nie einen Fehler zurückgeben.
...
ms.locfileid: "106368143"
---
# <a name="sample-ice-in-c"></a><span data-ttu-id="29800-106">Sample Ice in C++</span><span class="sxs-lookup"><span data-stu-id="29800-106">Sample ICE in C++</span></span>

<span data-ttu-id="29800-107">Dieser Beispielcode basiert auf einer benutzerdefinierten Ice-Aktion ( [ICE01](ice01.md)).</span><span class="sxs-lookup"><span data-stu-id="29800-107">This sample code is from an ICE custom action ( [ICE01](ice01.md)).</span></span> <span data-ttu-id="29800-108">Er überprüft, ob der ICE-Mechanismus funktioniert, indem er die Uhrzeit anzeigt.</span><span class="sxs-lookup"><span data-stu-id="29800-108">It validates that the ICE mechanism is working by displaying the time.</span></span> <span data-ttu-id="29800-109">Das Ice stellt eine Nachricht bereit, die den Zeitpunkt der Zeit des Installers für den ICE bereitstellt.</span><span class="sxs-lookup"><span data-stu-id="29800-109">The ICE posts a message giving the time the installer called the ICE.</span></span> <span data-ttu-id="29800-110">Dieses Eis sollte nie einen Fehler zurückgeben.</span><span class="sxs-lookup"><span data-stu-id="29800-110">This ICE should never return an error.</span></span>
