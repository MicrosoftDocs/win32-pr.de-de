...
---
# <a name="windows-filtering-platform-exceptions-for-teredo"></a>Ausnahmen für die Windows-Filter Plattform für Teredo

Ausnahmen, mit denen Anwendungen nicht angeforderten Datenverkehr über [Teredo](about-teredo.md) über eine Firewall empfangen werden können, müssen mithilfe der [Windows-Filter Plattform](/windows/desktop/FWP/windows-filtering-platform-start-page) -APIs erstellt werden. Dies erfolgt durch das Öffnen von eingehenden und ausgehenden Anwendungs basierten Ausnahmen (Anwendung <app name> ) in der Teredo-Unterebene von ALE für IPv6-Datenverkehr. Dadurch wird sichergestellt, dass nur Anwendungen mit der Teredo-Ausnahme Teredo verwenden können. Bei der Erstellung dieser Ausnahmen sollte Vorsicht geboten werden. Durch die Verwendung der Option "Allgemein" \* (alle) können Programme, die nicht mit der Teredo-Unterschicht oder dem Tunnel Datenverkehr registriert sind, die Firewall übergeben und eine Bedrohung für die Sicherheit darstellen.

In jeder Situation ist mindestens eine blockierte Anwendung erforderlich, aber es können NULL oder mehr zulässige Anwendungen von einer Firewall hinzugefügt werden, je nachdem, wie viele Anwendungen zugelassen werden müssen.
