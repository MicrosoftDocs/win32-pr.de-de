...
---
# <a name="example-c-program-using-cryptencryptmessage-and-cryptdecryptmessage"></a><span data-ttu-id="772d8-103">Beispiel-C-Programm: Verwenden von cryptencryptmessage und cryptdecryptmessage</span><span class="sxs-lookup"><span data-stu-id="772d8-103">Example C Program: Using CryptEncryptMessage and CryptDecryptMessage</span></span>

<span data-ttu-id="772d8-104">[**Cryptencryptmessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) ist der einzige Funktionsaufrufe, der erforderlich ist, um alle Aufgaben auszuführen, die unter [Verschlüsseln einer Nachricht](../secauthn/encrypting-a-message.md)aufgeführt sind.</span><span class="sxs-lookup"><span data-stu-id="772d8-104">[**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) is the only function call necessary to accomplish all of the tasks listed in [Encrypting a Message](../secauthn/encrypting-a-message.md).</span></span> <span data-ttu-id="772d8-105">Die Initialisierung der Datenstrukturen ist erforderlich.</span><span class="sxs-lookup"><span data-stu-id="772d8-105">Initialization of data structures is necessary.</span></span> <span data-ttu-id="772d8-106">Die folgende Abbildung zeigt die Beziehung zwischen den Funktionsparametern, die auf Strukturen oder Arrays und deren initialisierte Daten zeigen.</span><span class="sxs-lookup"><span data-stu-id="772d8-106">The following illustration shows the relationship between those function parameters that point to structures or arrays and their initialized data.</span></span> <span data-ttu-id="772d8-107">In diesem Beispiel wird die Nachricht auch mithilfe von [**cryptdecryptmessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage)entschlüsselt.</span><span class="sxs-lookup"><span data-stu-id="772d8-107">This example also decrypts the message using [**CryptDecryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptdecryptmessage).</span></span>

![Initialisierungs Zuordnung für einen "cryptencryptmessage"-Rückruf](images/crypenc.png)

<span data-ttu-id="772d8-109">**So verschlüsseln Sie Daten**</span><span class="sxs-lookup"><span data-stu-id="772d8-109">**To encrypt data**</span></span>

1.  <span data-ttu-id="772d8-110">Einen Zeiger auf den zu verschlüsselnden Inhalt erhalten.</span><span class="sxs-lookup"><span data-stu-id="772d8-110">Get a pointer to the content to be encrypted.</span></span>
2.  <span data-ttu-id="772d8-111">Legen Sie die Größe des zu verschlüsselnden Inhalts fest.</span><span class="sxs-lookup"><span data-stu-id="772d8-111">Determine the size of the content to be encrypted.</span></span>
3.  <span data-ttu-id="772d8-112">Abrufen eines Handles für einen Kryptografieanbieter.</span><span class="sxs-lookup"><span data-stu-id="772d8-112">Acquire a handle to a cryptographic provider.</span></span>
4.  <span data-ttu-id="772d8-113">Öffnen Sie einen [*Zertifikat Speicher*](../secgloss/c-gly.md).</span><span class="sxs-lookup"><span data-stu-id="772d8-113">Open a [*certificate store*](../secgloss/c-gly.md).</span></span>
5.  <span data-ttu-id="772d8-114">Holen Sie sich das Empfänger Zertifikat.</span><span class="sxs-lookup"><span data-stu-id="772d8-114">Get the recipient certificate.</span></span>
6.  <span data-ttu-id="772d8-115">Erstellen Sie das Array des Empfänger Zertifikats.</span><span class="sxs-lookup"><span data-stu-id="772d8-115">Create the recipient certificate array.</span></span>
7.  <span data-ttu-id="772d8-116">Initialisieren Sie die Struktur des [**crypt- \_ Algorithmus \_ Bezeichners**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) .</span><span class="sxs-lookup"><span data-stu-id="772d8-116">Initialize the [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) structure.</span></span>
8.  <span data-ttu-id="772d8-117">Initialisieren Sie die [**crypt- \_ Verschlüsselungs Nachrichten- \_ \_ para**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) -Struktur.</span><span class="sxs-lookup"><span data-stu-id="772d8-117">Initialize the [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para) structure.</span></span>
9.  <span data-ttu-id="772d8-118">Rufen Sie [**cryptencryptmessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) auf, um den Inhalt zu verschlüsseln und eine Digital umgehüllte Nachricht zu erstellen.</span><span class="sxs-lookup"><span data-stu-id="772d8-118">Call [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage) to encrypt the content and create a digitally enveloped message.</span></span>

<span data-ttu-id="772d8-119">Im folgenden Beispiel wird diese Prozedur implementiert.</span><span class="sxs-lookup"><span data-stu-id="772d8-119">The following example implements this procedure.</span></span>

<span data-ttu-id="772d8-120">Kommentare verknüpfen Code Fragmente mit den einzelnen Schritten in der Prozedur.</span><span class="sxs-lookup"><span data-stu-id="772d8-120">Comments relate code fragments to each step in the procedure.</span></span> <span data-ttu-id="772d8-121">Weitere Informationen zur-Funktion finden Sie unter [**cryptencryptmessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span><span class="sxs-lookup"><span data-stu-id="772d8-121">For more information about the function, see [**CryptEncryptMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptencryptmessage).</span></span> <span data-ttu-id="772d8-122">Weitere Informationen zu den Datenstrukturen finden Sie unter [**crypt \_ - \_ Algorithmusbezeichner**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) und [**crypt- \_ Verschlüsselungs \_ Nachricht \_**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span><span class="sxs-lookup"><span data-stu-id="772d8-122">For more information about the data structures, see [**CRYPT\_ALGORITHM\_IDENTIFIER**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_algorithm_identifier) and [**CRYPT\_ENCRYPT\_MESSAGE\_PARA**](/windows/desktop/api/Wincrypt/ns-wincrypt-crypt_encrypt_message_para).</span></span>

<span data-ttu-id="772d8-123">In diesem Beispiel werden die Funktionen [**mylenker Error**](myhandleerror.md) und [**bytedestr**](bytetostr.md)verwendet.</span><span class="sxs-lookup"><span data-stu-id="772d8-123">This example uses the functions [**MyHandleError**](myhandleerror.md) and [**ByteToStr**](bytetostr.md).</span></span> <span data-ttu-id="772d8-124">Der Code für diese Funktion ist im Beispiel enthalten.</span><span class="sxs-lookup"><span data-stu-id="772d8-124">The code for this function is included with the sample.</span></span> <span data-ttu-id="772d8-125">Der Code für dieses und andere Hilfsfunktionen ist auch unter [universell Funktionen](general-purpose-functions.md)aufgeführt.</span><span class="sxs-lookup"><span data-stu-id="772d8-125">Code for this and other auxiliary functions is also listed under [General Purpose Functions](general-purpose-functions.md).</span></span>
