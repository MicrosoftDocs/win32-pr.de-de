---
description: Sie können die Funktion "cryptsignmessage" verwenden, um eine Nachricht zu signieren.
ms.assetid: b400436f-a71f-426a-ac8a-7fdcfa6d7575
title: 'Beispiel-C-Programm: cosignieren und Decodieren einer Nachricht'
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "104343589"
---
# <a name="example-c-program-cosigning-and-decoding-a-message"></a><span data-ttu-id="ddf80-103">Beispiel-C-Programm: cosignieren und Decodieren einer Nachricht</span><span class="sxs-lookup"><span data-stu-id="ddf80-103">Example C Program: Cosigning and Decoding a Message</span></span>

<span data-ttu-id="ddf80-104">Sie können die Funktion " [**cryptsignmessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) " verwenden, um eine Nachricht zu signieren.</span><span class="sxs-lookup"><span data-stu-id="ddf80-104">You can use the [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) function to cosign a message.</span></span> <span data-ttu-id="ddf80-105">Dies erfolgt durch den Aufruf von [**cryptsignmessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) einmal, um die ursprüngliche Nachricht zu signieren, und dann erneut **cryptsignmessage** , um die signierte Nachricht zu signieren.</span><span class="sxs-lookup"><span data-stu-id="ddf80-105">This is accomplished by calling [**CryptSignMessage**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptsignmessage) once to sign the original message, and then call **CryptSignMessage** again to cosign the signed message.</span></span>

<span data-ttu-id="ddf80-106">Wenn Sie die Signatur einer cosignierten Nachricht überprüfen, verwenden Sie die [**cryptgetmessagesignercount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) -Funktion, um die Anzahl der Signatur Geber der Nachricht abzurufen, und dann die [**cryptverifymessagesignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) für jede Signatur.</span><span class="sxs-lookup"><span data-stu-id="ddf80-106">When you verify the signature of a cosigned message, you use the [**CryptGetMessageSignerCount**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptgetmessagesignercount) function to get the number of signers of the message and then call the [**CryptVerifyMessageSignature**](/windows/desktop/api/Wincrypt/nf-wincrypt-cryptverifymessagesignature) for each signature.</span></span> <span data-ttu-id="ddf80-107">Wenn alle Signaturen überprüft werden, wissen Sie, dass die cosigned-Nachricht gültig ist.</span><span class="sxs-lookup"><span data-stu-id="ddf80-107">If all of the signatures are verified, then you know the cosigned message is valid.</span></span>

<span data-ttu-id="ddf80-108">Im folgenden Beispiel wird gezeigt, wie eine Nachricht von mehr als einer Person signiert wird (cosignieren der Nachricht), alle Signaturen überprüft und die Nachricht decodiert wird.</span><span class="sxs-lookup"><span data-stu-id="ddf80-108">The following example shows how to sign a message by more than one person (cosign the message), verify all signatures, and decode the message.</span></span>
