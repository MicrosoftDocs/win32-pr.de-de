---
description: Die folgenden Beispiele veranschaulichen die Verwendung der PDH-API.
ms.assetid: 6df682cf-5f7c-4add-80bf-0ea471607b76
title: PDH-Beispiele
ms.topic: article
ms.date: 05/31/2018
ms.openlocfilehash: cedf2e4f1e386c4d8da61a7880291e3388033a84507434af7775e15faac69b59
ms.sourcegitcommit: e858bbe701567d4583c50a11326e42d7ea51804b
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 08/11/2021
ms.locfileid: "117793923"
---
# <a name="pdh-examples"></a>PDH-Beispiele

Die folgenden Beispiele veranschaulichen die Verwendung der PDH-API.

-   [Aufzählen von Prozessobjekten](enumerating-process-objects.md)
-   [Durchsuchen von Leistungsindikatoren](browsing-performance-counters.md)
-   [Schreiben von Leistungsdaten in eine Protokolldatei](writing-performance-data-to-a-log-file.md)
-   [Lesen von Leistungsdaten aus einer Protokolldatei](reading-performance-data-from-a-log-file.md)
-   [Übertragen von Daten aus einer Protokolldatei im Perfmon-Format in eine Protokolldatei im CSV-Format](transferring-data-from-a-perfmon-format-log-file-to-a-csv-format-log-file.md)

 

 



