...
---
# <a name="ipv4-only-client-code"></a><span data-ttu-id="76df2-103">Nur-IPv4-Client Code</span><span class="sxs-lookup"><span data-stu-id="76df2-103">IPv4-only Client Code</span></span>

<span data-ttu-id="76df2-104">Der folgende Code ist die Datei simplec. c, bei der es sich um einen nur-IPv4-Windows-Sockets-Client handelt (eine IPv6-aktivierte Version der Datei simplec. c finden Sie in Anhang B).</span><span class="sxs-lookup"><span data-stu-id="76df2-104">The following code is the Simplec.c file, which is an IPv4-only Windows Sockets client (an IPv6 enabled version of the Simplec.c file can be found in Appendix B).</span></span> <span data-ttu-id="76df2-105">Dieser Code wird nur zu Vergleichszwecken bereitgestellt – verwenden Sie Anhang B, um ein Beispiel für das Schreiben eines IPv6-fähigen Clients zu erhalten.</span><span class="sxs-lookup"><span data-stu-id="76df2-105">This code is provided for comparison purposes only — use Appendix B for an example of how to write an IPv6-enabled client.</span></span>
