...
---
# <a name="acceptsecuritycontext-kerberos-function"></a>Akzeptsecuritycontext (Kerberos)-Funktion
...
Ein Handle für die Anmelde Informationen des Servers. Der Server Ruft die Funktion " [**AcquireCredentialsHandle" (Kerberos)**](acquirecredentialshandle--kerberos.md) mit der "secpkg" \_ - \_ oder "secpkg" \_ -Funktion \_ auf, die beide Flag zum Abrufen dieses Handles festgelegt sind.
...
Ein Zeiger auf eine [ctxthandle](sspi-handles.md) -Struktur. Beim ersten **calltsecuritycontext (Kerberos)**-Befehl ist dieser Zeiger **null**. Bei nachfolgenden Aufrufen ist *phcontext* das Handle für den teilweise formatierten Kontext, der im *phnewcontext* -Parameter durch den ersten Aufruf zurückgegeben wurde.
...
Ein Zeiger auf eine [**secbufferdesc**](/windows/win32/api/sspi/ns-sspi-secbufferdesc) -Struktur, die durch einen Client-aufrufen von [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md) generiert wird, der den Eingabepuffer Deskriptor enthält.
...
Ein Zeiger auf eine [ctxthandle](sspi-handles.md) -Struktur. Beim ersten **calltsecuritycontext (Kerberos)**-Befehl empfängt dieser Zeiger das neue Kontext handle. Bei nachfolgenden Aufrufen kann *phnewcontext* mit dem im Parameter *phcontext* angegebenen Handle identisch sein.
...
Ein Zeiger auf eine [**secbufferdesc**](/windows/win32/api/sspi/ns-sspi-secbufferdesc) -Struktur, die den Ausgabepuffer Deskriptor enthält. Dieser Puffer wird an den Client gesendet, damit er in weitere Aufrufe von [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md)eingegeben werden kann. Ein Ausgabepuffer kann auch dann generiert werden, wenn die Funktion sec \_ E OK zurückgibt \_ . Jeder generierte Puffer muss an die Client Anwendung zurückgesendet werden.
...
Die Funktion " **Accept tsecuritycontext (Kerberos)** " ist der Server, der der Funktion [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md) entspricht.
...
2.  Der Server Ruft beim ersten Mal " **akzeptsecuritycontext (Kerberos)** " auf, wodurch ein Antwort Token generiert wird, das dann an den Client gesendet wird.
3.  Der Client empfängt das Token und übergibt es an [**InitializeSecurityContext (Kerberos)**](initializesecuritycontext--kerberos.md). Wenn **InitializeSecurityContext (Kerberos)** sec E OK zurückgibt, wurde die \_ \_ gegenseitige Authentifizierung abgeschlossen, und eine sichere Sitzung kann beginnen. Wenn **InitializeSecurityContext (Kerberos)** einen Fehlercode zurückgibt, endet die Aushandlung der gegenseitigen Authentifizierung. Andernfalls wird das von **InitializeSecurityContext (Kerberos)** zurückgegebene Sicherheits Token an den Client gesendet, und die Schritte 2 und 3 werden wiederholt.
