---
description: Dieses Beispielprogramm arbeitet mit dem Client Programm, das SSPI mit einem Windows Sockets-Client verwendet. Beide Beispiel Programme verwenden die Header Datei sspiexample. h, die in der Header Datei für SSPI-Client-und-Server Beispiele zu finden ist.
ms.assetid: 4fa044c7-cce2-4a43-8f68-16cf3a4daaee
title: Verwenden von SSPI mit einem Windows Sockets-Server
ms.topic: article
ms.date: 05/31/2018
...
ms.locfileid: "106355299"
---
# <a name="using-sspi-with-a-windows-sockets-server"></a><span data-ttu-id="26f51-104">Verwenden von SSPI mit einem Windows Sockets-Server</span><span class="sxs-lookup"><span data-stu-id="26f51-104">Using SSPI with a Windows Sockets Server</span></span>

<span data-ttu-id="26f51-105">Dieses Beispielprogramm arbeitet mit dem Client Programm, das [SSPI mit einem Windows Sockets-Client verwendet](using-sspi-with-a-windows-sockets-client.md).</span><span class="sxs-lookup"><span data-stu-id="26f51-105">This sample program works with the client program [Using SSPI with a Windows Sockets Client](using-sspi-with-a-windows-sockets-client.md).</span></span> <span data-ttu-id="26f51-106">Beide Beispiel Programme verwenden die Header Datei sspiexample. h, die in der [Header Datei für SSPI-Client-und-Server Beispiele](header-file-for-sspi-client-and-server.md)zu finden ist.</span><span class="sxs-lookup"><span data-stu-id="26f51-106">Both sample programs use the header file SspiExample.h which can be found in [Header File for SSPI Client and Server Samples](header-file-for-sspi-client-and-server.md).</span></span>

<span data-ttu-id="26f51-107">Dieses Programm umfasst Aufrufe von Funktionen in Secur32. lib und Ws2 \_ 32. lib, die in den Linkbibliotheken enthalten sein müssen.</span><span class="sxs-lookup"><span data-stu-id="26f51-107">This program includes calls to functions in Secur32.lib and Ws2\_32.lib, which must be included among the link libraries.</span></span>

<span data-ttu-id="26f51-108">Dieses Programm veranschaulicht Folgendes:</span><span class="sxs-lookup"><span data-stu-id="26f51-108">This program demonstrates the following:</span></span>

-   <span data-ttu-id="26f51-109">Einrichten einer Windows Sockets-Verbindung mit einem Client.</span><span class="sxs-lookup"><span data-stu-id="26f51-109">Establishing a Windows Sockets connection with a client.</span></span>
-   <span data-ttu-id="26f51-110">Initialisieren einer Sitzung.</span><span class="sxs-lookup"><span data-stu-id="26f51-110">Initializing a session.</span></span>
-   <span data-ttu-id="26f51-111">Herstellen einer Verbindung mit einem Client und Einrichten einer sicheren Kommunikationssitzung.</span><span class="sxs-lookup"><span data-stu-id="26f51-111">Connecting with a client and establishing a secure communication session.</span></span> <span data-ttu-id="26f51-112">Der Server bietet den Aushandlungs-SSP, der entweder mit NTLM oder Kerberos auf den Client reagieren kann.</span><span class="sxs-lookup"><span data-stu-id="26f51-112">The server here offers the Negotiate SSP which can respond to the client with either NTLM or Kerberos.</span></span>
-   <span data-ttu-id="26f51-113">Verwendet Client Identitätswechsel und kehrt auf sich selbst zurück.</span><span class="sxs-lookup"><span data-stu-id="26f51-113">Uses client impersonation and reverting to self.</span></span>
-   <span data-ttu-id="26f51-114">Sendet dem Client eine verschlüsselte Nachricht.</span><span class="sxs-lookup"><span data-stu-id="26f51-114">Sends the client an encrypted message.</span></span>

<span data-ttu-id="26f51-115">In diesem Beispielprogramm wird die eingeschränkte Fehlerbehandlung verwendet.</span><span class="sxs-lookup"><span data-stu-id="26f51-115">This sample program uses limited error handling.</span></span>
