...
---
# <a name="cim_volatilestorage-class"></a>CIM- \_ VolatileStorage-Klasse

Die **CIM \_ VolatileStorage** -Klasse stellt die Funktionen und die Verwaltung von flüchtigem Speicher dar.
...
Die **CIM- \_ VolatileStorage** -Klasse verfügt über diese Typen von Membern:
...
Die **CIM \_ VolatileStorage** -Klasse verfügt über diese Methoden.



| Methode                                                                     | BESCHREIBUNG                                                                                                                              |
|:---------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-volatilestorage.md)                 | Fordert eine zurück setzung des logischen Geräts an. Wird nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-volatilestorage.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und einen Zeitpunkt, zu dem ein Gerät in diesen Zustand versetzt werden soll. Wird nicht von WMI implementiert.<br/> |
...
Die **CIM \_ VolatileStorage** -Klasse verfügt über diese Eigenschaften.
...
Qualifizierer: [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF". DMTF- \| System Ressourcen Speicher Info \| 001,5 ")
</dt> </dl>

Wenn der Wert **true** ist, kann dieser Arbeitsspeicher zwischengespeichert werden.
...
Qualifizierer: [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF". DMTF- \| System Ressourcen Speicher Info \| 001,6 ")
</dt> </dl>

Der Cachetyp, der mit dem Arbeitsspeicher kompatibel ist. Wenn die **cachable** -Eigenschaft auf **false** festgelegt ist, hat diese Eigenschaft keine Bedeutung und sollte auf 4 ("nicht zutreffend") festgelegt werden.
...
Daten, die während des letzten fehlerhaften Speicherzugriffs aufgezeichnet wurden. Die Daten belegen die ersten *n* Oktette des Arrays, die die Anzahl der Bits enthalten muss, die von der **ErrorTransferSize** -Eigenschaft angegeben werden. Wenn **ErrorTransferSize** gleich 0 ist, hat diese Eigenschaft keine Bedeutung.
...
Reihenfolge der im **ErrorData** -Array gespeicherten Daten. Wenn **ErrorTransferSize** gleich 0 ist, hat diese Eigenschaft keine Bedeutung.
...
Der Typ des Fehlers, der zuletzt aufgetreten ist. Die Werte 12-14 sind im CIM-Schema nicht definiert, da die Semantik des Fehler Typs und der Fehler, der korrigiert werden konnte, in DMI gemischt werden. Die Eigenschaft " **correctableerror** " gibt an, ob Sie korrigiert werden konnte.
...
Eine frei Form Zeichenfolge, die den Typ der Fehlererkennung und-Korrektur beschreibt, der vom Speicherblock unterstützt wird.
...
Größe der Datenübertragung in Bits, die den letzten Fehler verursacht hat. 0 gibt keinen Fehler an. Wenn die **errorInfo** -Eigenschaft gleich 3, "OK" ist, sollte diese Eigenschaft auf "0" festgelegt werden.
...
Eine frei Form Zeichenfolge, die zusätzliche Informationen bereitstellt, wenn die **ErrorType** -Eigenschaft auf 1 ("Other") festgelegt ist. Wenn ein anderer Wert als 1 (eins) ist, hat diese Zeichenfolge keine Bedeutung.
...
Anfangsadresse, auf die von einer Anwendung oder einem Betriebssystem verwiesen wird und die von einem Speichercontroller für das Speicher Objekt zugeordnet werden. Die Startadresse wird in Kilobyte angegeben.
...
Die **CIM- \_ VolatileStorage** -Klasse wird vom [**CIM- \_ Speicher**](cim-memory.md)abgeleitet.
