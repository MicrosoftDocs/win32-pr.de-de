---
description: Die CIM \_ NumericSensor-Klasse stellt einen numerischen Sensor dar, der numerische Messwerte zurückgibt und optional Schwellenwerteinstellungen unterstützt.
...
ms.locfileid: "119820930"
---
# <a name="cim_numericsensor-class"></a>CIM \_ NumericSensor-Klasse

Die **CIM \_ NumericSensor-Klasse** stellt einen numerischen Sensor dar, der numerische Messwerte zurückgibt und optional Schwellenwerteinstellungen unterstützt.
...
Die **CIM \_ NumericSensor-Klasse** verfügt über folgende Typen von Membern:
...
Die **CIM \_ NumericSensor-Klasse** verfügt über diese Methoden.



| Methode                                                                   | BESCHREIBUNG                                                                                                                              |
|:-------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-numericsensor.md)                 | Fordert eine Zurücksetzung des logischen Geräts an. Nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-numericsensor.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und wann ein Gerät in diesen Zustand versetzt werden soll. Nicht von WMI implementiert.<br/> |
...
Die **CIM \_ NumericSensor-Klasse** verfügt über diese Eigenschaften.
...
Klasse oder Unterklasse, die bei der Erstellung einer -Instanz verwendet wird. Bei Verwendung mit anderen Schlüsseleigenschaften der -Klasse ermöglicht diese Eigenschaft die eindeutige Identifizierung aller Instanzen der -Klasse und ihrer Unterklassen.
...
Schwellenwert, der die Bereiche (Mindest- und Höchstwerte) angibt, um zu bestimmen, ob der Sensor unter normalen, nicht kritischen, kritischen oder schwerwiegenden Bedingungen betrieben wird. Wenn die **CurrentReading-Eigenschaft** unter **LowerThresholdFatal** liegt, ist der aktuelle Zustand schwerwiegend.
...
Schwellenwert, der die Bereiche (Mindest- und Höchstwerte) angibt, um zu bestimmen, ob der Sensor unter normalen, nicht kritischen, kritischen oder schwerwiegenden Bedingungen betrieben wird. Wenn sich die **CurrentReading-Eigenschaft** zwischen **LowerThresholdNonCritical** und **UpperThresholdNonCritical** befindet, meldet der Sensor einen normalen Wert. Wenn sich die **CurrentReading-Eigenschaft** zwischen **LowerThresholdNonCritical** und **LowerThresholdCritical** befindet, ist der aktuelle Status nicht kritisch.
...
Die **CIM \_ NumericSensor-Klasse** wird vom [**\_ CIM-Sensor abgeleitet.**](cim-sensor.md)

WMI implementiert diese Klasse nicht. Informationen zu Klassen, die von **CIM \_ NumericSensor abgeleitet wurden,** finden Sie unter [Win32-Klassen.](win32-provider.md)
