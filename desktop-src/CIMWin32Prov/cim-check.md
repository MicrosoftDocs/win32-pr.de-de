...
Die **CIM- \_ Check** -Klasse stellt eine Bedingung oder ein Merkmal dar, die in einer Umgebung erwartungsgemäß definiert ist, die von einer Instanz einer [**CIM \_ Computersystem**](cim-computersystem.md) -Klasse definiert oder festgelegt wird. Die Überprüfungen, die einem bestimmten Softwareelement zugeordnet sind, werden in einer von zwei Gruppen organisiert, wobei die **Phase** -Eigenschaft der [**CIM- \_ softwareelementchecks**](cim-softwareelementchecks.md) -Zuordnung verwendet wird.

Bedingungen, die erwartungsgemäß erfüllt werden, wenn sich ein Softwareelement in einer bestimmten Umgebung befindet, werden als Zustands Zustände bezeichnet. Bedingungen, die erfüllt sein müssen, um das aktuelle Softwareelement in seinen nächsten Zustand zu versetzen, werden als Bedingungen des nächsten Zustands bezeichnet.

Ein [**CIM \_ Computersystem**](cim-computersystem.md) -Objekt stellt die Umgebung dar, in der ein [**CIM- \_ Softwareelement**](cim-softwareelement.md) bereits installiert ist oder in dem ein **CIM- \_ Softwareelement** installiert wird. Für den Fall, in dem ein Softwareelement bereits installiert ist, wird die [**CIM \_ InstalledSoftwareElement**](cim-installedsoftwareelement.md) -Zuordnung verwendet, um das **CIM \_ Computersystem** -Objekt zu identifizieren, das die "Umgebung" darstellt. Wenn ein Softwareelement verteilt und auf einem anderen Computer installiert wird, ist das **CIM \_ Computersystem** -Objekt für das Zielsystem die Umgebung.
...
Die **CIM- \_ Check** -Klasse verfügt über diese Typen von Membern:
...
Die **CIM- \_ Check** -Klasse verfügt über diese Methoden.



| Methode                                             | BESCHREIBUNG                                                   |
|:---------------------------------------------------|:--------------------------------------------------------------|
| [**Invoke**](invoke-method-in-class-cim-check.md) | Führt eine bestimmte Aktion aus. Wird nicht von WMI implementiert.<br/> |
...
Die **CIM- \_ Check** -Klasse verfügt über diese Eigenschaften.
...
Diese Klasse wird von WMI nicht implementiert. Weitere Informationen zu von CIM-Prüfungen abgeleiteten Klassen finden **\_ Sie** unter [Win32-Klassen](win32-provider.md).
