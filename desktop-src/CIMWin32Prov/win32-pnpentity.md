...
---
# <a name="win32_pnpentity-class"></a>Win32- \_ pnptity-Klasse

Die **Win32 \_ pnptity** - [WMI-Klasse](../wmisdk/retrieving-a-class.md) stellt die Eigenschaften eines Plug & Play Geräts dar. Plug & Play Entitäten werden als Einträge in der Device Manager in der Systemsteuerung angezeigt.
...
Die **Win32- \_ pnptity** -Klasse verfügt über diese Typen von Membern:
...
Die **Win32- \_ pnptity** -Klasse verfügt über diese Methoden.



| Methode                                                             | BESCHREIBUNG                                                                                                                                                                                    |
|:-------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [**Ier**](disable-win32-pnpentity.md)                         | Deaktiviert dieses Plug & Play Gerät.<br/>                                                                                                                                                 |
| [**Aktivieren**](enable-win32-pnpentity.md)                           | Aktiviert dieses Plug & Play Gerät.<br/>                                                                                                                                                  |
| [**Getdeviceproperties**](getdeviceproperties-win32-pnpentity.md) | Ruft die angegebenen Eigenschaften dieses Plug & Play Geräts ab.<br/>                                                                                                                         |
| **Zurücksetzen**                                                          | Nicht implementiert. Informationen zur Implementierung dieser Methode finden Sie unter der [**Reset**](reset-method-in-class-cim-controller.md) -Methode in [**CIM \_ LogicalDevice**](cim-logicaldevice.md).<br/>                 |
| **SetPowerState**                                                  | Nicht implementiert. Informationen zur Implementierung dieser Methode finden Sie unter der [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) -Methode in [**CIM \_ LogicalDevice**](cim-logicaldevice.md).<br/> |
...
Die **Win32- \_ pnptity** -Klasse verfügt über diese Eigenschaften.
...
GUID (Global Unique Identifier) dieses Plug & Play Geräts.
...
Eine Hersteller definierte Identifikations Zeichenfolge, die von Setup verwendet wird, um ein Gerät mit einer INF-Datei abzugleichen. Ein Gerät kann über eine Liste der zugeordneten kompatiblen IDs verfügen. Die kompatiblen IDs sollten in der Reihenfolge der absteigenden Eignung aufgeführt werden. Wenn von Setup keine INF-Datei gefunden werden kann, die mit einer der Hardware-IDs eines Geräts übereinstimmt, werden kompatible IDs verwendet, um nach einer INF-Datei zu suchen. Eine kompatible ID hat das gleiche Format wie eine **HardwareID**. Weitere Informationen finden Sie unter [Windows-Treiberkit](https://developer.microsoft.com/windows/hardware/download-kits-windows-hardware-development).
...
Der Bezeichner des Plug & Play Geräts.
...
Eine Hersteller definierte Identifikations Zeichenfolge, die von Setup verwendet wird, um ein Gerät mit einer INF-Datei abzugleichen. Normalerweise verfügt ein Gerät über eine zugeordnete Liste von Hardware-IDs. Eine Ausnahme ist der 1394-Bustreiber, der keine Hardware-IDs verwendet. Die erste Hardware-ID in der Liste sollte die Geräte-ID sein. Die verbleibenden IDs sollten in der Reihenfolge der absteigenden Eignung aufgeführt werden.

Hardware-IDs werden in einem der folgenden Formate angezeigt:

-   \\enumeratorenumerator-spezifisch-Geräte-ID

    Dies ist das gängigste Format für einzelne PNP-Geräte. Ein Beispiel für einen Enumerator ist das BIOS oder ISAPNP.

-   \*enumeratorspezifische ID

    Ein Sternchen ( \* ) gibt an, dass mehr als ein Enumerator verwendet werden soll.

-   Geräteklassen spezifische ID
...
Weitere Informationen finden Sie im [Windows-Treiberkit](https://developer.microsoft.com/windows/hardware/download-kits-windows-hardware-development).
...
Der Name des Herstellers des Plug & Play Geräts.
...
> [!WARNING]
> Diese Eigenschaft ist, obwohl Sie in der MOF-Datei aufgelistet ist, in der-Klasse nicht vorhanden. Die-Eigenschaft wird hier nur aus Gründen der Vollständigkeit beschrieben und um die MOF-Datei selbst zu verdeutlichen.

 

Der Name des Typs dieses Plug & Play Geräts.

**Windows Server 2012 R2, Windows 8.1, Windows Server 2012, Windows 8, Windows Server 2008 R2, Windows 7, Windows Server 2008 und Windows Vista:** Diese Eigenschaft befindet sich nicht in der MOF-Datei.
...
Name des diensdienstanbieter, der dieses Plug & Play Gerät unterstützt. Weitere Informationen finden Sie unter [**Win32 \_ systemdriverpnptity**](win32-systemdriverpnpentity.md).
...
Die **Win32- \_ pnptity** -Klasse wird von [**CIM \_ LogicalDevice**](cim-logicaldevice.md)abgeleitet.

## <a name="examples"></a>Beispiele

Das [Get-BrokenHardware.ps1](https://Gallery.TechNet.Microsoft.Com/dbb678f4-b95b-45c3-bc8b-2ae2d052448e) PowerShell-Beispiel in der TechNet Gallery verwendet zum Abrufen einer Liste mit nicht funktionierenden Hardware mithilfe von WMI den Win32-Wert von **Win32 \_** .

Im folgenden VBScript-Codebeispiel wird eine Verbindung mit einer Gruppe von Remote Computern in derselben Domäne hergestellt, indem ein Array von Remote Computernamen erstellt und dann die Namen der Plug & Play Geräte – Instanzen von Win32 \_ pnptity – auf den einzelnen Computern angezeigt werden.
...
[Herstellen einer Verbindung mit WMI auf einem Remote Computer](../wmisdk/connecting-to-wmi-on-a-remote-computer.md)
</dt> <dt>
