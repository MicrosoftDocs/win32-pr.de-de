...
Die **CIM \_ diskdrive** -Klasse verfügt über diese Typen von Membern:
...
Die **CIM \_ diskdrive** -Klasse verfügt über diese Methoden.



| Methode                                                                | BESCHREIBUNG                                                                                                                              |
|:----------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-cdromdrive.md)                 | Fordert eine zurück setzung des logischen Geräts an. Wird nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-cdromdrive.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und einen Zeitpunkt, zu dem ein Gerät in diesen Zustand versetzt werden soll. Wird nicht von WMI implementiert.<br/> |
...
Die **CIM \_ diskdrive** -Klasse verfügt über diese Eigenschaften.
...
Es ist bekannt, dass sich das Gerät in einem Energiesparmodus befindet, aber der genaue Status in diesem Modus ist unbekannt.
...
Das Gerät befindet sich in einem Warn Status und auch in einem Energiesparmodus.
...
Funktionen des Medien Zugriffs Geräts. Diese Eigenschaft wird von [**CIM \_ mediaaccessdevice**](cim-mediaaccessdevice.md)geerbt.
...
Unterscheidet ein Gerät, das auf beide Seiten eines zweiseitigen Mediums zugreifen kann, von einem Gerät, das nur eine einzelne Seite liest und erfordert, dass die Medien eingeschaltet werden.
...
Gibt an, dass das Medium nicht explizit vom Gerät ausworfen werden muss, bevor von einem Auswahl Element darauf zugegriffen wird.
...
Array von Freiform-Zeichen folgen, die ausführliche Erläuterungen **für den Zugriff** auf Gerätefunktionen bereitstellen, die im Funktions Array angegeben sind. Diese Eigenschaft wird von [**CIM \_ mediaaccessdevice**](cim-mediaaccessdevice.md)geerbt.
...
Standard Blockgröße (in Bytes) für das Gerät.
...
Wenn der Wert **true** ist, wird der in der Eigenschaft **LastErrorCode** gemeldete Fehler gelöscht.
...
Eine frei Form Zeichenfolge, die den Typ der Fehlererkennung und-Korrektur beschreibt, die vom Gerät unterstützt werden.
...
Maximale Größe des vom Gerät unterstützten Mediums in Kilobyte. Kilobytes werden als Anzahl von Bytes, multipliziert mit 1000, interpretiert (nicht die Anzahl der Bytes multipliziert mit 1024).
...
**True** gibt an, dass das Medien Zugriffsgerät bereinigt werden muss. Ob eine manuelle oder automatische Bereinigung möglich ist, wird in der **Funktionen** -Array-Eigenschaft angegeben.
...
Wenn das Medien Zugriffsgerät mehrere einzelne Medien unterstützt, definiert diese Eigenschaft die maximale Anzahl, die unterstützt oder eingefügt werden kann.
...
Die [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) -Methode wird unterstützt.
...
Die [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) -Methode kann aufgerufen werden, wenn der *PowerState* -Parameter auf 5 festgelegt ist ("Power Cycle").
...
Die [**SetPowerState**](setpowerstate-method-in-class-cim-diskdrive.md) -Methode kann aufgerufen werden, wenn der *PowerState* -Parameter auf 5 ("Power Cycle") festgelegt ist und der *Zeit* Parameter auf ein bestimmtes Datum und eine bestimmte Uhrzeit bzw. ein bestimmtes Intervall festgelegt ist.
...
Die **CIM \_ diskdrive** -Klasse wird von [**CIM \_ mediaaccessdevice**](cim-mediaaccessdevice.md)abgeleitet.

Diese Klasse wird von WMI nicht implementiert. Siehe [Win32-Klassen](win32-provider.md) für Klassen, die von **CIM \_ diskdrive** abgeleitet werden.
