---
description: Die \_ CIM NonVolatileStorage-Klasse stellt die Funktionen und die Verwaltung von nicht flüchtigem Speicher dar. Nicht flüchtiger Arbeitsspeicher umfasst nativ Flash- und ROM-Speicher.
...
ms.locfileid: "119820960"
---
# <a name="cim_nonvolatilestorage-class"></a>CIM \_ NonVolatileStorage-Klasse

Die **\_ CIM NonVolatileStorage-Klasse** stellt die Funktionen und die Verwaltung von nicht flüchtigem Speicher dar. Nicht flüchtiger Arbeitsspeicher umfasst nativ Flash- und ROM-Speicher. Darüber hinaus kann nicht flüchtiger Speicher auf flüchtigem Speicher basieren, wenn der flüchtige Speicher durch einen Akku gesichert wird. Dieses Szenario wird beispielsweise von einer Instanz der [**CIM \_ AssociatedBattery-Beziehung**](cim-associatedbattery.md) beschrieben, die auf den nicht flüchtigen Speicher als **abhängig** und den Akku als **Vorgänger** verweist, und eine Instanz der [**\_ CIM-BasedOn-Beziehung,**](cim-basedon.md) die auf den nicht flüchtigen Speicher als **abhängigen** und den flüchtigen Speicher als **Vorgänger** verweist.
...
Die **\_ CIM NonVolatileStorage-Klasse** verfügt über diese Typen von Membern:
...
Die **\_ CIM NonVolatileStorage-Klasse** verfügt über diese Methoden.



| Methode                                                                        | BESCHREIBUNG                                                                                                                              |
|:------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-nonvolatilestorage.md)                 | Fordert eine Zurücksetzung des logischen Geräts an. Nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-nonvolatilestorage.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und wann ein Gerät in diesen Zustand versetzt werden soll. Nicht von WMI implementiert.<br/> |
...
Die **\_ CIM NonVolatileStorage-Klasse** verfügt über diese Eigenschaften.
...
Lese-/Schreibeigenschaften des Mediums.
...
Array von Oktetten, die zusätzliche Fehlerinformationen enthalten. Ein Beispiel hierfür ist die Fehlerüberprüfung und -korrektur (ECC) Oder die Rückgabe der Prüfbits, wenn eine CRC-basierte Fehlermethodik verwendet wird. Wenn im letzteren Fall ein Single-Bit-Fehler erkannt wird und der CRC-Algorithmus bekannt ist, kann das genaue fehlerhafte Bit bestimmt werden. Diese Art von Daten (ECC-, Check-Bit- oder Parity-Bit-Daten oder andere vom Anbieter bereitgestellte Informationen) ist in diesem Feld enthalten. Wenn die **ErrorInfo-Eigenschaft** gleich 3 ("OK") ist, hat diese Eigenschaft keine Bedeutung.
...
Endadresse, auf die von einer Anwendung oder einem Betriebssystem verwiesen und von einem Speichercontroller zugeordnet wird, für das Speicherobjekt. Die Endadresse wird in Kilobyte angegeben.
...
Enumeration, die den Speicherzugriffsvorgang angibt, der den letzten Fehler verursacht hat. Der Fehlertyp wird von der **ErrorInfo-Eigenschaft** beschrieben. Wenn die **ErrorInfo-Eigenschaft** gleich 3 ("OK") ist, hat diese Eigenschaft keine Bedeutung.
...
Fehlertyp, der zuletzt aufgetreten ist. Die Werte 12 bis 14 sind im CIM-Schema nicht definiert, da DMI die Semantik des Fehlertyps gemischt und ob er korrigiert werden konnte. Ob ein Fehler korrigiert werden kann, wird in der **CorrectableError-Eigenschaft** angegeben. Diese Eigenschaft wird vom [**\_ CIM-Arbeitsspeicher geerbt.**](cim-memory.md)
...
<span id="Bad_Read"></span><span id="bad_read"></span><span id="BAD_READ"></span>**Ungültiges Lesen** (4)
...
Gibt an, ob Paritätsalgorithmen, CRC-Algorithmen, ECC oder andere Mechanismen verwendet werden. Details zum Algorithmus können ebenfalls angegeben werden.
...
Qualifizierer: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. \|DMTF-Speichergerät \| 002.21", "MIF. DMTF \| Physical Memory Array \| 001.15"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("bytes")
</dt> </dl>

Bereich in Bytes, in den der letzte Fehler aufgelöst werden kann. Wenn z. B. Fehleradressen in Bit 11 aufgelöst werden (d. h. auf einer typischen Seitenbasis), können Fehler auf 4-KB-Grenzen aufgelöst werden, und diese Eigenschaft ist auf 4000 festgelegt. Wenn die **ErrorInfo-Eigenschaft** gleich 3 ("OK") ist, hat diese Eigenschaft keine Bedeutung.
...
Zeitpunkt, zu dem der letzte Speicherfehler aufgetreten ist. Der Fehlertyp wird durch die **ErrorInfo-Eigenschaft** beschrieben. Wenn die **ErrorInfo-Eigenschaft** gleich 3 ("OK") ist, hat diese Eigenschaft keine Bedeutung.
...
Größe der Datenübertragung in Bits, die den letzten Fehler verursacht haben. Der Wert 0 (null) gibt keinen Fehler an. Wenn die **ErrorInfo-Eigenschaft** gleich 3 ("OK") ist, sollte diese Eigenschaft auf 0 (null) festgelegt werden.
...
**True** gibt an, dass nicht flüchtiger Speicher schreibbar ist.
...
Qualifizierer: [**ModelCorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("[**\_ CIM-Arbeitsspeicher**](cim-memory.md).**ErrorInfo**")
</dt> </dl>

Freiformzeichenfolge, die Informationen bereitstellt, wenn die **ErrorType-Eigenschaft** auf 1 ("Other") festgelegt ist. Wenn diese Zeichenfolge nicht auf 1 festgelegt ist, hat sie keine Bedeutung.
...
Windows Plug & Play gerätebezeichner des logischen Geräts. Diese Eigenschaft wird von [**CIM \_ LogicalDevice geerbt.**](cim-logicaldevice.md)
...
Anfangsadresse, auf die von einer Anwendung oder einem Betriebssystem verwiesen und von einem Speichercontroller für dieses Speicherobjekt zugeordnet wird. Die Startadresse wird in Kilobyte angegeben.
...
True **gibt an,** dass die Adressinformationen in der **ErrorAddress-Eigenschaft** eine Adresse auf Systemebene sind. False **gibt an,** dass es sich um eine physische Adresse handelt. Wenn die **ErrorInfo-Eigenschaft** gleich 3 ("OK") ist, hat diese Eigenschaft keine Bedeutung.
...
Die **CIM \_ NonVolatileStorage-Klasse** wird von [**CIM Memory \_ abgeleitet.**](cim-memory.md)
