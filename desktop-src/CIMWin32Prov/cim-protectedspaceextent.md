...
---
# <a name="cim_protectedspaceextent-class"></a>CIM \_ protectedspaceblock-Klasse

Die **CIM \_ protectedspaceblock** -Klasse stellt adressierbare logische Block Adressen dar, die als einzelner Speicherblock behandelt werden, sich jedoch in einem einzelnen physischen Block befinden. Erweiterte Speicherplatz Blöcke schließen alle logischen Blöcke aus, die als Prüf Daten zugeordnet sind, und enthalten Informationen zu den detaillierten Zustellungs Informationen für Benutzerdaten. Eine alternative Möglichkeit, wenn die automatische Konfiguration verwendet wird, besteht darin, die [**CIM- \_ aggregatepsextent**](cim-aggregatepsextent.md) -Klasse zu instanziieren oder zu erweitern.
...
Die **CIM \_ protectedspaceblock** -Klasse verfügt über diese Typen von Membern:
...
Die **CIM \_ protectedspaceblock** -Klasse verfügt über diese Methoden.



| Methode                                                                          | BESCHREIBUNG                                                                                                                              |
|:--------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-protectedspaceextent.md)                 | Fordert eine zurück setzung des logischen Geräts an. Wird nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-protectedspaceextent.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und einen Zeitpunkt, zu dem ein Gerät in diesen Zustand versetzt werden soll. Wird nicht von WMI implementiert.<br/> |
...
Die **CIM \_ protectedspaceblock** -Klasse verfügt über diese Eigenschaften.
...
Block Größe der Blöcke in Bytes, die den Speicher Block bilden. Bei variabler Blockgröße sollte die maximale Blockgröße in Bytes angegeben werden. Wenn die Blockgröße unbekannt ist oder ein Block Konzept nicht gültig ist (z. b. für Aggregat Blöcke, Arbeitsspeicher oder logische Datenträger), geben Sie 1 (eins) ein.
...
Qualifizierer: [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF". Von DMTF \| geschützter Speicherbereich \| 001,6 "), [**Einheiten**](/windows/desktop/WmiSdk/standard-qualifiers) (" Bytes ")
</dt> </dl>

Bei einer **CIM \_ protectedspaceblock** -Klasse, die einer [**CIM \_ volumeset**](cim-volumeset.md) -Klasse zugeordnet ist, ist diese Eigenschaft die Anzahl von Benutzerdaten bytes, die im Bereich des geschützten Bereichs abgelegt werden, bevor Sie zum nächsten Bereich des geschützten Bereichs im Volumesatz wechselt. Andernfalls wird der Bereich des geschützten Bereichs als nicht zugeordnet betrachtet, und diese Eigenschaft muss auf 0 festgelegt werden.
...
Die **CIM \_ protectedspaceblock** -Klasse wird von [**CIM \_ storageblock**](cim-storageextent.md)abgeleitet.
