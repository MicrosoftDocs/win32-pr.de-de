...
---
# <a name="win32_mappedlogicaldisk-class"></a>Win32 \_ mappedlogicaldisk-Klasse

Die **Win32 \_ mappedlogicaldisk** - [WMI-Klasse](/windows/desktop/WmiSdk/retrieving-a-class) stellt Netzwerkspeicher Geräte dar, die auf dem Computersystem als logische Datenträger zugeordnet sind.
...
Die **Win32 \_ mappedlogicaldisk** -Klasse verfügt über diese Typen von Membern:
...
Die **Win32 \_ mappedlogicaldisk** -Klasse verfügt über diese Methoden.



| Methode            | BESCHREIBUNG                                                                                                                                                                                |
|:------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Zurücksetzen**         | Nicht implementiert. Informationen zur Implementierung dieser Methode finden Sie unter der [**Reset**](reset-method-in-class-cim-controller.md) -Methode in [**CIM \_ LogicalDisk**](cim-logicaldisk.md).<br/>                 |
| **SetPowerState** | Nicht implementiert. Informationen zur Implementierung dieser Methode finden Sie unter der [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) -Methode in [**CIM \_ LogicalDisk**](cim-logicaldisk.md).<br/> |
...
Die **Win32 \_ mappedlogicaldisk** -Klasse verfügt über diese Eigenschaften.
...
Größe (in Bytes) der Blöcke, die diesen Speicherblock bilden. Wenn dieses Konzept für den Gerätetyp nicht gültig ist, ist der Wert 1.
...
Qualifizierer: [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("Win32API \| File System Functions \| [**GetVolumeInformation**](/windows/desktop/api/fileapi/nf-fileapi-getvolumeinformationa) \| FS \_ Vol \_ ist \_ komprimiert")
</dt> </dl>
...
Eindeutiger Bezeichner des Speicherarrays.

Diese Eigenschaft wird von [**CIM \_ LogicalDevice**](cim-logicaldevice.md)geerbt.

Beispiel: "Arbeitsspeicher Array 1"
...
Weitere Informationen zu dem Fehler, der in **LastErrorCode** aufgezeichnet wurde, sowie Informationen zu den Maßnahmen, die ausgeführt werden können, finden Sie unter.
...
Typen von Fehlerüberprüfungen, die von der Hardware verwendet werden.
...
Auf dem logischen Datenträger verfügbarer Speicherplatz.

Diese Eigenschaft wird von [**CIM \_ LogicalDisk**](cim-logicaldisk.md)geerbt.
...
Enthält die maximale Länge einer Dateinamen Komponente, die vom Windows-Laufwerk unterstützt wird.
...
Qualifizierer: [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("Win32API \| Windows Networking Functions \| WNetGetConnection")
</dt> </dl>

Der Netzwerk Pfadname für das logische Gerät.
...
**True** gibt an, dass die Kontingent Verwaltung für dieses Volume nicht aktiviert ist.
...
**True** gibt an, dass die Kontingent Verwaltung verwendet, aber deaktiviert wurde. Unvollständig bezieht sich auf die im Dateisystem verbleibenden Informationen, nachdem die Kontingent Verwaltung deaktiviert wurde.
...
**True** gibt an, dass das Dateisystem für die Kontingent Verwaltung eingerichtet wird.
...
Die ID der Sitzung des Benutzers. Der Benutzer kann mit einer lokalen Anmeldung oder einer Terminalsitzung verbunden sein.
...
Größe des logischen Datenträgers.
...
**True** gibt an, dass das Dateisystem, dem dieses Netzwerklaufwerk zugeordnet ist, Datenträger Kontingente unterstützt.
...
Qualifizierer: [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("Win32API \| File System Functions \| [**GetVolumeInformation**](/windows/desktop/api/fileapi/nf-fileapi-getvolumeinformationa) \| FS \_ file \_ Compression")
</dt> </dl>

**True** gibt an, dass die logische Datenträger Partition dateibasierte Komprimierung unterstützt, wie z. b. bei NTFS. Diese Eigenschaft ist **false**, wenn die **komprimierte** Eigenschaft den Wert **true** hat.
...
Der Volumename des logischen Datenträgers. Dieser Eigenschafts Wert kann maximal 32 Zeichen umfassen.
...
Seriennummer des logischen Datenträgers. Dieser Eigenschafts Wert kann maximal 11 Zeichen umfassen.
...
Die für diese Klasse zurückgegebenen Instanzen lauten wie folgt, um zu untersuchen, dass Benutzer A die-Instanzen auflistet:

-   Der Anbieter sucht eine Anmelde Sitzung von Benutzer a auf diesem Computer:

    -   Wenn eine solche Anmelde Sitzung (und nur eine) vorhanden ist, gibt der Anbieter die zugeordneten Laufwerke dieser Sitzung zurück.
    -   Wenn für Benutzer A auf dem Computer mehr als eine Sitzung vorhanden ist, werden keine zugeordneten Laufwerks Instanzen zurückgegeben (weil der Anbieter keine angemessene Methode zur Entscheidung über die zu verwendende Sitzung hat).

-   Wenn keine Sitzungen von Benutzer a ausgeführt werden und ein lokal angemeldeter Benutzer B vorhanden ist:

    -   Wenn eine einzelne Sitzung für Benutzer b vorhanden ist, nimmt der Anbieter einen Identitätswechsel vor und gibt die zugeordneten Laufwerke von Benutzer b zurück. In diesem Fall wird das Szenario des Helpdesk unterstützt, in dem die Instanzen eines lokal angemeldeten Benutzers angezeigt werden. Ob Instanzen zurückgegeben werden, hängt jedoch von den Einstellungen der lokalen Sicherheitsrichtlinie in der Systemsteuerung Verwaltungs Tools ab. Wenn die folgende Richtlinie auf "Objekt Ersteller" festgelegt ist, werden keine zugeordneten Laufwerks Instanzen zurückgegeben, selbst wenn ein Mitglied der Gruppe "Administratoren" ist:

        "System Objekt: Standard Besitzer für Objekte, die von Mitgliedern der Gruppe" Administratoren "erstellt wurden.

    -   Wenn mehr als eine Sitzung von Benutzer B auf dem Computer ausgeführt wird, hat der Anbieter keine Möglichkeit, zu entscheiden, welche verwendet werden soll. In diesem Fall werden keine zugeordneten Laufwerks Instanzen zurückgegeben.

Weitere Informationen zur Verwendung von **Win32 \_ mappedlogicaldisk** finden Sie unter [wie kann ich ermitteln, welche Laufwerke Netzwerkfreigaben zugeordnet sind?](https://blogs.technet.com/b/heyscriptingguy/archive/2005/10/27/how-can-i-determine-which-drives-are-mapped-to-network-shares.aspx)

## <a name="examples"></a>Beispiele

Der folgende PowerShell-Code Ausschnitt zeigt zugeordnete Laufwerke.
