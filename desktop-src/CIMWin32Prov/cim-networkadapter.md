---
description: Die CIM NetworkAdapter-Klasse ist eine abstrakte Klasse, die allgemeine Netzwerkhardwarekonzepte definiert (z. B. \_ permanente Adresse oder Betriebsgeschwindigkeit). Die Informationen werden mithilfe der CIM \_ DeviceSAPImplementation-Zuordnung übermittelt.
...
ms.locfileid: "119021028"
---
# <a name="cim_networkadapter-class"></a>CIM \_ NetworkAdapter-Klasse

Die **CIM \_ NetworkAdapter-Klasse** ist eine abstrakte Klasse, die allgemeine Netzwerkhardwarekonzepte definiert (z. B. permanente Adresse oder Betriebsgeschwindigkeit). Die Informationen werden mithilfe der [**CIM \_ DeviceSAPImplementation-Zuordnung**](cim-devicesapimplementation.md) übermittelt.

Netzwerkadapter und zugehörige Endpunkte stellen das Potenzial für die Konnektivität zwischen Peers dar. Die Konnektivitätsmöglichkeiten unterscheiden sich erheblich von den Beziehungen master oder subordinate und controller oder controlled-by des [**\_ CIM-Controllers.**](cim-controller.md) Gelegentlich fungiert ein einzelnes Gerät jedoch sowohl als Netzwerkadapter als auch als Controller, z. B. wenn ein Fiberkanaladapter als SCSI-Controller eines Computersystems fungiert. In diesem Fall sind Aspekte des Geräts netzwerkorientiert, während andere controllerorientiert sind. Die Controller- und Adapterklassen sollten instanziiert werden, und es sollte auch eine Geräteidentitätsbeziehung erstellt werden, um die verschiedenen Aspekte des Geräts zu verknüpfen.
...
Die **CIM \_ NetworkAdapter-Klasse** verfügt über diese Membertypen:
...
Die **CIM \_ NetworkAdapter-Klasse** verfügt über diese Methoden.



| Methode                                                                    | BESCHREIBUNG                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-networkadapter.md)                 | Fordert eine Zurücksetzung des logischen Geräts an. Nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und wann ein Gerät in diesen Zustand eingedrung werden soll. Nicht von WMI implementiert.<br/> |
...
Die **CIM \_ NetworkAdapter-Klasse** verfügt über diese Eigenschaften.
...
True **gibt an,** dass der Netzwerkadapter automatisch die Geschwindigkeit oder andere Kommunikationsmerkmale der angeschlossenen Netzwerkmedien bestimmen kann.
...
Maximale Geschwindigkeit in Bits pro Sekunde (BPS) für den Netzwerkadapter.
...
Liste der Netzwerkadressen für einen Adapter.
...
Netzwerkadresse, die in einen Adapter hart codiert ist. Die hart codierte Adresse kann mit einem Firmwareupgrade oder einer Softwarekonfiguration geändert werden. Wenn das Feld geändert wird, sollte es aktualisiert werden, wenn die Änderung vorgenommen wird. Diese Eigenschaft sollte leer gelassen werden, wenn keine hart codierte Adresse für den Netzwerkadapter vorhanden ist.
...
Qualifizierer: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| RFC1213-MIB.ifSpeed", "MIF. \|DMTF-Netzwerkadapter 802 Port \| 001.5"), [**Einheiten**](/windows/desktop/WmiSdk/standard-qualifiers) ("Bits pro Sekunde")
</dt> </dl>

Schätzung der aktuellen Bandbreite in BPS. Für Endpunkte, die in Bandbreite variieren, oder für Endpunkte, für die keine genaue Schätzung vorgenommen werden kann, sollte diese Eigenschaft die nominale Bandbreite enthalten.
...
Die **CIM \_ NetworkAdapter-Klasse** wird von [**CIM \_ LogicalDevice abgeleitet.**](cim-logicaldevice.md)

WMI implementiert diese Klasse nicht. Informationen zu Klassen, die von **CIM \_ NetworkAdapter abgeleitet sind,** finden Sie unter [Win32-Klassen](win32-provider.md).
