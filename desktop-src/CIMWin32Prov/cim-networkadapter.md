...
---
# <a name="cim_networkadapter-class"></a>CIM- \_ netzwerkadapterklasse

Die **CIM- \_ netzwerkadapterklasse** ist eine abstrakte Klasse, die allgemeine Konzepte der Netzwerkhardware definiert (z. b. permanente Adresse oder Geschwindigkeit der Operation). Die Informationen werden mithilfe der CIM-Zuordnung " [**\_ devicesapimplementation**](cim-devicesapimplementation.md) " übermittelt.

Netzwerkadapter und verwandte Endpunkte stellen das Potenzial der Konnektivität zwischen Peers dar. Das Verbindungs Potenzial unterscheidet sich deutlich vom Master-oder untergeordneten und Controller-bzw. von der kontrollierten Beziehung des [**CIM- \_ Controllers**](cim-controller.md). Gelegentlich fungiert ein einzelnes Gerät jedoch sowohl als Netzwerkadapter als auch als Controller, z. b. Wenn ein Fiber-Channel-Adapter als SCSI-Controller eines Computer Systems fungiert. In diesem Fall sind die Aspekte des Geräts netzwerkorientiert, andere sind Controller orientiert. Die Controller-und Adapter Klassen sollten instanziiert werden, und eine Geräte Identitäts Beziehung sollte ebenfalls erstellt werden, um die verschiedenen Aspekte des Geräts zu verknüpfen.
...
Die **CIM- \_ netzwerkadapterklasse** verfügt über diese Typen von Membern:
...
Die **CIM- \_ netzwerkadapterklasse** verfügt über diese Methoden.



| Methode                                                                    | BESCHREIBUNG                                                                                                                              |
|:--------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-networkadapter.md)                 | Fordert eine zurück setzung des logischen Geräts an. Wird nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-networkadapter.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und einen Zeitpunkt, zu dem ein Gerät in diesen Zustand versetzt werden soll. Wird nicht von WMI implementiert.<br/> |
...
Die **CIM- \_ netzwerkadapterklasse** verfügt über diese Eigenschaften.
...
Wenn der Wert **true** ist, kann der Netzwerkadapter die Geschwindigkeit oder andere Kommunikationsmerkmale der angeschlossenen Netzwerk Medien automatisch bestimmen.
...
Maximale Geschwindigkeit in Bits pro Sekunde (Bit/s) für den Netzwerkadapter.
...
Die Netzwerkadresse ist in einem Adapter hart codiert. Die hart codierte Adresse kann durch ein Firmwareupdate oder eine Softwarekonfiguration geändert werden. Wenn Sie geändert wird, sollte das Feld aktualisiert werden, wenn die Änderung vorgenommen wird. Diese Eigenschaft sollte leer gelassen werden, wenn für den Netzwerkadapter keine hart codierte Adresse vorhanden ist.
...
Qualifizierer: [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIB. IETF \| RFC1213-MIB. IfSpeed "," MIF. DMTF- \| Netzwerk Adapter 802-Port \| 001,5 "), [**Einheiten**](/windows/desktop/WmiSdk/standard-qualifiers) (Bits pro Sekunde)
</dt> </dl>

Schätzung der aktuellen Bandbreite in Bit/s. Für Endpunkte, die die Bandbreite variieren, oder für diejenigen, bei denen keine genaue Schätzung vorgenommen werden kann, sollte diese Eigenschaft die nominale Bandbreite enthalten.
...
Die **CIM- \_ netzwerkadapterklasse** wird von [**CIM \_ LogicalDevice**](cim-logicaldevice.md)abgeleitet.

Diese Klasse wird von WMI nicht implementiert. Informationen zu Klassen, die von **CIM \_ Network Adapter** abgeleitet sind, finden Sie unter [Win32-Klassen](win32-provider.md).
