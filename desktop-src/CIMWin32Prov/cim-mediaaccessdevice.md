...
---
# <a name="cim_mediaaccessdevice-class-cimwin32-wmi-providers"></a>CIM_MediaAccessDevice-Klasse (cimwin32-WMI-Anbieter)

Die **CIM \_ mediaaccessdevice** -Klasse stellt die Möglichkeit dar, auf ein oder mehrere Medien zuzugreifen und dann die Medien zum Speichern und Abrufen von Daten zu verwenden.
...
|:-----------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**Zurücksetzen**](reset-method-in-class-cim-mediaaccessdevice.md)                 | Fordert eine zurück setzung des logischen Geräts an. Wird nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-mediaaccessdevice.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und einen Zeitpunkt, zu dem ein Gerät in diesen Zustand versetzt werden soll. Wird nicht von WMI implementiert.<br/> |
...
Qualifizierer: [**arrayType**](/windows/desktop/WmiSdk/standard-qualifiers) ("indiziert"), [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF". DMTF- \| Speichergeräte \| 001,9 "," MIF. DMTF- \| Speichergeräte \| 001,11 "," MIF. DMTF- \| Speichergeräte \| 001,12 "," MIF. DMTF-Datenträger \| \| 003,7 "), [**modelkorrespondenz**](/windows/desktop/WmiSdk/standard-qualifiers) ("**CIM \_ mediaaccessdevice**.**Capabilitybeschreibungen**")
</dt> </dl>

Funktionen des Medien Zugriffs Geräts.
...
Array von Freiform-Zeichen folgen, das ausführliche Erläuterungen **für den Zugriff** auf Gerätefunktionen bereitstellt, die im Funktions Array angegeben sind.
...
Eine frei Form Zeichenfolge, die den Algorithmus oder das Tool angibt, das zum Komprimieren der logischen Datei verwendet wird. Wenn es nicht möglich ist, das Komprimierungs Schema zu beschreiben (weil es unbekannt ist), verwenden Sie Folgendes: Wenn, verwenden Sie "unknown". Wenn, verwenden Sie "komprimiert". , verwenden Sie "nicht komprimiert".
...
Maximale Größe des von diesem Gerät unterstützten Mediums in Kilobyte. Kilobytes werden als Anzahl von Bytes, multipliziert mit 1000, interpretiert (nicht die Anzahl der Bytes multipliziert mit 1024).
...
Die **CIM \_ mediaaccessdevice** -Klasse wird von [**CIM \_ LogicalDevice**](cim-logicaldevice.md)abgeleitet.

Diese Klasse wird von WMI nicht implementiert. Informationen zu Klassen, die von **CIM \_ mediaaccessdevice** abgeleitet sind, finden Sie unter [Win32 Classes](win32-provider.md).
