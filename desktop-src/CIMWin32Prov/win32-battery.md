...
ms.locfileid: "119917990"
---
# <a name="win32_battery-class"></a>Win32 \_ Battery-Klasse

Die **WMI-Klasse "Win32 \_ Battery"** [stellt](/windows/desktop/WmiSdk/retrieving-a-class) einen Akku dar, der mit dem Computersystem verbunden ist.
...
Die **Win32 \_ Battery-Klasse** verfügt über die folgenden Membertypen:
...
Die **Win32 \_ Battery-Klasse** verfügt über diese Methoden.



| Methode            | Beschreibung                                                                                                                                                                                          |
|:------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Zurücksetzen**         | Nicht implementiert. Informationen zum Implementieren dieser Methode finden Sie in der [**Dokumentation zur Reset-Methode**](reset-method-in-class-cim-controller.md) in [**CIM \_ Battery.**](cim-battery.md)<br/>                 |
| **SetPowerState** | Nicht implementiert. Informationen zum Implementieren dieser Methode finden Sie in der Dokumentation zur [**SetPowerState-Methode**](setpowerstate-method-in-class-cim-controller.md) in [**CIM \_ Battery.**](cim-battery.md)<br/> |
...
Die **Win32 \_ Battery-Klasse** verfügt über diese Eigenschaften.
...
Qualifizierer: [**VERALTET,**](/windows/desktop/WmiSdk/standard-wmi-qualifiers) [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("HKEY \_ LOCAL MACHINE System \_ \\ \\ \\ \\ CurrentControlSet \\ \\ \| Services-Ratenrate"), [**Einheiten**](/windows/desktop/WmiSdk/standard-qualifiers) ("Minuten")
</dt> </dl>

Erforderliche Zeit zum vollständigen Aufladen des Akkus. Diese Eigenschaft wird nicht unterstützt. **BatteryRechargeTime** verfügt nicht über eine Ersatzeigenschaft und gilt jetzt als veraltet.
...
Status des Akkus. Der Wert 10 (Nicht definiert) ist im CIM-Schema ungültig, da er in DMI darstellt, dass kein Akku installiert ist. In diesem Fall sollte das Objekt nicht instanziiert werden.

Diese Eigenschaft wird von [**CIM \_ Battery geerbt.**](cim-battery.md)
...
Das System hat Zugriff auf den Wechselstrom, sodass kein Akku entladen wird. Der Akku wird jedoch nicht unbedingt geladen.
...
<span id="Charging"></span><span id="charging"></span><span id="CHARGING"></span>**Aufladen** (6)
...
<span id="Charging_and_High"></span><span id="charging_and_high"></span><span id="CHARGING_AND_HIGH"></span>**Laden und Hoch** (7)
...
<span id="Charging_and_Low"></span><span id="charging_and_low"></span><span id="CHARGING_AND_LOW"></span>**Laden und Niedrig** (8)
...
<span id="Charging_and_Critical"></span><span id="charging_and_critical"></span><span id="CHARGING_AND_CRITICAL"></span>**Laden und Kritisch** (9)
...
<span id="Partially_Charged"></span><span id="partially_charged"></span><span id="PARTIALLY_CHARGED"></span>**Teilweise berechnet** (11)
...
Entwerfen Sie die Kapazität des Akkus in Milliwattstunden. Wenn die Eigenschaft nicht unterstützt wird, geben Sie 0 (null) ein.
...
Qualifizierer: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. DMTF \| Portable Battery \| 002.9"), Units ("milli milli" (DMTF Portable Akku 002.9"), [**Einheiten**](/windows/desktop/WmiSdk/standard-qualifiers) ("milli milli")
</dt> </dl>

Entwurfsspannung des Akkus in Millisekunden. Wenn das Attribut nicht unterstützt wird, geben Sie 0 (null) ein.
...
Identifiziert den Akku.

Diese Eigenschaft wird von [**CIM \_ LogicalDevice geerbt.**](cim-logicaldevice.md)

Beispiel: "Interner Akku"
...
Freiformzeichenfolge, die weitere Informationen über den in der **LastErrorCode-Eigenschaft** aufgezeichneten Fehler sowie Informationen zu eventuell ergriffenen Korrekturmaßnahmen enthält.
...
Schätzung des Prozentsatzes der verbleibenden vollständigen Gebühr.
...
Qualifizierer: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. DMTF \| Portable Battery \| 002.15"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("minutes")
</dt> </dl>

Schätzen Sie die Zeit in Minuten, in der die Akkuladung unter den aktuellen Auslastungsbedingungen erschöpft ist, wenn die Stromversorgung des Hilfsprogramms ausgeschaltet ist oder verloren geht und aus bleibt oder ein Laptop von einer Stromquelle getrennt ist.
...
Qualifizierer: [**VERALTET,**](/windows/desktop/WmiSdk/standard-wmi-qualifiers) [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("HKEY \_ LOCAL MACHINE System \_ \\ \\ \\ \\ CurrentControlSet \\ \\ Services \| BatteryLife"), [**Einheiten**](/windows/desktop/WmiSdk/standard-qualifiers) ("Minuten")
</dt> </dl>

Die Zeit, die benötigt wird, um den Akku vollständig zu leeren, nachdem er vollständig aufgeladen wurde. Diese Eigenschaft wird nicht mehr verwendet und gilt als veraltet.
...
Die erwartete Akkulebensdauer in Minuten, vorausgesetzt, der Akku ist vollständig aufgeladen. Die -Eigenschaft stellt die erwartete Gesamtlebensdauer des Akkus und nicht seine aktuelle verbleibende Lebensdauer dar, was durch die **EstimatedRunTime-Eigenschaft angegeben** wird.
...
Volle Ladekapazität des Akkus in Milliwattstunden. Der Vergleich des Werts mit der **DesignCapacity-Eigenschaft** bestimmt, wann der Akku austauscht werden muss. Das Ende der Lebensdauer eines Akkus liegt in der Regel vor, wenn die **FullChargeCapacity-Eigenschaft** unter 80 % der **DesignCapacity-Eigenschaft** fällt. Wenn die Eigenschaft nicht unterstützt wird, geben Sie 0 (null) ein.
...
Maximale Zeit in Minuten, um den Akku vollständig zu laden. Die -Eigenschaft stellt die Zeit für das Auffüllen eines vollständig erschöpften Akkus dar, nicht die aktuelle verbleibende Ladezeit, die in der **TimeToFullCharge-Eigenschaft angegeben** ist.
...
Definiert die Bezeichnung, unter der das -Objekt bekannt ist. Bei Unterklassen kann die Eigenschaft überschrieben werden, um eine Schlüsseleigenschaft zu sein.
...
Die [**SetPowerState-Methode**](setpowerstate-method-in-class-cim-controller.md) wird unterstützt. Diese Methode befindet sich in der übergeordneten [**CIM \_ LogicalDevice-Klasse**](cim-logicaldevice.md) und kann implementiert werden. Weitere Informationen finden Sie unter [Entwerfen Managed Object Format -Klassen (MOF).](/windows/desktop/WmiSdk/designing-managed-object-format--mof--classes)
...
Versionsnummer der Datenspezifikation, die vom Akku unterstützt wird. Wenn der Akku diese Funktion nicht unterstützt, sollte der Wert leer gelassen werden.
...
Verstrichene Zeit in Sekunden seit dem letzten Wechsel der USV des Computersystems zum Akkubetrieb oder der Zeit seit dem letzten Neustart des Systems oder der USV, unabhängig davon, welcher Zeitraum geringer ist. Wenn der Akku "online" ist, wird 0 (null) zurückgegeben.
...
Qualifizierer: [**MappingStrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("MIF. DMTF \| Portable Battery \| 002.16"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("minutes")
</dt> </dl>

Verbleibende Zeit zum vollständigen Aufladen des Akkus in Minuten zur aktuellen Laderate und Nutzung.
...
Die **Win32 \_ Battery-Klasse** wird von [**CIM Battery \_ abgeleitet,**](cim-battery.md) die von [**CIM \_ LogicalDevice abgeleitet ist.**](cim-logicaldevice.md)

Windows Server 2008 enthält die USV-Treiber (APC) im Betriebssystem, mit denen Sie die USV als Akkuversorgung behandeln können. Auf diese Weise können Sie den UPS-Status mithilfe eines Skripts überwachen und bei Bedarf Maßnahmen ergreifen.

## <a name="examples"></a>Beispiele

Im [Toggle-Wireless.ps1](https://Gallery.TechNet.Microsoft.Com/Toggle-Wirelessps1-2d244a8f) PowerShell-Codebeispiel wird **Win32 \_ Battery** abgefragt, um zu bestimmen, ob die Funkverbindung ein-/ausschaltet werden soll, um Energie zu sparen.

Im [Beispiel Ups Information](https://Gallery.TechNet.Microsoft.Com/7196121e-97de-4290-9939-26d0ce266270) Perl auflisten werden Informationen zu den unterbrechungsfreie Stromquellen aufgeführt, die an einen Computer angefügt sind.
