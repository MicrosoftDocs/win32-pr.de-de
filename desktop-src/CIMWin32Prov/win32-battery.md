...
---
# <a name="win32_battery-class"></a>Win32- \_ Akku Klasse

Die [WMI-Klasse](/windows/desktop/WmiSdk/retrieving-a-class) des **Win32- \_ Akkus** stellt einen Akku dar, der mit dem Computersystem verbunden ist.
...
Die **Win32- \_ Akku** Klasse verfügt über diese Typen von Membern:
...
Die **Win32- \_ Akku** Klasse verfügt über diese Methoden.



| Methode            | BESCHREIBUNG                                                                                                                                                                                          |
|:------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Zurücksetzen**         | Nicht implementiert. Informationen zur Implementierung dieser Methode finden Sie unter der [**Reset**](reset-method-in-class-cim-controller.md) -Methode in der [**CIM- \_ Akku**](cim-battery.md) -Dokumentation.<br/>                 |
| **SetPowerState** | Nicht implementiert. Informationen zur Implementierung dieser Methode finden Sie unter der [**SetPowerState**](setpowerstate-method-in-class-cim-controller.md) -Methode in der [**CIM- \_ Akku**](cim-battery.md) -Dokumentation.<br/> |
...
Die **Win32- \_ Akku** Klasse verfügt über diese Eigenschaften.
...
Qualifizierer: [**veraltet**](/windows/desktop/WmiSdk/standard-wmi-qualifiers), [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("HKEY \_ local \_ Machine \\ \\ System \\ \\ CurrentControlSet \\ \\ Services \| LADEGERATE"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("Minutes")
</dt> </dl>

Erforderliche Zeit zum vollständigen Berechnen der Akkukapazität. Diese Eigenschaft wird nicht unterstützt. " **Batteryakku** " hat keine Ersatz Eigenschaft und wird nun als veraltet eingestuft.
...
Der Akku Status. Der Wert 10 (nicht definiert) ist im CIM-Schema ungültig, da er in DMI angibt, dass kein Akku installiert ist. In diesem Fall sollte das Objekt nicht instanziiert werden.
...
Das System hat Zugriff auf die Systemsteuerung, sodass kein Akku entladen wird. Der Akku wird jedoch nicht zwangsläufig berechnet.
...
Die Entwurfs Kapazität der Akkukapazität in Milliwatt Stunden. Wenn die Eigenschaft nicht unterstützt wird, geben Sie 0 (null) ein.
...
Die Entwurfs Spannung des Akkus in Millivolt. Wenn das Attribut nicht unterstützt wird, geben Sie 0 (null) ein.
...
Eine frei Form Zeichenfolge, die weitere Informationen über den in der **LastErrorCode** -Eigenschaft aufgezeichneten Fehler sowie Informationen zu eventuell durchzuführenden Korrekturmaßnahmen bereitstellt.
...
Qualifizierer: [**veraltet**](/windows/desktop/WmiSdk/standard-wmi-qualifiers), [**mappingstrings**](/windows/desktop/WmiSdk/standard-qualifiers) ("HKEY \_ local \_ Machine \\ \\ System \\ \\ CurrentControlSet \\ \\ Services \| batterylife"), [**Units**](/windows/desktop/WmiSdk/standard-qualifiers) ("Minutes")
</dt> </dl>

Die Zeitspanne, die benötigt wird, um den Akku vollständig zu entladen, nachdem er vollständig abgerechnet wurde. Diese Eigenschaft wird nicht mehr verwendet und wird als veraltet eingestuft.
...
Erwartete Lebensdauer des Akkus (in Minuten), vorausgesetzt, dass der Akku vollständig abgerechnet wird. Die-Eigenschaft stellt die erwartete Gesamtlebensdauer des Akkus dar, nicht die aktuelle verbleibende Lebensdauer, die durch die **EstimatedRunTime** -Eigenschaft angegeben wird.
...
Volle Ladekapazität des Akkus in Milliwatt Stunden. Der Vergleich des Werts mit der **DesignCapacity** -Eigenschaft bestimmt, wann der Akku ersetzt werden muss. Das Ende der Lebensdauer des Akkus ist normalerweise der Fall, wenn die **FullChargeCapacity** -Eigenschaft unter 80% der **DesignCapacity** -Eigenschaft liegt. Wenn die Eigenschaft nicht unterstützt wird, geben Sie 0 (null) ein.
...
Maximale Zeit in Minuten, um den Akku vollständig zu berechnen. Die-Eigenschaft stellt die Zeit dar, die eine vollständig abgelegte Akkukapazität wieder auflädt, nicht die derzeit verbleibende Ladezeit, die in der **Timeto fullladung** -Eigenschaft angegeben ist.
...
Definiert die Bezeichnung, mit der das-Objekt bekannt ist. Bei einer Unterklasse kann die Eigenschaft als Schlüsseleigenschaft überschrieben werden.
...
Die Versionsnummer der Daten Spezifikation, die vom Akku unterstützt wird. Wenn der Akku diese Funktion nicht unterstützt, sollte der Wert leer gelassen werden.
...
Verstrichene Zeit in Sekunden seit der letzten Umstellung des Computer Systems auf die Akkuleistung oder die Zeit seit dem letzten Neustart des Systems oder der UPS, je nachdem, welcher Wert kleiner ist. Wenn der Akku "on line" ist, wird 0 (null) zurückgegeben.
...
Verbleibende Zeit zum vollständigen Berechnen der Akkukapazität in Minuten zum aktuellen Abrechnungs Kurs und zur aktuellen Nutzung.
...
Die **Win32- \_ Akku** Klasse wird von der CIM- [**\_ Batterie**](cim-battery.md) abgeleitet, die von [**CIM \_ LogicalDevice**](cim-logicaldevice.md)abgeleitet ist.

Windows Server 2008 enthält die (APC) UPS-Treiber im Betriebssystem, sodass Sie die UPS als Akku Versorgung behandeln können. Dies ermöglicht es Ihnen, den Status "UPS" mithilfe eines Skripts zu überwachen und bei Bedarf Aktionen auszuführen.

## <a name="examples"></a>Beispiele

Mit dem [Toggle-Wireless.ps1](https://Gallery.TechNet.Microsoft.Com/Toggle-Wirelessps1-2d244a8f) PowerShell-Codebeispiel wird **Win32- \_ Akku** abgefragt, um zu bestimmen, ob das drahtlos Gerät gewechselt werden soll, um Energie zu sparen.

Das Beispiel [List UPS Information](https://Gallery.TechNet.Microsoft.Com/7196121e-97de-4290-9939-26d0ce266270) perl listet Informationen zu den unterbrechungsfreien Stromquellen an, die an einen Computer angefügt sind.
