...
---
# <a name="cim_usbhub-class"></a>CIM-Klasse für den \_ bhub

Die **CIM \_** -Klasse "-Klasse" stellt die Funktionen und die Verwaltung eines USB-Hubs dar.
...
Die **CIM \_** -Klasse "-Klasse" hat folgende Member:
...
|:------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|
| [**GetDescriptor**](getdescriptor-method-in-class-cim-usbhub.md) | Gibt den USB-Gerätedeskriptor zurück. Wird nicht von WMI implementiert.<br/>                                                                    |
| [**Zurücksetzen**](reset-method-in-class-cim-usbhub.md)                 | Fordert eine zurück setzung des logischen Geräts an. Wird nicht von WMI implementiert.<br/>                                                               |
| [**SetPowerState**](setpowerstate-method-in-class-cim-usbhub.md) | Definiert den gewünschten Energiezustand für ein logisches Gerät und einen Zeitpunkt, zu dem ein Gerät in diesen Zustand versetzt werden soll. Wird nicht von WMI implementiert.<br/> |
...
Diese Eigenschaft wird von CIM-Start [**\_ Gerät**](cim-usbdevice.md)geerbt.
...
Qualifizierer: [**modelcorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("CIM-Start [**\_ Gerät**](cim-usbdevice.md)".**Currentconfigvalue**")
</dt> </dl>

Alternative USB-Einstellungen für jede Schnittstelle in der aktuell ausgewählten Konfiguration (angegeben durch die **currentconfigvalue** -Eigenschaft). Dieses Array enthält einen Eintrag für jede Schnittstelle in der Konfiguration. Wenn die **currentconfigvalue** -Eigenschaft den Wert 0 (null) aufweist, was darauf hinweist, dass das Gerät nicht konfiguriert ist, ist das Array nicht definiert. Weitere Informationen zum Auswerten dieser Oktett-Zeichenfolge finden Sie in der USB-Spezifikation.
...
Qualifizierer: [**modelcorrespondence**](/windows/desktop/WmiSdk/standard-qualifiers) ("CIM-Start [**\_ Gerät**](cim-usbdevice.md)".**Currentalternativ esettings**")
</dt> </dl>

Konfiguration, die zurzeit für das Gerät ausgewählt ist. Wenn dieser Wert 0 (null) ist, wird das Gerät nicht konfiguriert.
...
**True** gibt an, dass die Stromversorgung gleichzeitig auf alle Ports auf dem Hub umgestellt wird. Wenn der Wert **false** ist, wird die Stromversorgung für jeden Port einzeln gewechselt.
...
Anzahl der Geräte Konfigurationen, die für das Gerät definiert sind.
...
Anzahl der Downstream-Ports auf dem Hub, einschließlich derjenigen, die in das-Silicon des Hubs eingebettet sind.
...
Neueste vom USB-Gerät Unterstützte USB-Version. Diese Eigenschaft wird als Binary-Coded Decimal (BCD) ausgedrückt, wobei ein Dezimaltrennzeichen zwischen den zweiten und dritten Ziffern impliziert wird. Der Wert 0x201 gibt z. b. an, dass Version 2,01 unterstützt wird.
...
Die **CIM \_** -Klasse "-Klasse" wird von CIM-Start [**\_ Gerät**](cim-usbdevice.md)abgeleitet.

Diese Klasse wird von WMI nicht implementiert. Informationen zu WMI-Klassen, die von **CIM- \_ Anwendungen** abgeleitet sind, finden Sie unter [Win32-Klassen](win32-provider.md).
