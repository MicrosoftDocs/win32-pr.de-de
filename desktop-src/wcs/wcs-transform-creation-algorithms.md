...
- WCS (Windows Color System), Transformationserstellung
...
Jeder geöffnete ARBEITSBEREICH kann eine andere Anzahl von Schritten in jedem Eingabekanal aufweisen, und die Samplingspeicherorte müssen nicht gleichmäßig verteilt sein. Für ein angegebenes offenes INTEGRAL-Stratum gibt es eine zugeordnete Farbkombination, z. B. ![ Zeigt C subscript 1, ..., C subscript k.](images/transformcreation-image045.gif) , wobei das ![ C-Subskript i anzeigt.](images/transformcreation-image047.gif) s sind unterschiedliche ganze Zahlen zwischen 1 und *n*. Dabei handelt es sich um die Kanalindizes, die den "aktiven" Colorants in dieser Schicht entsprechen.

SCHRITT 1: Filtern Sie das eingegebene Array von Gerätewerten heraus, die nicht in dieser Schicht enthalten sind. Ein Gerätewert ![ Zeigt X Subscript 1, X subscript 2, ..., X subscript n an.](images/transformcreation-image049.gif) ist in der Schicht enthalten, wenn und nur, wenn ![ einen Satz von Werten für einen Kanal anzeigt.](images/transformcreation-image051.gif) und alle anderen Kanäle sind 0. Wenn der gefilterte Satz *N* Einträge enthält, lassen Sie

![Zeigt eine Gleichung an, die verwendet werden soll, wenn der gefilterte Satz über N Einträge verfügt.](images/transformcreation-image053.png)

Für jede ![Zeigt i gleich 1, 2, ..., k an.](images/transformcreation-image041.gif) , iterieren Sie die folgenden Schritte 2-5:

SCHRITT 2: Wenn ![ d subscript-vorläufig (k) gleich 1 anzeigt.](images/transformcreation-image055.gif) , Samp *i* hat nur einen Punkt, der 1,0 sein muss. Fahren Sie mit dem nächsten *i* fort. Fahren Sie andernfalls mit SCHRITT 3 fort.

SCHRITT 3: Sortieren der gefilterten Stichproben in aufsteigender Reihenfolge im ![Zeigt C subscript i an.](images/transformcreation-image047.gif) channel.

SCHRITT 4: Definieren des "vorläufigen" Samplingrasters mithilfe der Knoten

![Zeigt die Knoten an, die zum Definieren des Rasters für die vorläufige Stichprobenentnahme verwendet werden.](images/transformcreation-image057.png)

where ![Zeigt j gleich 1, 2, ..., d subscript vorläufiges (k) an.](images/transformcreation-image059.gif) .

SCHRITT 5: Regularisieren Des vorläufigen Rasters, um sicherzustellen, dass es mit strenger Monotonität übereinstimmt und auch mit 1.0 endet. Da das Array bereits sortiert ist, sind die Knoten im vorläufigen Raster bereits monotone nichtdekreasierende Knoten. Benachbarte Knoten können jedoch identisch sein. Sie können dies beheben, indem Sie bei Bedarf identische Knoten entfernen. Wenn der Endpunkt nach diesem Verfahren kleiner als 1,0 ist, ersetzen Sie ihn durch 1.0.

Beachten Sie, dass SCHRITT 5 der Grund dafür ist, dass die STRATa in jedem Kanal eine andere Anzahl von Schritten aufweisen kann. Nach der Regularisierung kann die Anzahl der Schritte in einem Kanal kleiner als sein. ![Zeigt d subscript vorläufiges (k) an.](images/transformcreation-image061.gif) .

## <a name="interpolation"></a>Interpolation

Sie können die Schichtung des Einheitencubes erstellen, indem Sie offene STRATA- und geschlossene STRATA-Schichten erstellen. Führen Sie die folgenden Schritte aus, um die Interpolation mit dieser "SPARSE-STRUKTUR" durchzuführen. Angenommen, ein angegebener Eingabegerätwert ![Zeigt (X subscript 1, X subscript 2, ..., X subscript n).](images/transformcreation-image049.gif) .

SCHRITT 1: Bestimmen Sie die Anzahl der "aktiven" Kanäle. Dies ist die Anzahl von Kanälen ungleich 0 (null). Dadurch wird die Schichtdimension *k* bestimmt, die nach dem enthaltenden Stratum gesucht werden soll. Genauer gesagt ist die Strata-Dimension 3, wenn die Anzahl der aktiven Kanäle ![ kleiner oder gleich 3 ist.](images/transformcreation-image063.gif) Andernfalls entspricht die Strata-Dimension der Anzahl der aktiven Kanäle.

SCHRITT 2: Innerhalb von ![Zeigt sigma subscript k.](images/transformcreation-image026.gif) , suchen Sie nach dem enthaltenden Stratum. Ein Gerätewert ist in einem offenen Stratum enthalten, wenn alle Kanäle, die dem Stratum entsprechen, einen Wert ungleich 0 (null) aufweisen und alle anderen Kanäle 0 (null) sind. Ein Gerätewert ist in einem geschlossenen Stratum enthalten, wenn jeder Kanal, der nicht durch das Stratum dargestellt wird, 0 (null) ist. Wenn kein enthaltenes Stratum gefunden wird, gibt es eine Fehlerbedingung. Abbrechen und Melden von Fehlern. Wenn ein enthaltende Stratum gefunden wird, fahren Sie mit dem nächsten Schritt fort.

SCHRITT 3: Wenn das enthaltende Stratum geschlossen ist, kann die Interpolation innerhalb des Stratums von jedem bekannten Interpolationsalgorithmus durchgeführt werden. In dieser Implementierung ist die Wahl des Algorithmus die interpolale Interpolation. Wenn das enthaltende Stratum geöffnet ist und der Gerätewert streng innerhalb des Stratums liegt, d. b.

![Zeigt X subscript i größer oder gleich... ](images/transformcreation-image065.gif) Erster Knoten im *i* th-Kanal

Wobei *i* ein Kanalindex für das Stratum ist, funktioniert der Standardinterpolationsalgorithmus, wie z. B. die interpolhedrale Interpolation.

Wenn ![ X subscript i less than... ](images/transformcreation-image067.gif) erster Knoten im i th-Kanal für einige *i* anzeigt, fällt der Gerätewert in die "Lücke" zwischen dem Stratum und den unterdimensionalen Unterräumen.  Diese MOI bezieht sich nicht auf einen Interpolationsalgorithmus an sich, sodass jeder Interpolationsalgorithmus verwendet werden kann, um innerhalb dieser "Lücke" zu interpolieren, obwohl der bevorzugte Algorithmus die folgende transfinite Interpolation ist.

Die Architektur des Interpolationsmoduls wird in den beiden Teilen von Abbildung 1 veranschaulicht.

![Diagramm, das Teil 1 der Architektur des Interpolationsmoduls zeigt.](images/transformcreation-image068.png)

![Diagramm, das Teil 2 der Architektur des Interpolationsmoduls zeigt.](images/transformcreation-image070.png)

**Abbildung 1:** Architektur des Intepolation-Moduls

Wie bereits erläutert, ist dieser Algorithmus in der Lage, eine relativ dichte Stichprobenentnahme in Bereichen des Gerätebereichs zu erreichen, die eine wichtige Kombination von Colorants enthalten, während gleichzeitig die Gesamtgröße der benötigten LUTs minimiert wird. Die folgende Tabelle zeigt einen Vergleich der Anzahl der Knoten, die für die SPARSE-IMPLEMENTIERUNG erforderlich sind (unter Verwendung von Algorithmus \# 1 und normalem Modus) und der entsprechenden einheitlichen UNIFORM-Implementierung.



| **Anzahl der Eingabekanäle** | **SparsePARTy** | **Einheitliches LOSS** |
|------------------------------|----------------|-----------------|
| 5                            | 142498         | 1419857         |
| 6                            | 217582         | 24137567        |
| 7                            | 347444         | 410338673       |
...
Ein grundlegender Schritt im Fall eines rechteckigen Rasters ist die Interpolation innerhalb einer einschließenden Zelle. Für einen Eingabepunkt können Sie die einschließende Zelle leicht bestimmen. In einem rechteckigen Raster wird der Ausgabewert an jedem der Scheitelpunkte (Eckpunkte) der einschließenden Zelle angegeben. Sie sind auch die einzigen Begrenzungsbedingungen , die ein Interpolant erfüllen muss: Das Interpolant muss alle diese Punkte durchlaufen. Beachten Sie, dass sich diese Begrenzungsbedingungen auf "diskreten" Punkten befinden, in diesem Fall auf den 2n Eckpunkten der Zelle, wobei n die Dimension des Farbraums ist.

Es ist hilfreich, das Konzept der Begrenzungsbedingungen zu formalisieren, bevor Sie mit diesem Verfahren fortfahren. Für jede Teilmenge S der Grenze der einschließenden Zelle (der Einheitencube in n Dimensionen) ist eine Begrenzungsbedingung für S eine Spezifikation der Funktion BC: S → Rm, wobei m die Ausgabedimension ist. Anders ausgedrückt: Ein Interpolant, der als Interp: \[ 0,1 \] n→ Rm bezeichnet werden kann, ist erforderlich, um Zu erfüllen: Interp(x) = BC(x) für alle x in S.
...
dabei ist σ:{1,2,..,n}→{1,2,...,n} eine Permutation von "Symbolen" 1, 2, ..., n, d. h. eine Bijektivzuordnung des n-Symbolsatzes. Wenn z.B. n = 3 und σ = (3, 2, 1), was σ(1)=3, σ(2)=2, σ(3)=1 bedeutet, wird das entsprechende leerehedron durch z≥y≥x definiert, wobei die gemeinsame Notation x, y, z für x1, x2, x3 verwendet wird. Beachten Sie, dass diese Klammern nicht voneinander abgeknöpft sind. Zum Zweck der Interpolation haben Punkte, die auf einem gemeinsamen Gesicht von zwei unterschiedlichen Klammern stehen, den gleichen Interpolationswert, unabhängig davon, welches Klammerzeichen in der Interpolation verwendet wird. Im Standardszenario der Interpolation an endlichen Punkten für einen bestimmten Eingabepunkt (x1, ..., xn) müssen Sie jedoch zunächst bestimmen, in welchem Senkrechtthedron es sich befindet, oder entsprechend der entsprechenden Permutation σ, dann wird das interpolale Interpolant für die Klammer als definiert.
...
Der schwierige Teil des Erstellens eines PROFILS für die ATOB- und BToA-Suche ist das Erstellen der AToB- und BToA-Suchtabellen. Diese Tabellen werden zwischen dem Geräteraum, z. B. RGB oder C PCI, und dem Profilverbindungsraum (PCS) dargestellt, bei dem es sich um eine Variante von CIELAB handelt. Dies ist im Wesentlichen identisch mit dem Farbverwaltungsprozess, der in der VOR-Ort-Transformation verwendet wird, um eine Zuordnung vom Gerätebereich zum Geräteraum zu erstellen. Sie müssen jedoch über die folgenden Informationen verfügen, um die Transformation zu ermöglichen.

1) Verweisanzeigebedingungen für den PCS.

2) Referenz zur PCS-Gamut.

3) Gerätemodell, das zwischen PCS-Werten und Farbmetrik konvertiert.

Das WCS-Profil und das zugeordnete WEBCAM werden als Parameter bereitgestellt. Es gibt zwei Baselinegerätemodelle, die zwischen Farbmetrik und PCS-Codierung konvertieren. Der Grund, warum Sie zwei benötigen, wird unten erläutert.
...
2) Sie können auch ein WCS-Profil verwenden, um Beispiele zu speichern, die den Referenzumfang des PCS definieren. Das Farbverwaltungssystem (Color Management System, CMS) von AUX verfügt über zwei Möglichkeiten zum Erstellen von Gamutgrenzen. Eine besteht in der Stichprobenentnahme des gesamten Gerätebereichs und der Verwendung des Gerätemodells zum Erstellen von Messwerten. Die zweite Methode besteht in der Verwendung von gemessenen Stichproben aus dem Profil, um eine Allgemeine Referenzgrenze zu erstellen. Da die Gamut der PCS des PCS zu groß ist, um einen nützlichen Verweis zu machen, ist die erste Methode ungeeignet. Die zweite Methode ist jedoch ein flexibler, profilbasierter Ansatz. Sie können die Messdaten im PCS-Geräteprofil ändern, um den REFERENZ-PCS-Gamut neu zu definieren.
...
Verwenden Sie die beiden gerade erstellten Gamutgrenzen, um einen GMM zu initialisieren. Verwenden Sie dann das Gerätemodell, GMM und das PCS-Gerätemodell, um eine Transformation zu erstellen. Führen Sie eine Stichprobenentnahme des Gerätebereichs durch die Transformation aus, um ein AToB-Intervall zu erstellen.
...
**Abbildung 6:** Erstellen eines BToA-3D-Werts (BToA)
...
Beachten Sie, dass die würfelhafte Anpassung zwischen dem Medien-White point und dem PCS-Weißpunkt (der durch DIETT-Lösung als D50-Punkt bezeichnet wird) transparent durch die WEBCAM erfolgt, da die WEBCAM (CIECAM02 in WCS) an dem Prozess beteiligt ist.

## <a name="hdr-virtual-rgb-devices"></a>Virtuelle HDR-RGB-Geräte

Beim Generieren von Profilen für virtuelle HDR-RGB-Geräte müssen besondere Überlegungen berücksichtigt werden. Das heißt, Geräte, für die die Farbwerte kleiner als 0,0 oder größer als 1,0 sein können. Bei der Generierung der ATOB-EINGANGS-LUTs wird ein größerer Satz von 1D-Eingabe-LUTs erstellt. Farbliche Werte werden skaliert und auf den Bereich 0 versetzt. 1 mit den minimalen und maximalen Farbwerten im WCS-Profil.

Da der Farbraum für HDR-Geräte wahrscheinlich nicht vollständig aufgefüllt wird, wird auch im 3D-FORMAT für das Tag spezielle Unterstützung bereitgestellt. Um Farben im bereich mit geringer Dichte zu verarbeiten, werden die Farbmittel neu codiert, sodass eine Extrapolierung über 0,0 und 1,0 hinaus erreicht werden kann. Der verwendete Bereich ist -1 . +4.

Aufgrund der für die 3D-VERSION angewendeten Neuskalierung wird ein Satz von 1D-Ausgabe-LUTs erstellt, um das Ergebnis wieder dem Bereich 0 zu zuordnen. 1.

## <a name="more-than-one-pcs"></a>Mehrere PCS

Die MANAGEMENT-Lösung hat festgestellt, dass ein PCS nicht flexibel genug war, um alle beabsichtigten Verwendungen eines CMS zu erfüllen. In Version 4 der Profilspezifikation verdeutlichte dieS, dass es tatsächlich zwei PCS-Codierungen gibt. Eine wird für die Farbmetrikabsichten verwendet. ein anderer wird für die wahrnehmliche Absicht verwendet. (Für die Absicht Sättigung ist kein PCS angegeben. Der BEREICH HAT diesen Teil mehrdeutig gelassen.) Für die Farbmetrik-PCS ist eine minimale und maximale Lichtheit angegeben, aber die Werte für "hue" und "hue" liegen ungefähr bei ± 127. Dieser PCS sieht wie ein rechteckiges Prisma aus. Wie bereits erwähnt, ähnelt das wahrnehmliche PCS-Volume dem Gamut eines Druckers.

Die beiden PCS vonCS verfügen auch über zwei unterschiedliche digitale Codierungen. In der wahrnehmlichen PCS stellt der Wert 0 (null) eine Lichtheit von 0 (null) dar. In der farbmetrikmetrischen PCS stellt der Wert 0 (null) die minimale Lichtheit der PCS dar, die größer als 0 (null) ist. Sie können dieses Problem lösen, indem Sie für jede PCS-Codierung ein anderes Gerätemodell verwenden.

## <a name="gamut-mapping"></a>Gamut-Zuordnung

Zum Erstellen der AToB-LUTs in einem PCI-Profil ordnen Sie die Geräte gamut dem entsprechenden PCS-Bereich zu. Um die BToA-LUTs zu erstellen, ordnen Sie den PCS-Bereich dem Gamut des Geräts zu. Die Zuordnung für die AToB-LUTs ähnelt der Zuordnung in einem messbasierten CMS. Ordnen Sie für die wahrnehmbare PCS den Gamut-Gamut des Geräts der wahrnehmlichen PCS-Gamutgrenze zu, indem Sie entweder Clipping oder Komprimierung für alle farbenverfälssten Farben verwenden. Für die farbmetrikmetrischen Absichten müssen Sie möglicherweise die Lichtheit beschneiden, aber die Werte für "hue" und "hue" passen alle in den Farbmetrik-PCS-Gamut.

Die Zuordnung für die BToA-LUTs ist etwas anders. Die farbmetrikmetrischen Absichten sind immer noch einfach. Sie beschneiden einfach PCS-Werte auf den Gamut des Geräts. Allerdings erfordert dieS, dass alle möglichen PCS-Werte einem Gerätewert und nicht nur denen innerhalb des Referenzumfangs des wahrnehmlichen PCS zuordnen. Daher müssen Sie sicherstellen, dass die GMMs Quellfarben verarbeiten können, die sich außerhalb des Referenz-Gamuts befinden. Dies kann durch Clipping dieser Farben an die Geräte-Gamutgrenze behandelt werden.

## <a name="baseline-device-to-icc-profile-class-mapping"></a>Zuordnung von Baselinegerät zu PROFILklasse "BASELINE"



| Baselinegerätetyp              | PROFIL-Klasse FÜR DIE KLASSE       | Anmerkung                                                                      |
|-----------------------------------|-------------------------|-----------------------------------------------------------------------------|
| RGB-Erfassungsgerät                | Eingabegerät ("scnr")   | PCS ist CIELAB. AToB0Tag ist Device to PCS with relative colorimetric intent (Gerät zu PCS mit relativer Farbmetrikabsicht). |
| CRT- undTS-Monitor                  | Display Device ("mntr") | PCS ist CIEXYZ. Informationen zur Modellkonvertierung finden Sie im Folgenden.                      |
| RGB-Projektor                     | Farbraum ("Abstand")    | PCS ist CIELAB.                                                              |
| RGB- und C RGB-Drucker              | Ausgabegerät ("prtr")  | PCS ist CIELAB.                                                              |
| Virtuelles RGB-Gerät (Nicht-HDR-Fall) | Display Device ("mntr") | PCS ist CIEXYZ.                                                              |
...
Die Konvertierung von Monitorprofilen umfasst nicht das Erstellen von LUTs, sondern das Erstellen einer Matrix oder eines TRC-Modells. Das modell, das inTSB verwendet wird, ist etwas anders als das in der WCS CRT- oder DERTS-Modellierung verwendete Modell, da der Begriff "schwarze Korrektur" fehlt. Sie haben folgende Möglichkeiten:

WCS-Modell: ![Zeigt ein W C S-Modell an.](images/transformcreation-image132.png)

MODELLS: ![Zeigt ein I C C-Modell an.](images/transformcreation-image134.png)

Die Konvertierung vom WCS-Modell in dasSTELLUNG-Modell erfolgt wie folgt.

Definieren neuer Kurven:

![Zeigt eine Matrix zum Definieren neuer Kurven an.](images/transformcreation-image136.png)

Dies sind keine Tonverlaufskurven, da sie nicht 1 zu 1 zuordnen. Eine Normalisierung erreicht dies. Die endgültigen Definitionen des MODELLS FÜR DIES sind:

![Zeigt die endgültigen Definitionen des I C C-Modells an.](images/transformcreation-image138.png)

![Zeigt die endgültige Matrix für das I C C-Modell.](images/transformcreation-image140.png)

Für virtuelle RGB-Geräte ohne HDR generieren Sie auch ein DISPLAY-PROFIL für die Raumeffizienz. In diesem Fall kann die Tristimulus-Matrix *M QUIZ* direkt aus den Primärwerten des WCS-Profils ohne die obige Modellkonvertierung erhalten werden. Ein letzter, aber wichtiger Hinweis ist, dass diese Tristimulus-Matrix logisch an D50 angepasst werden muss, um der PCI-Spezifikation des PCS zu entsprechen. Anders ausgedrückt: Die Einträge in jeder Zeile der Matrix, die im PROFIL DESTB codiert werden soll, müssen 96,42, 100 und 82,49 summieren. In der aktuellen Implementierung wird dietic-Anpassung von CAT02 durchgeführt, bei der es sich auch um die inCAM02 verwendete versortte Anpassungstransformation handelt.

## <a name="black-preservation-and-black-generation"></a>Black Preservation und Black Generation

Die Implementierung der schwarzer Aufbewahrung ist mit der Generierung des schwarzen Kanals auf Geräten verknüpft, die einen schwarzen Kanal unterstützen. Um dies zu erreichen, werden Informationen zu jeder Quellfarbe gesammelt, damit Gerätemodelle, die einen schwarzen Kanal unterstützen, bestimmen können, wie der schwarze Kanal am besten für die Ausgabe festgelegt werden kann. Obwohl die Schwarzerbewahrung für Farbtransformationen relevant ist, die zwischen einem Schwarzkanalgerät in ein anderes konvertieren, wird die schwarze Generation für alle Transformationen implementiert, die ein Zielgerät mit schwarzem Kanal verwenden.

Informationen zum schwarzen Kanal werden in einer Datenstruktur namens [**BlackInformation aufgezeichnet.**](/previous-versions/windows/desktop/api/WcsPlugIn/ns-wcsplugin-_blackinformation) Die **BlackInformation-Struktur** enthält einen booleschen Wert, der angibt, ob die Farbe nur ein schwarzes Färbungs- und einen numerischen Wert enthält, der den Grad der "Schwarzheit" angibt, der als schwarzer Gewichtung bezeichnet wird. Bei Quellgeräten, die einen schwarzen Kanal unterstützen, ist das schwarze Gewicht der Prozentsatz der schwarzen Farbgebung in der Quellfarbe. Für Quellgeräte, die keinen schwarzen Kanal enthalten, wird das schwarze Gewicht mithilfe der anderen Farbelemente und des Darstellungswerts berechnet. Ein Wert namens "Farberhaltung" wird berechnet, indem der Unterschied zwischen dem maximalen Farbwert und dem minimalen Farbwert dividiert durch den maximalen Farbwert ermittelt wird. Ein Wert namens "Relative Lightness" wird berechnet, indem der Unterschied zwischen der Lichtheit der Farbe und der minimalen Lichtheit für das Zielgerät dividiert durch den Unterschied zwischen der minimalen und der maximalen Lichtheit für das Zielgerät geteilt wird. Wenn es sich bei dem Quellgerät um ein additives Gerät (Monitor oder Projektor) handelt, wird das schwarze Gewicht als 1,0 abzüglich der Farbvermehrung multipliziert mit der relativen Lichtheit bestimmt. Wenn es sich bei dem Quellgerät beispielsweise um einen RGB-Monitor handelt, werden der Höchst- und Mindestwert von R, G und B für jede Farbe berechnet, und das schwarze Gewicht wird durch die Formel bestimmt:

BW = (1,0 – (max(R,G,B) – min(R,G,B)) / max(R, G, B)) \* relative Lightness

Wenn das Quellgerät subtraktive Farbgebung unterstützt, z. B. einen CMY-Drucker, müssen die einzelnen Farbmittel "komplementiert" (subtrahiert von 1,0) sein, bevor sie in der obigen Formel verwendet werden. Für einen CMY-Drucker also R = 1,0 – C, G = 1,0 – M und B = 1,0 – Y.

Die schwarzen Informationen für jede Farbe, die von der Farbtransformation verarbeitet wird, werden während des Farbübersetzungsprozesses bestimmt. Die Nur-Schwarz-Informationen werden nur bestimmt, wenn die schwarze Aufbewahrung angegeben ist. Das schwarze Gewicht wird immer bestimmt, wenn das Zielgerätemodell ein schwarzes Farbmittel unterstützt. Die schwarzen Informationen werden über die [**ColorimetricToDeviceColorsWithBlack-Methode**](/previous-versions/windows/desktop/api/WcsPlugIn/nf-wcsplugin-idevicemodelplugin-colorimetrictodevicecolorswithblack) an das Zielgerätemodell übergeben, die den resultierenden DABEI verwendet.

Beachten Sie, dass der oben beschriebene Prozess aufgrund der Optimierung der Farbtransformation nur während der Erstellung der optimierten Transformierungs-ANWEISUNG und nicht während der Ausführung der TranslateColors-Methode erfolgt.
...
Größe = Anzahl der Schritte pro <sub>Kanalquelle\ Gerät(Anzahl\ von\ Kanälen\ in\ Quelle\ Gerät)</sub> x Anzahl der Kanäle im Ausgabegerät

Wie Sie sehen können, wächst die Größe der Tabelle exponentiell, abhängig von der Anzahl der Kanäle auf dem Quellgerät. Viele Quellgeräte unterstützen drei Farbkanäle, z. B. Rot, Grün und Blau. Wenn ein Quellgerät jedoch vier Kanäle unterstützt, z. B. C WIE, wachsen die Größe der Tabelle und die Zeit, die zum Erstellen der Tabelle erforderlich ist, um den Faktor der Anzahl von Schritten. In einem messbasierten CMS, bei dem Transformationen "im Fluge" erstellt werden, ist dieses Mal möglicherweise nicht akzeptabel.

Um die Zeit zu reduzieren, die zum Erstellen der Farbkonvertierungstabelle erforderlich ist, ist es möglich, zwei Fakten zu nutzen. Erstens: Während das Quellgerät möglicherweise mehr als drei Farbkanäle unterstützt, verfügt der geräteunabhängige Zwischenfarbraum (CIECAM02 Ja <sub>C</sub> b <sub>C</sub> ) nur über drei Farbkanäle. Zweitens ist der zeitaufwändigste Teil der Verarbeitung nicht die Gerätemodellierung (Konvertierung von Gerätefarbkoordinaten in Tristimuluswerte), sondern die Gamutzuordnung. Anhand dieser Fakten können Sie eine vorläufige Farbkonvertierungstabelle erstellen, die Farben im geräteunabhängigen Farbraum über die Gamutzuordnungsschritte und schließlich über das Ausgabegerätefarbmodell konvertiert. Die Konstruktion dieser Tabelle hat die dritte Dimension. Anschließend erstellen wir die dimension four final color-conversion table, indem wir die Quellfarbkombinationen in zwischengeschalteten geräteunabhängigen Raum konvertieren und dann die Konvertierung in den Ausgabegerätefarbraum mithilfe der vorläufigen Farbkonvertierungstabelle beenden. Daher verringern Sie die Anzahl von Berechnungen (Anzahl der Schritte in der Nachschlagetabelle) <sub>von\\Channels</sub> gamut mapping computations auf die Anzahl der Schritte in der Zwischentabelle ₃ gamut mapping computations. Auch wenn Sie die Anzahl der Schritte in der (Nachschlagetabelle) <sub>Anzahl\ von\Kanälenberechnungen</sub> der Gerätemodellierung und der suche nach dreidimensionalen Tabellen ausführen müssen, ist dies immer noch viel schneller als die ursprüngliche Berechnung.

Der vorherige Prozess funktioniert gut, sofern keine Informationen zwischen dem Quellgerätemodell und anderen Komponenten in der Farbtransformation übergeben werden müssen. Wenn jedoch sowohl das Ausgabegerät als auch das Quellgerät ein schwarzes Färbungsprodukt unterstützen und das schwarze Quellfärbungs-Färbungsgerät verwendet wird, um das Schwarz-Ausgabe-Färbungsprodukt zu bestimmen, kann der Prozess die Schwarz-Quellinformationen nicht ordnungsgemäß kommunizieren. Ein alternativer Prozess ist das Erstellen einer vorläufigen Farbkonvertierungstabelle, die Farben im geräteunabhängigen Farbraum nur über die Gamut-Zuordnungsschritte konvertiert. Erstellen Sie dann die dimension four final color-conversion table mit den folgenden Schritten: a) konvertieren Sie die Quellfarbkombinationen in zwischengeschalteten geräteunabhängigen Raum, b) führen Sie die Gamut-Zuordnungsschritte aus, indem Sie in der vorläufigen Farbtabelle interpolieren, anstatt die tatsächlichen Gamut-Zuordnungsprozesse zu verwenden, und c) verwenden Sie die resultierenden Werte aus den Gamut-Zuordnungsschritten und alle Informationen zum schwarzen Quellkanal, um die Farbmittel des Ausgabegeräts mithilfe des Ausgabegerätemodells zu berechnen. Dieser Prozess kann auch verwendet werden, wenn Informationen zwischen dem Quell- und dem Ausgabegerätemodell übertragen werden, auch wenn kein schwarzer Kanal verfügbar ist. Beispiel: Die beiden Module werden mit einer Plug-In-Architektur implementiert, die den Datenaustausch zwischen Modulen ermöglicht.

Die vorherigen beiden Prozesse können verwendet werden, um die Zeit effektiv zu verbessern, die zum Erstellen der vierdimensionalen Farbtransformationstabelle erforderlich ist.

### <a name="checkgamut"></a>CheckGamut

Die ICM CreateTransform und **CreateMultiProfileTransform** verwenden ein Wort mit Flagwerten, von denen einer ENABLE \_ GAMUT \_ CHECKING ist. Wenn dieses Flag festgelegt ist, muss die Transformation von DAHER anders erstellt werden. Die ersten Schritte sind identisch: Die Quell- und Ziel-CAMs müssen initialisiert werden, und dann müssen die Quell- und Ziel-Gamut-Begrenzungsdeskriptoren initialisiert werden. Unabhängig von der angegebenen Absicht muss checkGamut GMM verwendet werden. CheckGamut GMM sollte mithilfe der Quell- und Zielgerätemodelle und gamut-Begrenzungsdeskriptoren initialisiert werden. Die Transformation sollte dann jedoch eine abgeschnittene Transformation erstellen, die das Quellgerätemodell, die Quell-CAM, alle ineinanderliegenden GMMs und CheckGamut GMM umfasst. Dadurch wird sichergestellt, dass die delta J-, Delta C- und delta h-Werte, die vom CheckGamut CMM ausgegeben werden, zu den endgültigen resultierenden Werten werden.

Die Bedeutung von CheckGamut ist klar, wenn die Transformation nur zwei Geräteprofile auft. Wenn mehr als zwei Geräteprofile und mehr als zwei GMMs verfügbar sind, meldet CheckGamut, ob die Farben, die durch das erste Gerätemodell und alle bis auf den letzten GMM transformiert wurden, in den Gamut des Zielgeräts fallen.
